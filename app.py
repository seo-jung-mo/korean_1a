...
# Streamlit can execute this file without adding its directory to sys.path.
# Resolve sibling modules from the app's actual location, not the launch folder.
APP_DIRECTORY = str(Path(__file__).resolve().parent)
if APP_DIRECTORY not in sys.path:
    sys.path.insert(0, APP_DIRECTORY)
try:
    from lesson_progress import (
        LessonProgressStore, completion_steps, course_progress,
        is_lesson_key, lesson_key_unit, unlocked_units,
    )
except ModuleNotFoundError:
    # Some Streamlit launchers do not add the script directory to sys.path.
    # Load the sibling module by its absolute path as a final fallback.
    import importlib.util

    _progress_module_path = Path(__file__).resolve().with_name("lesson_progress.py")
    _progress_spec = importlib.util.spec_from_file_location("lesson_progress", _progress_module_path)
    ...
    LessonProgressStore = _progress_module.LessonProgressStore
    completion_steps = _progress_module.completion_steps
    course_progress = _progress_module.course_progress
    is_lesson_key = _progress_module.is_lesson_key
    lesson_key_unit = _progress_module.lesson_key_unit
    unlocked_units = _progress_module.unlocked_units
...
@st.cache_data(show_spinner=False)
def crop_reference_image_region(image_path, crop_box):
    """Crop a reference scene while leaving corner rounding to the shared 10px CSS."""
    with Image.open(image_path) as source:
        return source.convert("RGBA").crop(crop_box)
...
def reset_unit2_activity1_phone_choices():
    """Reset dependent phone answers when a different person is selected."""
    for key in (
        "unit2_activity1_heard_phone",
        "unit2_activity1_confirmation",
        "unit2_activity1_confirmation_result",
        "unit2_activity1_part2_passed",
    ):
        st.session_state.pop(key, None)
    st.session_state["unit2_activity1_phone_reset_nonce"] = st.session_state.get("unit2_activity1_phone_reset_nonce", 0) + 1


def close_unit2_activity1_confirmation_feedback():
    """Close feedback and clear both selected answers' visual emphasis."""
    for key in (
        "unit2_activity1_heard_phone",
        "unit2_activity1_confirmation",
        "unit2_activity1_confirmation_result",
...
def lesson_button(*args, **kwargs):
    """Keep clicks transient and explain every disabled learning button."""
    show_disabled_help = kwargs.pop("show_disabled_help", True)
    label = str(args[0] if args else kwargs.get("label", ""))
    if label in {
        "정답 확인", "Check answer", "다음 문장 →", "Next sentence →",
        "1번 답 확인", "2번 답 확인", "Check Activity 1 answers", "Check Activity 2 answer",
    }:
        show_disabled_help = False
    if not show_disabled_help:
        kwargs.pop("help", None)
    key = kwargs.get("key")
    if re.fullmatch(r"unit[1-5]_intro_sequence_(?:locked_)?[0-3]", str(key)):
        show_disabled_help = False
    if key is not None:
        st.session_state.setdefault("_lesson_buttons", set()).add(key)
    disabled_help = None
    if kwargs.get("disabled") and show_disabled_help:
        disabled_help = kwargs.pop("help", None) or disabled_button_help(label, key)
    clicked = getattr(st, "button")(*args, **kwargs)
    if disabled_help:
        st.markdown(
            f'<div class="disabled-button-guide" role="status">'
            f'{html.escape(disabled_help)}</div>',
            unsafe_allow_html=True,
        )
    return clicked


def disabled_button_help(label, key=None):
    """Explain why a button is unavailable and what the learner should do."""
    label_text = str(label)
    key_text = str(key or "")
    if any(marker in label_text for marker in ("완료 ✓", "제출 완료 ✓", "complete ✓", "submitted ✓")):
        return interface_text("이미 완료한 활동입니다. 다음 학습 단계로 이동하세요.", "This activity is already complete. Continue to the next learning step.")
    review_help = {
        "task_vocab": ("오늘의 어휘 3개를 모두 소리 내어 읽고 체크해 주세요.", "Read all three vocabulary words aloud and check each one."),
        "task_grammar": ("먼저 어휘 복습을 완료하면 문법 복습이 열립니다.", "Complete the vocabulary review first to unlock grammar review."),
        "task_sentence": ("먼저 문법 복습을 완료하면 문장 조합이 열립니다.", "Complete the grammar review first to unlock sentence building."),
    }
    if key_text in review_help:
        return interface_text(*review_help[key_text])
    if key_text.startswith("builder_word_"):
        return interface_text("현재 문장이 끝났습니다. 아래의 다음 문장 버튼으로 이동하세요.", "This sentence is complete. Use the Next Sentence button below.")
    if "builder" in key_text and any(token in key_text for token in ("submit", "check")):
        return interface_text("먼저 단어를 순서대로 선택해 문장을 완성해 주세요.", "Select the words in order to complete the sentence first.")
    if "activity2_submit" in key_text:
        unit_match = re.search(r"(\d+)$", key_text)
        unit_number = int(unit_match.group(1)) if unit_match else st.session_state.get("selected_unit_number", 1)
        if not get_unit_completion_steps(unit_number)[3]:
            return interface_text("먼저 활동 1을 완료하면 활동 2를 제출할 수 있습니다.", "Complete Activity 1 first to submit Activity 2.")
        return interface_text("활동 2의 필수 문장·선택·읽기 항목을 모두 완료해 주세요.", "Complete all required writing, selection, and reading items in Activity 2.")
    if any(token in key_text for token in ("grammar1_done", "grammar2_done", "activity1_done")):
        return interface_text("먼저 이 단계의 문제·말하기·읽기 활동을 모두 완료해 주세요.", "Complete all questions, speaking, and reading activities in this step first.")
    if any(token in key_text for token in ("check", "save")) or any(token in label_text for token in ("정답", "답 확인", "Check")):
        return interface_text("아직 필요한 답이 모두 입력되지 않았습니다. 빈 문항을 선택하거나 작성해 주세요.", "Some required answers are missing. Select or enter every answer first.")
    if any(token in label_text for token in ("다음", "Next")):
        return interface_text("현재 활동을 먼저 정확하게 완료하면 다음으로 이동할 수 있습니다.", "Complete the current activity correctly before continuing.")
    if any(token in label_text for token in ("완료", "제출", "Complete", "Submit")):
        return interface_text("먼저 이 활동의 필수 입력·선택·읽기 항목을 모두 완료해 주세요.", "Complete all required input, selection, and reading items in this activity first.")
    return interface_text("먼저 화면의 필수 학습 항목을 완료해 주세요.", "Complete the required learning items on this screen first.")


def prepare_lesson_progress():
    """Restore once, then detach saved answer keys from widget cleanup."""
    # Older saves may contain button values, which Streamlit cannot restore.
    for key in list(st.session_state):
        if key.startswith("unit1_intro_sequence_") and key.rsplit("_", 1)[-1].isdigit():
            st.session_state.pop(key, None)
    saved_values = st.session_state.get("_lesson_saved_values", {})
    for key in list(saved_values):
        if key.startswith("unit1_intro_sequence_") and key.rsplit("_", 1)[-1].isdigit():
            saved_values.pop(key, None)
    if st.session_state.get("_unit2_phone_choices_version", 0) < 2:
        # Phone-number radio answers are dependent on the selected person and
        # must not appear preselected when the lesson is first opened.
        for key in list(st.session_state):
            if key.startswith(("unit2_activity1_heard_phone", "unit2_activity1_confirmation")):
                st.session_state.pop(key, None)
        st.session_state["_unit2_phone_choices_version"] = 2
    # Earlier Unit 2 next buttons were accidentally saved as answer state.
    # Remove those values before Streamlit creates the transient buttons.
    saved_values = st.session_state.get("_lesson_saved_values", {})
    for key in list(saved_values):
        if key.startswith("grammar_next_2_"):
            saved_values.pop(key, None)
            st.session_state.pop(key, None)
    ...
    if "_lesson_store" not in st.session_state:
        ...
        replay_cleanup = {
            "unit1_vocab_reviewing": ("vocab_read_cards_1", "vocab_index_1", "vocab_category_1", "vocab_revealed_1", "unit1_read_", "unit1_intro_sequence_", "unit1_picture_"),
            "unit1_grammar1_reviewing": ("unit1_grammar1_",),
            "unit1_grammar2_reviewing": ("unit1_grammar2_",),
            "unit1_activity1_reviewing": ("unit1_activity1_", "activity1_completed_1"),
            "unit1_activity2_reviewing": ("unit1_activity2_", "unit1_final_", "activity2_submission_notice_1"),
        }
        for flag, prefixes in replay_cleanup.items():
            if values.get(flag):
                old_nonce = int(values.get(flag.replace("_reviewing", "_reset_nonce"), 0) or 0)
                for key in list(values):
                    if key == flag or key.endswith("_rewarded_1"):
                        continue
                    if any(key == prefix or key.startswith(prefix) for prefix in prefixes):
                        values.pop(key, None)
                if "reset_nonce" in flag or flag.endswith("activity2_reviewing") or flag.endswith("grammar2_reviewing"):
                    values[flag.replace("_reviewing", "_reset_nonce")] = old_nonce + 1
    ...
    for key in st.session_state["_lesson_saved_values"]:
        if is_lesson_key(key) and key in st.session_state:
            st.session_state[key] = st.session_state[key]


def checkpoint_lesson_progress():
    """Commit validated history and current answers, including before reruns."""
    previous = st.session_state["_lesson_history"]
    history = {
        unit: sum(get_unit_completion_steps(unit)) for unit in range(1, 11)
    }
    buttons = st.session_state.get("_lesson_buttons", set())
    values = {
        key: value for key, value in st.session_state.items()
        if is_lesson_key(key) and key not in buttons
    }
    changed = any(history[unit] > previous.get(unit, 0) for unit in history)
    if changed or values != st.session_state["_lesson_saved_values"]:
        st.session_state["_lesson_history"] = st.session_state["_lesson_store"].save(history, values)
        st.session_state["_lesson_saved_values"] = deepcopy(values)
    return changed
...
def restart_current_unit_from_home(unit_number):
    """Clear unfinished UI state while preserving immutable completion history."""
    for key in list(st.session_state):
        if lesson_key_unit(key) == unit_number:
            st.session_state.pop(key, None)
    st.session_state.selected_unit_number = unit_number
    st.session_state.page_nav = "내 학습"
    st.session_state.force_unit_home_step = unit_number
    st.session_state.scroll_learning_home = True
    if unit_number == 1:
        st.session_state.pop("_unit1_lesson_tabs", None)
...
def advance_intro_reading(unit_number, line_count):
    ...
    line_key = f"unit{unit_number}_read_line"
    current_round = st.session_state.get(round_key, 0)
    if current_round >= 3:
        return
    next_line = st.session_state.get(line_key, 0) + 1
    st.session_state[round_key] = current_round + next_line // line_count
    st.session_state[line_key] = next_line % line_count


def restart_intro_reading(unit_number):
    st.session_state[f"unit{unit_number}_read_round"] = 0
    st.session_state[f"unit{unit_number}_read_line"] = 0
    st.session_state.pop(f"unit{unit_number}_intro_sequence_requested", None)


def render_intro_reading_controls(unit_number, current_round, line_count, *, completed_actions=True):
    """Keep both actions in the same row before and after reading completion."""
    completed = current_round >= 3
    with st.container(key=f"unit{unit_number}_intro_reading_controls"):
        status = interface_text("대화 3회 읽기를 완료했어요 ✓", "You completed three readings ✓") if completed else interface_text(
            f"{current_round + 1}회차 · 강조된 한 줄을 소리 내어 읽고 버튼을 누르세요.",
            f"Round {current_round + 1} · Read the highlighted line aloud, then press the button.",
        )
        if completed and st.session_state.get(f"unit{unit_number}_intro_sequence_requested", False):
            status = interface_text(
                "이제 순서 확인 카드의 1번부터 차례대로 눌러 보세요.",
                "Now select the sequence cards in order, starting with number 1.",
            )
        st.markdown(f'<div class="intro-reading-status" role="status">{html.escape(status)}</div>', unsafe_allow_html=True)
        if completed and not completed_actions:
            return
        reading_column, next_column = st.columns([1, 1.2], vertical_alignment="top")
        with reading_column:
            if completed and completed_actions:
                lesson_button(interface_text("대화 다시 학습하기", "Practice the dialogue again"),
                              key=f"unit{unit_number}_intro_reading_restart", width="stretch",
                              on_click=restart_intro_reading, args=(unit_number,))
            elif not completed:
                lesson_button(interface_text("이 문장 읽었어요 →", "I read this sentence →"),
                              key=f"unit{unit_number}_read_advance", type="primary", width="stretch",
                              on_click=advance_intro_reading, args=(unit_number, line_count))
        with next_column:
            label = {
                1: interface_text("다음: 자기소개 순서 확인하기 →", "Next: Review the self-introduction order →"),
                2: interface_text("다음: 대화 순서 확인하기 →", "Next: Review the dialogue order →"),
            3: interface_text("다음: 물건·위치 순서 확인하기 →", "Next: Review the object-location order →"),
            4: interface_text("다음: 오늘 할 일 순서 확인하기 →", "Next: Review the activity dialogue order →"),
            5: interface_text("다음: 장보기 대화 순서 확인하기 →", "Next: Review the shopping dialogue order →"),
            }.get(unit_number, interface_text(
                f"다음: {LATER_UNIT_INTRODUCTIONS[unit_number]['sequence'][0]} 확인하기 →",
                f"Next: Review the {LATER_UNIT_INTRODUCTIONS[unit_number]['sequence'][1]} →",
            ) if unit_number in LATER_UNIT_INTRODUCTIONS else interface_text("다음 단계로 이동하기 →", "Continue to the next step →"))
            if completed and completed_actions:
                lesson_button(label, key=f"unit{unit_number}_intro_reading_continue", type="primary", width="stretch",
                              on_click=set_session_state_value,
                              args=(f"unit{unit_number}_intro_sequence_requested", True))
            elif not completed:
                lesson_button(label, key=f"unit{unit_number}_intro_reading_continue_locked", disabled=True,
                              help=interface_text(
                                  f"대화 모델을 3회 읽으면 열립니다. 현재 {current_round}/3회 완료했어요.",
                                  f"This opens after three readings. You have completed {current_round}/3.",
                              ), width="stretch")


def render_intro_sequence_success(unit_number):
    if unit_number == 1:
        message = interface_text(
            "자기소개 순서를 모두 확인했어요 ✓",
            "You checked the full self-introduction order ✓",
        )
    elif unit_number == 2:
        message = interface_text(
            "전화번호 대화 순서를 모두 확인했어요 ✓",
            "You checked the full phone-number dialogue order ✓",
        )
    elif unit_number == 3:
        message = interface_text("물건과 위치 순서를 모두 확인했어요 ✓", "You checked the full object and location order ✓")
    elif unit_number == 4:
        message = interface_text("오늘 할 일 대화 순서를 모두 확인했어요 ✓", "You checked the full activity dialogue order ✓")
    elif unit_number == 5:
        message = interface_text("장보기 대화 순서를 모두 확인했어요 ✓", "You checked the full shopping dialogue order ✓")
    else:
        message = interface_text("대화 순서를 모두 확인했어요 ✓", "You checked the full dialogue order ✓")
    render_learning_success(message, icon=":material/check_circle:")


def request_lesson_tab(unit_number, tab_index):
    """Request a visible lesson-tab change after Streamlit finishes rerunning."""
    st.session_state.requested_lesson_tab = (unit_number, tab_index)
    if unit_number == 1:
        st.session_state.unit1_intro_collapsed = tab_index > 0
    elif unit_number == 2:
        st.session_state.unit2_intro_collapsed = tab_index > 0
    elif unit_number == 3:
        st.session_state.unit3_intro_collapsed = tab_index > 0
    elif unit_number == 4:
        st.session_state.unit4_intro_collapsed = tab_index > 0
    elif 5 <= unit_number <= 10:
        st.session_state[f"unit{unit_number}_intro_collapsed"] = tab_index > 0


def collapse_unit2_intro_on_tab_change(first_tab_label):
    st.session_state.unit2_intro_collapsed = (
        st.session_state.get("_unit2_lesson_tabs") != first_tab_label
    )


def collapse_unit3_intro_on_tab_change(first_tab_label):
    st.session_state.unit3_intro_collapsed = (
        st.session_state.get("_unit3_lesson_tabs") != first_tab_label
    )


def collapse_unit4_intro_on_tab_change(first_tab_label):
    st.session_state.unit4_intro_collapsed = (
        st.session_state.get("_unit4_lesson_tabs") != first_tab_label
    )


def collapse_unit5_intro_on_tab_change(first_tab_label):
    st.session_state.unit5_intro_collapsed = (
        st.session_state.get("_unit5_lesson_tabs") != first_tab_label
    )


def collapse_later_unit_intro_on_tab_change(unit_number, first_tab_label):
    st.session_state[f"unit{unit_number}_intro_collapsed"] = (
        st.session_state.get(f"_unit{unit_number}_lesson_tabs") != first_tab_label
    )


def collapse_unit1_intro_on_tab_change(first_tab_label):
    """Collapse the introduction when entering any stage after vocabulary."""
    st.session_state.unit1_intro_collapsed = (
        st.session_state.get("_unit1_lesson_tabs") != first_tab_label
    )


def restart_unit1_grammar1():
    """Clear Unit 1 Grammar 1 inputs while preserving earned history and XP."""
    reset_prefixes = (
        "unit1_grammar1_",
        "unit1_textbook_activities_opened",
        "grammar1_index_1",
        "grammar1_result_1",
        "grammar1_checked_choice_1_",
        "grammar1_feedback_visible_1_",
        "grammar_quiz_choice_1_",
    )
    for key in list(st.session_state):
        if any(key == prefix or key.startswith(prefix) for prefix in reset_prefixes):
            st.session_state.pop(key, None)
    for index in range(4):
        st.session_state[f"unit1_grammar1_picture_{index}"] = None
    st.session_state["unit1_grammar1_friend_name"] = ""
    st.session_state["unit1_grammar1_friend_country"] = None
    st.session_state["unit1_grammar1_final_choice"] = None
    st.session_state.unit1_grammar1_reviewing = True
    st.session_state.requested_lesson_tab = (1, 1)
    st.session_state.unit1_intro_collapsed = True


def complete_unit1_grammar1():
    ...
    request_lesson_tab(1, 1)


def unit1_wrap_up_ready():
    """Require Activity 2 and all three review stages."""
    return get_unit_completion_steps(1)[4] and all(
        st.session_state.get(key, False)
        for key in (
            "review_vocab_done_1", "review_grammar_done_1", "review_sentence_done_1",
        )
    )
...
def restart_unit1_stage(stage):
    """Restart one stage without removing earned history or XP."""
    for key in list(st.session_state):
        if stage == "vocab":
            reset = (
                (key.startswith("vocab_") and lesson_key_unit(key) == 1
                 and key != "vocab_rewarded_1")
                or key.startswith(("unit1_picture_", "unit1_mystery_", "unit1_read_", "unit1_intro_sequence_"))
            )
        elif stage == "grammar2":
            reset = key.startswith("unit1_grammar2_") or key == "grammar2_done_1"
        else:
            reset = key.startswith("unit1_activity1_") or key == "activity1_completed_1"
        if reset:
            st.session_state.pop(key, None)
    if stage == "vocab":
        st.session_state["vocab_index_1"] = 0
        st.session_state["vocab_read_cards_1"] = []
        st.session_state["vocab_category_1"] = None
        st.session_state["vocab_revealed_1"] = False
        st.session_state["unit1_read_round"] = 0
        st.session_state["unit1_read_line"] = 0
        st.session_state["unit1_intro_sequence_step"] = 0
        st.session_state["unit1_picture_card_index"] = 0
        st.session_state["unit1_picture_dialogue_done"] = False
    elif stage == "grammar2":
        for index in range(4):
            st.session_state[f"unit1_grammar2_choice_{index}"] = None
        st.session_state["unit1_grammar2_friend_name"] = ""
        st.session_state["unit1_grammar2_friend_job"] = None
        st.session_state["unit1_grammar2_reset_nonce"] = st.session_state.get("unit1_grammar2_reset_nonce", 0) + 1
    else:
        st.session_state["unit1_activity1_anna_job"] = None
        st.session_state["unit1_activity1_juno_job"] = None
        st.session_state["unit1_activity1_friend_name"] = None
        st.session_state["unit1_activity1_friend_job"] = None
        st.session_state["unit1_activity1_dialogue_read"] = False
    st.session_state[f"unit1_{stage}_reviewing"] = True
    request_lesson_tab(1, {"vocab": 0, "grammar2": 2, "activity1": 3}[stage])


def complete_unit1_stage(stage):
    completion_key = "activity1_completed_1" if stage == "activity1" else f"{stage}_done_1"
    st.session_state[completion_key] = True
    st.session_state[f"unit1_{stage}_reviewing"] = False


def restart_unit1_activity2():
    """Clear Unit 1 Activity 2 inputs while preserving completion history and XP."""
    for key in list(st.session_state):
        if (
            key.startswith(("unit1_activity2_", "unit1_final_"))
            or key == "activity2_submission_notice_1"
        ):
            st.session_state.pop(key, None)
    # Reassign widget values so Streamlit cannot restore the previous browser value.
    for key in ("unit1_activity2_wei_name", "unit1_activity2_yuna_name"):
        st.session_state[key] = None
    st.session_state["unit1_final_name"] = ""
    for key in ("unit1_activity2_wei_job", "unit1_activity2_yuna_job"):
        st.session_state[key] = "선택하세요"
    st.session_state["unit1_final_country"] = None
    st.session_state["unit1_final_job"] = None
...
def restart_unit2_stage(stage):
    ...
    if stage == "vocab":
        ...
        defaults.update({f"unit2_visual_number_{i}": None for i in range(4)})
    elif stage == "grammar1":
        ...
        defaults = {"unit2_activity1_topic": None,
                    "unit2_activity1_phone": None,
        ...
        defaults.update({f"unit2_contact_{field}_choice_{i}": None for field in ("name", "phone", "email_name") for i in range(3)})
...
def restart_unit3_stage(stage):
    """Restart only the selected Unit 3 stage, preserving other work and rewards."""
    prefixes = {
        "vocab": ("unit3_vocab_", "unit3_meaning_text"),
        "grammar1": ("unit3_grammar1_", "grammar1_index_3", "grammar1_result_3",
                     "grammar1_checked_choice_3_", "grammar1_feedback_visible_3_",
                     "grammar1_passed_3_", "grammar_quiz_choice_3_"),
        "grammar2": ("unit3_grammar2_",),
        "activity1": ("unit3_activity1_",),
        "activity2": ("unit3_activity2_", "activity2_submission_notice_3"),
    }[stage]
    for key in list(st.session_state):
        reset = key.startswith(prefixes) or (
            stage == "vocab" and key.startswith("vocab_")
            and lesson_key_unit(key) == 3 and key != "vocab_rewarded_3"
        )
        if reset:
            st.session_state.pop(key, None)
    if stage == "vocab":
        st.session_state["vocab_index_3"] = 0
        st.session_state["vocab_read_cards_3"] = []
        st.session_state["vocab_revealed_3"] = False
    else:
        question_counts = {
            "grammar1": {"grammar1_picture": 3, "grammar1_dialogue_v2": 3},
            "grammar2": {"grammar2_presence": 5, "grammar2_location": 5},
            "activity1": {"activity1_reading": 2},
            "activity2": {"activity2_reading": 2},
        }[stage]
        suffix = f"_r{st.session_state.get('unit3_reset_nonce', 0)}"
        for question_stage, count in question_counts.items():
            for index in range(count):
                st.session_state[f"unit3_{question_stage}_choice_{index}{suffix}"] = None
        if stage == "grammar1":
            st.session_state["grammar1_index_3"] = 0
            for index in range(4):
                st.session_state[f"grammar_quiz_choice_3_{index}"] = None
        elif stage == "activity1":
            st.session_state["unit3_activity1_dialogue_row"] = "1) 마리 · 책"
        elif stage == "activity2":
            for slot_name in ("first", "second", "third"):
                st.session_state[f"unit3_activity2_builder_{slot_name}_item"] = None
                st.session_state[f"unit3_activity2_builder_{slot_name}_location"] = None
    st.session_state[f"unit3_{stage}_reviewing"] = True
    request_lesson_tab(3, UNIT2_STAGES.index(stage))


def complete_unit3_stage(stage):
    if stage == "activity2":
        complete_activity2(3)
    else:
        key = "activity1_completed_3" if stage == "activity1" else f"{stage}_done_3"
        st.session_state[key] = True
    st.session_state[f"unit3_{stage}_reviewing"] = False
    request_lesson_tab(3, UNIT2_STAGES.index(stage))


def render_unit3_stage_actions(stage, ready, completion_help=None):
    render_stage_actions(3, stage, ready, completion_help)


def restart_unit4_stage(stage):
    """Clear only the selected Unit 4 stage while preserving earned progress."""
    prefixes = {
        "vocab": ("unit4_vocab_",),
        "grammar1": ("unit4_grammar1_", "grammar1_index_4", "grammar1_result_4",
                     "grammar1_checked_choice_4_", "grammar1_feedback_visible_4_",
                     "grammar1_passed_4_", "grammar_quiz_choice_4_"),
        "grammar2": ("unit4_grammar2_",),
        "activity1": ("unit4_activity1_",),
        "activity2": ("unit4_activity2_", "activity2_submission_notice_4"),
    }[stage]
    for key in list(st.session_state):
        reset = key.startswith(prefixes) or (
            stage == "vocab" and key.startswith("vocab_")
            and lesson_key_unit(key) == 4 and key != "vocab_rewarded_4"
        )
        if reset:
            st.session_state.pop(key, None)
    if stage == "vocab":
        st.session_state["vocab_index_4"] = 0
        st.session_state["vocab_read_cards_4"] = []
        st.session_state["vocab_revealed_4"] = False
    else:
        question_counts = {
            "grammar1": {},
            "grammar2": {"grammar2_preference": 4, "grammar2_dialogue": 4},
            "activity1": {"activity1_reading": 2},
            "activity2": {"activity2_message": 2},
        }[stage]
        for question_stage, count in question_counts.items():
            for index in range(count):
                st.session_state[f"unit4_{question_stage}_choice_{index}"] = None
        if stage == "grammar1":
            st.session_state["grammar1_index_4"] = 0
            for index in range(4):
                st.session_state[f"grammar_quiz_choice_4_{index}"] = None
        elif stage == "activity2":
            st.session_state["unit4_activity2_place"] = ""
            st.session_state["unit4_activity2_first_action"] = interface_text("선택하세요", "Choose")
            st.session_state["unit4_activity2_second_action"] = interface_text("선택하세요", "Choose")
    st.session_state[f"unit4_{stage}_reviewing"] = True
    request_lesson_tab(4, UNIT2_STAGES.index(stage))


def complete_unit4_stage(stage):
    if stage == "activity2":
        complete_activity2(4)
    else:
        key = "activity1_completed_4" if stage == "activity1" else f"{stage}_done_4"
        st.session_state[key] = True
    st.session_state[f"unit4_{stage}_reviewing"] = False
    request_lesson_tab(4, UNIT2_STAGES.index(stage))


def render_unit4_stage_actions(stage, ready, completion_help=None):
    render_stage_actions(4, stage, ready, completion_help)


def restart_unit5_stage(stage):
    """Restart one Unit 5 stage without clearing earlier work or rewards."""
    prefixes = {
        "vocab": ("unit5_vocab_",),
        "grammar1": ("unit5_grammar1_", "unit5_g1_", "grammar1_index_5", "grammar1_result_5",
                     "grammar1_checked_choice_5_", "grammar1_feedback_visible_5_",
                     "grammar1_passed_5_", "grammar_quiz_choice_5_"),
        "grammar2": ("unit5_grammar2_", "unit5_g2_"),
        "activity1": ("unit5_activity1_", "unit5_a1_"),
        "activity2": ("unit5_activity2_", "unit5_a2_", "activity2_submission_notice_5"),
    }[stage]
    for key in list(st.session_state):
        reset = key.startswith(prefixes) or (
            stage == "vocab" and key.startswith("vocab_")
            and lesson_key_unit(key) == 5 and key != "vocab_rewarded_5"
        )
        if reset:
            st.session_state.pop(key, None)
    if stage == "vocab":
        st.session_state["vocab_index_5"] = 0
        st.session_state["vocab_read_cards_5"] = []
        st.session_state["vocab_revealed_5"] = False
    else:
        counts = {
            "grammar1": {}, "grammar2": {},
            "activity1": {"activity1_reading": 2},
        ...
        for question_stage, count in counts.items():
            for index in range(count):
                st.session_state[f"unit5_{question_stage}_choice_{index}"] = None
        if stage == "grammar1":
            st.session_state["grammar1_index_5"] = 0
            for index in range(4):
                st.session_state[f"grammar_quiz_choice_5_{index}"] = None
        elif stage == "activity2":
            st.session_state["unit5_a2_first"] = None
            st.session_state["unit5_a2_second"] = None
    st.session_state[f"unit5_{stage}_reviewing"] = True
    request_lesson_tab(5, UNIT2_STAGES.index(stage))


def complete_unit5_stage(stage):
    if stage == "activity2":
        complete_activity2(5)
    else:
        key = "activity1_completed_5" if stage == "activity1" else f"{stage}_done_5"
        st.session_state[key] = True
    st.session_state[f"unit5_{stage}_reviewing"] = False
    request_lesson_tab(5, UNIT2_STAGES.index(stage))


def render_unit5_stage_actions(stage, ready, completion_help=None):
    render_stage_actions(5, stage, ready, completion_help)


def restart_later_unit_stage(unit_number, stage):
    """Clear one Unit 6–10 exercise while retaining earned completion history."""
    short_name = {"vocab": "vocab", "grammar1": "g1", "grammar2": "g2", "activity1": "a1", "activity2": "a2"}[stage]
    prefixes = [f"unit{unit_number}_{stage}_", f"unit{unit_number}_{short_name}_"]
    if stage == "vocab":
        prefixes += [f"vocab_index_{unit_number}", f"vocab_read_cards_{unit_number}",
                     f"vocab_revealed_{unit_number}", f"vocab_category_{unit_number}",
                     f"vocab_done_{unit_number}"]
    elif stage == "grammar1":
        prefixes += [f"grammar1_index_{unit_number}", f"grammar1_result_{unit_number}",
                     f"grammar1_checked_choice_{unit_number}_", f"grammar1_feedback_visible_{unit_number}_",
                     f"grammar1_passed_{unit_number}_", f"grammar_quiz_choice_{unit_number}_",
                     f"grammar1_done_{unit_number}"]
    elif stage == "grammar2":
        prefixes.append(f"grammar2_done_{unit_number}")
    elif stage == "activity1":
        prefixes.append(f"activity1_completed_{unit_number}")
    else:
        prefixes += [f"unit_completed_{unit_number}", f"activity2_submission_notice_{unit_number}"]
    for key in list(st.session_state):
        if any(key == prefix or key.startswith(prefix) for prefix in prefixes) and "rewarded" not in key:
            st.session_state.pop(key, None)
    if stage == "vocab":
        st.session_state[f"vocab_index_{unit_number}"] = 0
        st.session_state[f"vocab_read_cards_{unit_number}"] = []
    elif stage == "grammar1":
        st.session_state[f"grammar1_index_{unit_number}"] = 0
    st.session_state[f"unit{unit_number}_{stage}_reviewing"] = True
    request_lesson_tab(unit_number, UNIT2_STAGES.index(stage))


def render_later_unit_stage_actions(unit_number, stage, completion_help=None):
    """Match the replay and next-step controls used in Units 1–5."""
    ...
    completion_key = (f"vocab_done_{unit_number}", f"grammar1_done_{unit_number}",
                      f"grammar2_done_{unit_number}", f"activity1_completed_{unit_number}",
                      f"unit_completed_{unit_number}")[index]
    reviewing_key = f"unit{unit_number}_{stage}_reviewing"
    if st.session_state.get(reviewing_key, False) and st.session_state.get(completion_key, False):
        st.session_state[reviewing_key] = False
    finished = get_unit_completion_steps(unit_number)[index] and not st.session_state.get(reviewing_key, False)
    st.divider()
    replay, action = render_unit1_action_columns(
        interface_text(f"{title} 학습을 완료했어요 ✓", f"You completed {title} ✓") if finished else None,
        stage=f"unit{unit_number}_{stage}",
    )
    with replay:
        lesson_button(interface_text(f"{title} 다시 학습하기", f"Practice {title} again"),
                      key=f"unit{unit_number}_{stage}_replay", width="stretch",
                      on_click=restart_later_unit_stage, args=(unit_number, stage))
    with action:
        if index < 4:
            next_title = interface_text(*titles[index + 1])
            lesson_button(interface_text(f"다음: {next_title} 학습하기 →", f"Next: Learn {next_title} →"),
                          key=f"unit{unit_number}_{stage}_next", type="primary", width="stretch",
                          disabled=not finished, help=completion_help if not finished else None,
                          on_click=request_lesson_tab, args=(unit_number, index + 1))
        else:
            lesson_button(interface_text("다음: 단원 정리·복습 →", "Next: Unit summary and review →"),
                          key=f"unit{unit_number}_activity2_next", type="primary", width="stretch",
                          disabled=not finished, on_click=open_unit_summary, args=(unit_number,))


def continue_unit3_vocabulary():
    request_lesson_tab(3, 0)
    st.session_state["_intro_vocabulary_heading_unit"] = 3
    st.rerun()


def render_unit2_stage_actions(stage, ready, completion_help=None):
    render_stage_actions(2, stage, ready, completion_help)


def auto_complete_stage(unit_number, stage, ready):
    """Save newly finished work once, preserving the learner's selected tab."""
    index = UNIT2_STAGES.index(stage)
    steps = get_unit_completion_steps(unit_number)
    reviewing = st.session_state.get(f"unit{unit_number}_{stage}_reviewing", False)
    if not ready or (index > 0 and not steps[index - 1]) or (steps[index] and not reviewing):
        return
    selected_label = st.session_state.get(f"_unit{unit_number}_lesson_tabs", "")
    titles = [("어휘와 표현", "Vocabulary & Expressions"), ("문법 1", "Grammar 1"),
              ("문법 2", "Grammar 2"), ("활동 1", "Activity 1"), ("활동 2", "Activity 2")]
    selected_index = next((i for i, title in enumerate(titles)
                           if selected_label.startswith("● " + interface_text(*title))), index)
    if unit_number == 2:
        complete_unit2_stage(stage)
    elif unit_number == 3:
        complete_unit3_stage(stage)
    elif unit_number == 4:
        complete_unit4_stage(stage)
    elif unit_number == 5:
        complete_unit5_stage(stage)
    elif 6 <= unit_number <= 10 and stage == "grammar1":
        st.session_state[f"grammar1_done_{unit_number}"] = True
        st.session_state[f"unit{unit_number}_grammar1_reviewing"] = False
    elif stage == "grammar1":
        complete_unit1_grammar1()
    elif stage == "activity2":
        complete_activity2(unit_number)
        if unit_number >= 6:
            st.session_state[f"unit{unit_number}_activity2_reviewing"] = False
    else:
        complete_unit1_stage(stage)
    request_lesson_tab(unit_number, selected_index)
    st.session_state["_automatic_stage_completion"] = True
    st.rerun()


def render_stage_actions(unit_number, stage, ready, completion_help=None):
    """Shared automatic completion and explicit navigation for Units 1 and 2."""
    auto_complete_stage(unit_number, stage, ready)
    index = UNIT2_STAGES.index(stage)
    titles = [("어휘와 표현", "Vocabulary & Expressions"), ("문법 1", "Grammar 1"),
              ("문법 2", "Grammar 2"), ("활동 1", "Activity 1"), ("활동 2", "Activity 2")]
    title = interface_text(*titles[index])
    finished = get_unit_completion_steps(unit_number)[index] and not st.session_state.get(f"unit{unit_number}_{stage}_reviewing", False)
    first_missing = None
    if ready and not finished and index > 0:
        steps = get_unit_completion_steps(unit_number)
        first_missing = next((i for i in range(index) if not steps[i]), None)
        if first_missing is not None:
            prerequisite = interface_text(*titles[first_missing])
            completion_help = interface_text(
                f"현재 단계의 답은 완료되었습니다. 먼저 {prerequisite} 학습을 완료하면 다음 단계가 열립니다.",
                f"Your answers for this step are complete. Finish {prerequisite} first to unlock the next step.",
            )
    if unit_number == 1:
        replay_key, next_key = {
            "vocab": ("unit1_vocab_replay", "unit1_picture_continue_grammar1"),
            "grammar1": ("unit1_grammar1_restart", "unit1_grammar1_continue_grammar2"),
            "grammar2": ("unit1_grammar2_replay", "unit1_grammar2_continue_activity1"),
            "activity1": ("unit1_activity1_replay", "unit1_activity1_continue_activity2"),
            "activity2": ("unit1_activity2_replay", "unit1_activity2_next"),
        }[stage]
        restart = restart_unit1_grammar1 if stage == "grammar1" else restart_unit1_activity2 if stage == "activity2" else restart_unit1_stage
        restart_args = () if stage in ("grammar1", "activity2") else (stage,)
    elif unit_number == 2:
        replay_key, next_key = f"unit2_{stage}_replay", f"unit2_{stage}_next"
        restart, restart_args = restart_unit2_stage, (stage,)
    elif unit_number == 4:
        replay_key, next_key = f"unit4_{stage}_replay", f"unit4_{stage}_next"
        restart, restart_args = restart_unit4_stage, (stage,)
    elif unit_number == 5:
        replay_key, next_key = f"unit5_{stage}_replay", f"unit5_{stage}_next"
        restart, restart_args = restart_unit5_stage, (stage,)
    else:
        replay_key, next_key = f"unit3_{stage}_replay", f"unit3_{stage}_next"
        restart, restart_args = restart_unit3_stage, (stage,)
    st.divider()
    replay, action = render_unit1_action_columns(
        interface_text(f"{title} 학습을 완료했어요 ✓", f"You completed {title} ✓") if finished else None,
        stage=f"unit{unit_number}_{stage}",
    ...
    with replay:
        lesson_button(interface_text(f"{title} 다시 학습하기", f"Practice {title} again"),
                      key=replay_key, width="stretch",
                      on_click=restart, args=restart_args)
    with action:
        if index < 4:
            next_title = interface_text(*titles[index + 1])
            if first_missing is not None:
                prerequisite = interface_text(*titles[first_missing])
                lesson_button(interface_text(f"먼저 {prerequisite} 학습하기 →", f"Learn {prerequisite} first →"),
                              key=next_key, type="primary", width="stretch",
                              on_click=request_lesson_tab, args=(unit_number, first_missing))
            else:
                lesson_button(interface_text(f"다음: {next_title} 학습하기 →", f"Next: Learn {next_title} →"),
                              key=next_key, type="primary", width="stretch",
                              disabled=not finished, help=completion_help if not finished else None,
...
def unit2_wrap_up_ready():
    return get_unit_completion_steps(2)[4] and all(
        st.session_state.get(key, False) for key in (
            "review_vocab_done_2", "review_grammar_done_2", "review_sentence_done_2",
        )
    )
...
def unit3_wrap_up_ready():
    return get_unit_completion_steps(3)[4] and all(
        st.session_state.get(key, False) for key in (
            "review_vocab_done_3", "review_grammar_done_3", "review_sentence_done_3",
        )
    )


def complete_unit3_wrap_up():
    if unit3_wrap_up_ready():
        st.session_state.unit3_wrap_up_completed = True


def unit4_wrap_up_ready():
    return get_unit_completion_steps(4)[4] and all(
        st.session_state.get(key, False) for key in (
            "review_vocab_done_4", "review_grammar_done_4", "review_sentence_done_4",
        )
    )


def complete_unit4_wrap_up():
    if unit4_wrap_up_ready():
        st.session_state.unit4_wrap_up_completed = True


def unit5_wrap_up_ready():
    return get_unit_completion_steps(5)[4] and all(
        st.session_state.get(key, False) for key in (
            "review_vocab_done_5", "review_grammar_done_5", "review_sentence_done_5",
        )
    )


def complete_unit5_wrap_up():
    if unit5_wrap_up_ready():
        st.session_state.unit5_wrap_up_completed = True
...
def render_unit1_stage_actions(stage, ready, completion_help=None):
    render_stage_actions(1, stage, ready, completion_help)


def queue_grammar1_final_scroll(unit_number):
    st.session_state[f"_scroll_grammar1_final_{unit_number}"] = True


def render_lesson_tab_navigation(tab_index, align_heading=False):
    """Select a lesson tab and align the five-stage learning area in the viewport."""
    navigation_run = st.session_state.get("_lesson_navigation_run", 0) + 1
    st.session_state["_lesson_navigation_run"] = navigation_run
    st.html(
        f"""
        <script>
        (() => {{
          const navigationRun = {navigation_run};
          const tabIndex = {int(tab_index)};
          const alignHeading = {str(align_heading).lower()};
          let attempts = 0;
          let aligned = false;
          const selectTab = () => {{
            ...
            if (target || alignHeading) {{
              if (!alignHeading && target.getAttribute('aria-selected') !== 'true') {{
                target.click();
              }}
              if (!aligned && !alignHeading) {{
                target.focus({{preventScroll: true}});
                aligned = true;
              }}
              const alignLessonArea = () => {{
                const doc = window.parent.document;
                const headings = [...doc.querySelectorAll('[id="unit-learning-heading"]')].filter(el => el.getClientRects().length);
                const headingContainer = headings[headings.length - 1];
                const destination = alignHeading ? (headingContainer?.querySelector('h2') || headingContainer) : group;
                if (!destination) return;
                if (!alignHeading) {{
                  destination.style.scrollMarginTop = '88px';
                  destination.scrollIntoView({{block: 'start', behavior: 'instant'}});
                  return;
                }}
                // Find the actual scrolling ancestor rather than assuming
                // that stMain owns scrolling in every Streamlit layout.
                let scroller = destination.parentElement;
                while (scroller && scroller !== doc.documentElement) {{
                  const overflow = window.parent.getComputedStyle(scroller).overflowY;
                  if (/(auto|scroll|overlay)/.test(overflow) && scroller.scrollHeight > scroller.clientHeight) break;
                  scroller = scroller.parentElement;
                }}
                scroller = scroller && scroller !== doc.documentElement ? scroller : doc.scrollingElement;
                if (!scroller) return;
                const viewportTop = scroller === doc.scrollingElement ? 0 : scroller.getBoundingClientRect().top;
                const header = doc.querySelector('[data-testid="stHeader"]');
                const headerBottom = header && header.getClientRects().length ? header.getBoundingClientRect().bottom : 0;
                const visibleTop = Math.max(viewportTop, headerBottom);
                const delta = destination.getBoundingClientRect().top - visibleTop;
                if (Math.abs(delta) > 1) scroller.scrollTo({{top: scroller.scrollTop + delta, behavior: 'instant'}});
              }};
              alignLessonArea();
            }}
            if (attempts++ < 50) window.setTimeout(selectTab, 100);
          }};
          ...
          const stopAlignment = () => {{ attempts = 51; }};
...
def enforce_unit_selection(available_units, fallback_label):
    """Keep the full curriculum visible without opening a locked unit."""
    selected_label = st.session_state.get("selected_unit_label", fallback_label)
    match = re.match(r"(\d+)단원", selected_label)
    selected_unit = int(match.group(1)) if match else 1
    if selected_unit in available_units:
        st.session_state.selected_unit_number = selected_unit
        st.session_state.pop("unit_access_notice", None)
    else:
        st.session_state.selected_unit_label = fallback_label
        fallback_match = re.match(r"(\d+)단원", fallback_label)
        st.session_state.selected_unit_number = int(fallback_match.group(1)) if fallback_match else 1
        st.session_state.unit_access_notice = selected_unit
...
def complete_activity2(unit_number):
    ...
    if not st.session_state.get(completion_key, False) and not get_unit_completion_steps(unit_number)[4]:
        st.session_state[completion_key] = True
        st.session_state.total_xp = st.session_state.get("total_xp", 0) + 20
        save_progress()
    elif unit_number >= 6 and st.session_state.get(f"unit{unit_number}_activity2_reviewing", False):
        st.session_state[completion_key] = True
...
# 실제 학습에서는 순차 잠금이 기본입니다. 전체 단원을 검수할 때만
# KOREAN_APP_REVIEW_MODE=1로 실행해 점검 모드를 명시적으로 켭니다.
...
def get_unit_completion_steps(unit_number):
    """Keep earned completion while validating new work in sequence."""
    return completion_steps(
        st.session_state, st.session_state.get("_lesson_history", {}), unit_number
    )
...
GRAMMAR1_SEQUENCE_BANK = {
    1: [
        ...
        ("제 친구는 학생__." , ["예요", "이에요", "하고", "에서"], "이에요", "학생 친구를 소개하는 문장이에요. ‘학생’은 받침이 있으므로 ‘이에요’를 사용해요."),
    ],
    2: [
    ...
    5: [
        ...
        ("주노 씨는 식당__ 가요.", ["에서", "에", "를", "하고"], "에", "가는 장소인 ‘식당’ 뒤에 ‘에’를 붙여요."),
        ("안나 씨는 학교__ 가요.", ["가", "를", "에", "하고"], "에", "목적지인 ‘학교’ 뒤에는 ‘에’를 사용해요."),
        ("유진 씨는 회사__ 가요.", ["에서", "를", "는", "에"], "에", "‘회사에 가요’처럼 장소와 ‘가다’를 연결해요."),
    ],
    6: [
...
TEXTBOOK_VOCABULARY = {
    ...
    7: [("월요일", "Monday"), ("화요일", "Tuesday"), ("수요일", "Wednesday"), ("목요일", "Thursday"), ("금요일", "Friday"), ("토요일", "Saturday"), ("일요일", "Sunday"), ("오늘", "today"), ("내일", "tomorrow"), ("오전", "a.m."), ("오후", "p.m."), ("시", "o'clock / hour"), ("분", "minute"), ("반", "half past"), ("월", "month"), ("일", "day / date"), ("시작하다", "to start"), ("만나다", "to meet"), ("수업", "class")],
...
VOCABULARY_EXAMPLES = {
    ...
    7: {
        ...
        "오전": "오전 아홉 시에 수업이 있어요.", "오후": "오후 세 시에 친구를 만나요.",
        "시": "수업은 일곱 시에 시작해요.", "분": "지금 일곱 시 삼십 분이에요.", "반": "지금 일곱 시 반이에요.",
        "월": "제 생일은 시월이에요.", "일": "제 생일은 오월 십육일이에요.", "시작하다": "수업이 아홉 시에 시작해요.",
...
# Fill every remaining English interface phrase with the learner's selected
# language. Hand-reviewed translations above keep priority over this catalog.
_translation_catalog_path = Path(__file__).with_name("assets") / "interface_translations.json"
try:
    _generated_interface_translations = json.loads(
        _translation_catalog_path.read_text(encoding="utf-8")
    )
except (OSError, json.JSONDecodeError):
    _generated_interface_translations = {}
for _language, _translations in _generated_interface_translations.items():
    _language_table = INTERFACE_TRANSLATIONS.setdefault(_language, {})
    for _english_key, _translated_value in _translations.items():
        _language_table.setdefault(_english_key, _translated_value)
...
def pronunciation_mission():
    ...
    with left:
        ...
        run_demo = lesson_button("샘플 발음 분석 실행", type="primary", width="stretch")
        if audio or run_demo:
            ...
            if st.session_state.get("unlocked_stage", 1) < 2 and lesson_button("미션 완료하고 다음 단계 열기 →", type="primary", key="unlock_pronunciation"):
...
def sentence_builder():
    ...
    with st.container(border=True):
        ...
        for position, word in enumerate(words):
            with cols[position]:
                if word not in st.session_state.builder_answer and lesson_button(word, key=f"builder_word_{st.session_state.builder_index}_{position}", width="stretch", disabled=st.session_state.builder_finished):
                    st.session_state.builder_answer.append(word)
                    st.rerun()
        a, b, c = st.columns(3)
        with a:
            if lesson_button("선택 지우고 다시 하기", key="builder_reset", width="stretch"):
                st.session_state.builder_answer = []
                st.rerun()
        with b:
            if lesson_button(
        ...
        with c:
            if st.session_state.builder_completed and lesson_button("다음 문장으로  →", key="builder_next", type="primary", width="stretch"):
...
def sync_mode():
    ...
    with c1:
        ...
        if lesson_button("정답 제출  →", type="primary", key="sync_submit", width="stretch"):
...
def inject_css():
    st.markdown(
        ...
        .brand { display:flex; align-items:flex-end; gap:9px; margin:-22px 0 3rem; }
        .brand-mark { align-self:center; color:var(--lime); font-size:16px; filter:drop-shadow(0 0 7px rgba(201,246,109,.55)); }
        ...
        .brand .instructor-name::after {
            ...
            height:2px;
            background:var(--lime);
            clip-path:polygon(0 45%, 100% 0, 96% 100%, 2% 82%);
            box-shadow:0 0 7px rgba(201,246,109,.42);
        }
        .brand .momo-logo {
            display:block;
            width:69px;
            height:30px;
            overflow:visible;
            filter:drop-shadow(0 0 5px rgba(201,246,109,.22));
        }
        .brand .momo-logo path,
        .brand .momo-logo rect {
            fill:none;
            stroke:var(--lime);
            stroke-width:2.35;
        ...
        .brand .momo-logo .momo-accent { stroke-width:3.2; }
        .brand .momo-signature {
            position:absolute;
            z-index:2;
            top:-7px;
            right:-14px;
            ...
            box-shadow:0 0 7px rgba(201,246,109,.22);
        }
        .brand .course-name {
            align-self:flex-end;
            color:var(--ink);
        ...
        .lime { color:var(--lime); } .coral { color:var(--coral); } .blank-placeholder { color:var(--muted); }
        ...
        .reading-line { display:flex; align-items:baseline; gap:.6em; padding:12px 14px; margin:7px 0; border-left:3px solid #333; border-radius:8px; color:#767676; background:#1c1c1c; transition:color .2s ease, background-color .2s ease, border-color .2s ease; }
        .reading-line-marker { flex:0 0 1.2em; width:1.2em; text-align:center; line-height:1.5; }
        .reading-line-text { flex:1; min-width:0; line-height:1.5; }
        ...
        div:is(
            [class*="st-key-grammar_next_1_"],
            [class*="st-key-grammar_next_2_"],
            [class*="st-key-grammar_next_3_"],
            [class*="st-key-grammar_next_4_"],
            [class*="st-key-grammar_next_5_"]
        ) button:disabled {
            border:1px solid #737373 !important;
            opacity:1 !important;
        }
        .st-key-unit1_intro_reading_controls button,
        .st-key-unit2_intro_reading_controls button {
            box-sizing:border-box; width:100%; height:44px; min-height:44px;
            display:flex; align-items:center; justify-content:center;
            padding:2px 8px; border-radius:10px; font-size:16px; line-height:18px;
            text-align:center;
        }
        .st-key-unit1_intro_reading_controls button p,
        .st-key-unit2_intro_reading_controls button p { font-size:16px; line-height:18px; margin:0; }
        .intro-reading-status { box-sizing:border-box; height:72px; display:flex; align-items:center; font-size:14px; line-height:20px; }
        ...
        div:is(.st-key-unit1_wrap_up_guidance, .st-key-unit2_wrap_up_guidance, .st-key-unit3_wrap_up_guidance, .st-key-unit4_wrap_up_guidance, .st-key-unit5_wrap_up_guidance) {
        ...
        div:is(.st-key-unit1_wrap_up_guidance, .st-key-unit2_wrap_up_guidance, .st-key-unit3_wrap_up_guidance, .st-key-unit4_wrap_up_guidance, .st-key-unit5_wrap_up_guidance) [data-testid="stMarkdownContainer"] {
            width:100%; display:flex; align-items:center;
        }
        div:is(.st-key-unit1_wrap_up_guidance, .st-key-unit2_wrap_up_guidance, .st-key-unit3_wrap_up_guidance, .st-key-unit4_wrap_up_guidance, .st-key-unit5_wrap_up_guidance) p {
        ...
        .disabled-button-guide {
            ...
            font-size:.82rem;
            line-height:1.45;
            transition:max-height .16s ease, opacity .16s ease, margin .16s ease, padding .16s ease;
        }
        div[data-testid="stElementContainer"]:has(button:disabled:hover)
        + div[data-testid="stElementContainer"] .disabled-button-guide,
        div[data-testid="stElementContainer"]:has(button:disabled:focus-visible)
        + div[data-testid="stElementContainer"] .disabled-button-guide,
        div[data-testid="stElementContainer"]:has(button:disabled:active)
        + div[data-testid="stElementContainer"] .disabled-button-guide {
            max-height:140px;
            margin:6px 0 8px;
            padding:8px 10px;
        ...
        .stApp [data-testid="stSelectbox"] [role="group"]:has(> input[role="combobox"]:is([value="선택하세요"], [value="Choose"], [value="Select one"], [value=""][placeholder="선택하세요"], [value=""][placeholder="Choose"], [value=""][placeholder="Select one"])) {
            display:flex !important;
            flex-direction:row-reverse !important;
        }
        .stApp [data-testid="stSelectbox"] [role="group"]:has(> input[role="combobox"]:is([value="선택하세요"], [value="Choose"], [value="Select one"], [value=""][placeholder="선택하세요"], [value=""][placeholder="Choose"], [value=""][placeholder="Select one"])) > input {
    ...
    st.markdown(
        ...
        <style>
        .disabled-button-guide {
            max-height:0; margin:0; padding:0 10px; overflow:hidden; opacity:0;
            border:1px solid transparent; border-radius:9px;
            background:#202020; color:#f3f0ea; font-size:.82rem; line-height:1.45;
            transition:max-height .16s ease, opacity .16s ease, margin .16s ease, padding .16s ease;
        }
        div[data-testid="stElementContainer"]:has(button:disabled:hover)
        + div[data-testid="stElementContainer"] .disabled-button-guide,
        ...
        div[data-testid="stElementContainer"]:has(button:disabled:active)
        + div[data-testid="stElementContainer"] .disabled-button-guide {
            max-height:140px; margin:6px 0 8px; padding:8px 10px; opacity:1;
            border-color:#737373;
        }
        </style>
        """,
    ...
    _discard_theme_css(
        ...
        :root {{
            ...
            --line:{theme['line']}; --lime:{theme['accent']}; --primary-color:{theme['accent']};
        ...
        .stRadio [role="radiogroup"] label:not(:has(input:checked)) {{
            background:transparent !important;
            border-color:var(--line) !important;
            color:{theme['ink']} !important;
            box-shadow:none !important;
        }}
        .stRadio [role="radiogroup"] label:not(:has(input:checked)) input[type="radio"] {{ accent-color:#777 !important; }}
        .stRadio [role="radiogroup"] label:has(input:checked) input[type="radio"] {{ accent-color:var(--lime) !important; }}
        /* Keep phone-answer radios neutral until the learner selects one. */
        :is([class*="st-key-unit2_activity1_heard_phone"], [class*="st-key-unit2_activity1_confirmation"])
        ...
        div[class*="st-key-unit2_activity1_heard_phone"] [data-baseweb="radio"]:not(:has(input:checked)) > div:first-child,
        div[class*="st-key-unit2_activity1_confirmation"] [data-baseweb="radio"]:not(:has(input:checked)) > div:first-child {{
            border-color:#888 !important;
            background:transparent !important;
            box-shadow:none !important;
        }}
        /* Base Web draws radio circles with divs, not the native input. */
        :is([class*="st-key-unit1_grammar1_picture_"], [class*="st-key-unit1_grammar2_choice_"])
        [data-baseweb="radio"]:not(:has(input:checked)) > div:first-child {{
            background:transparent !important;
            border:2px solid #888 !important;
            box-shadow:none !important;
        ...
        .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) {{
            background:color-mix(in srgb, var(--lime) 18%, var(--panel)) !important;
            border-color:var(--lime) !important;
            color:var(--lime) !important;
            box-shadow:0 0 0 1px color-mix(in srgb, var(--lime) 20%, transparent) !important;
        }}
        .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) p,
        .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) span {{
            color:var(--lime) !important;
    ...
    st.markdown(
        f"""
        <style>
        :root {{ --ink:{theme['ink']}; --muted:{theme['muted']}; --panel:{theme['panel']}; --line:{theme['line']}; --lime:{theme['accent']}; --button-background:{theme['button']}; }}
        .stApp {{ background:{theme['app']}; color:{theme['ink']}; }}
        [data-testid="stSidebar"] {{ background:{theme['sidebar']}; border-right-color:{theme['line']}; }}
        div[data-testid="stButton"] button {{ background:{theme['button']}; color:{theme['ink']}; border-color:{theme['line']}; }}
        .stRadio label, .stCheckbox label {{ color:{theme['ink']} !important; }}
    ...
    st.markdown(
        f"""
        <style>
        /* A closed unit picker is neutral even while it retains input focus. */
        [data-testid="stSidebar"] div.st-key-selected_unit_label
        div[data-baseweb="select"]:not(:has([aria-expanded="true"])),
        [data-testid="stSidebar"] div.st-key-selected_unit_label
        div[data-baseweb="select"]:not(:has([aria-expanded="true"])) > div {{
            border-color:{theme['select_border']} !important;
            outline-color:{theme['select_border']} !important;
            box-shadow:none !important;
    ...
    if st.session_state.get("app_theme") == "밝은색":
        st.markdown(
            """
            <style>
            /* Keep bordered lesson containers visible against the light canvas. */
            .stApp div[data-testid="stVerticalBlockBorderWrapper"] {
                border:1.5px solid #b8aa96 !important;
                box-shadow:0 1px 3px rgba(63, 50, 34, .08) !important;
            }
            .stApp [data-testid="stVerticalBlockBorderWrapper"] h1,
            .stApp [data-testid="stVerticalBlockBorderWrapper"] h2,
            .stApp [data-testid="stVerticalBlockBorderWrapper"] h3,
            .stApp [data-testid="stVerticalBlockBorderWrapper"] p { color:#26221d; }
            .stApp [data-testid="stCaptionContainer"],
            .stApp .tiny, .stApp .sub, .stApp .metric-note { color:#625b52 !important; }
            .stApp .unit-learning-intro { color:#514a41 !important; }
            .stApp .unit-step-summary { color:#3f6200 !important; }

            .stApp div[data-testid="stButton"] button:not([kind="primary"]) {
                border-color:#a99c89 !important;
            }
            .stApp input, .stApp textarea,
            .stApp [data-baseweb="select"] > div {
                border-color:#a99c89 !important;
            ...
            .stApp [data-baseweb="tab"] { color:#3c3731 !important; }

            /* Answer choices need their own light surface; the app's base theme is dark. */
            .stApp [data-testid="stRadio"] [role="radiogroup"] label {
                color:#302b25 !important;
                background:#fffdf9 !important;
                border:1px solid #b9ad9c !important;
                border-radius:9px !important;
                padding:.5rem .75rem !important;
            }
            .stApp [data-testid="stRadio"] [role="radiogroup"] label p,
            .stApp [data-testid="stRadio"] [role="radiogroup"] label span,
            .stApp [data-testid="stRadio"] [role="radiogroup"] label div {
                color:#302b25 !important;
                -webkit-text-fill-color:#302b25 !important;
            }
            .stApp [data-testid="stRadio"] [role="radiogroup"] label:hover {
                background:#f1ecdf !important; border-color:#817462 !important;
            }
            .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) {
                background:#e5edd7 !important; border-color:#5f821b !important;
                box-shadow:0 0 0 1px rgba(95,130,27,.18) !important;
            }
            .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) p,
            .stApp [data-testid="stRadio"] [role="radiogroup"] label:has(input:checked) span {
                color:#345000 !important; -webkit-text-fill-color:#345000 !important; font-weight:700 !important;
            }
            .stApp [data-testid="stCheckbox"] label,
            .stApp [data-testid="stCheckbox"] label p { color:#302b25 !important; }

            /* Feedback panels use calm, readable light-theme surfaces. */
            .stApp [data-testid="stAlert"] {
                color:#26333d !important;
                background:#eef3f5 !important;
                border:1px solid #a8bac4 !important;
                border-radius:10px !important;
            }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentSuccess"]) {
                background:#edf4e5 !important; border-color:#9bb77c !important;
            }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentWarning"]) {
                background:#fff3df !important; border-color:#d7ae6d !important;
            }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentError"]) {
                background:#fae9e5 !important; border-color:#d99a8d !important;
            }
            .stApp [data-testid="stAlert"] p,
            .stApp [data-testid^="stAlertContent"] p,
            .stApp [data-testid="stAlert"] li,
            .stApp [data-testid="stAlert"] strong {
                color:#26333d !important; -webkit-text-fill-color:#26333d !important;
            }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentSuccess"]) p,
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentSuccess"]) strong { color:#2d4b16 !important; }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentWarning"]) p,
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentWarning"]) strong { color:#674713 !important; }
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentError"]) p,
            .stApp [data-testid="stAlert"]:has([data-testid="stAlertContentError"]) strong { color:#702f26 !important; }

            .stApp [data-testid="stSelectbox"] [role="combobox"],
            .stApp [data-testid="stTextInput"] input,
            .stApp [data-testid="stTextArea"] textarea {
                background:#fffdf9 !important; color:#302b25 !important;
                -webkit-text-fill-color:#302b25 !important;
                border-color:#aa9d8b !important;
            }

            /* Green vocabulary family: deeper leaf tones on a pale leaf background. */
            .stApp div[class*="st-key-unit2_sino_number_"] button,
            .stApp div.st-key-vocab_category_country_1 button,
            .stApp div:is(.st-key-vocab_select_1_0,.st-key-vocab_select_1_1,.st-key-vocab_select_1_2,.st-key-vocab_select_1_3,.st-key-vocab_select_1_4,.st-key-vocab_select_1_5,.st-key-vocab_select_1_6,.st-key-vocab_select_1_7,.st-key-vocab_select_1_8,.st-key-vocab_select_1_9,.st-key-vocab_select_1_10) button,
            .stApp div:is(.st-key-vocab_select_3_0,.st-key-vocab_select_3_1,.st-key-vocab_select_3_2,.st-key-vocab_select_3_3,.st-key-vocab_select_3_4,.st-key-vocab_select_3_5) button {
                color:#456b00 !important;
                border-color:#6f9d18 !important;
                background:#f0f6e5 !important;
            }
            /* Coral vocabulary family: warm brick tones instead of low-contrast pink. */
            .stApp div[class*="st-key-unit2_number_example_"] button,
            .stApp div.st-key-vocab_category_job_1 button,
            .stApp div:is(.st-key-vocab_select_1_11,.st-key-vocab_select_1_12,.st-key-vocab_select_1_13,.st-key-vocab_select_1_14,.st-key-vocab_select_1_15,.st-key-vocab_select_1_16,.st-key-vocab_select_1_17,.st-key-vocab_select_1_18,.st-key-vocab_select_1_19,.st-key-vocab_select_1_20) button,
            .stApp div:is(.st-key-vocab_select_3_6,.st-key-vocab_select_3_7,.st-key-vocab_select_3_8,.st-key-vocab_select_3_9,.st-key-vocab_select_3_10,.st-key-vocab_select_3_11,.st-key-vocab_select_3_12,.st-key-vocab_select_3_13,.st-key-vocab_select_3_14,.st-key-vocab_select_3_15) button {
                color:#a6402b !important;
                border-color:#d26148 !important;
                background:#fff0eb !important;
            }
            .stApp div[class*="st-key-unit2_sino_number_"] button[data-testid="stBaseButton-primary"],
            .stApp div.st-key-vocab_category_country_1 button[data-testid="stBaseButton-primary"],
            .stApp div:is(.st-key-vocab_select_1_0,.st-key-vocab_select_1_1,.st-key-vocab_select_1_2,.st-key-vocab_select_1_3,.st-key-vocab_select_1_4,.st-key-vocab_select_1_5,.st-key-vocab_select_1_6,.st-key-vocab_select_1_7,.st-key-vocab_select_1_8,.st-key-vocab_select_1_9,.st-key-vocab_select_1_10) button[data-testid="stBaseButton-primary"],
            .stApp div:is(.st-key-vocab_select_3_0,.st-key-vocab_select_3_1,.st-key-vocab_select_3_2,.st-key-vocab_select_3_3,.st-key-vocab_select_3_4,.st-key-vocab_select_3_5) button[data-testid="stBaseButton-primary"] {
                color:#ffffff !important; background:#5b850b !important; border-color:#4d7207 !important;
            }
            .stApp div[class*="st-key-unit2_number_example_"] button[data-testid="stBaseButton-primary"],
            .stApp div.st-key-vocab_category_job_1 button[data-testid="stBaseButton-primary"],
            .stApp div:is(.st-key-vocab_select_1_11,.st-key-vocab_select_1_12,.st-key-vocab_select_1_13,.st-key-vocab_select_1_14,.st-key-vocab_select_1_15,.st-key-vocab_select_1_16,.st-key-vocab_select_1_17,.st-key-vocab_select_1_18,.st-key-vocab_select_1_19,.st-key-vocab_select_1_20) button[data-testid="stBaseButton-primary"],
            .stApp div:is(.st-key-vocab_select_3_6,.st-key-vocab_select_3_7,.st-key-vocab_select_3_8,.st-key-vocab_select_3_9,.st-key-vocab_select_3_10,.st-key-vocab_select_3_11,.st-key-vocab_select_3_12,.st-key-vocab_select_3_13,.st-key-vocab_select_3_14,.st-key-vocab_select_3_15) button[data-testid="stBaseButton-primary"] {
                color:#ffffff !important; background:#c55238 !important; border-color:#ad422c !important;
            }
            .stApp div[class*="st-key-vocab_"] button[data-testid="stBaseButton-primary"] p,
            .stApp div[class*="st-key-unit2_"] button[data-testid="stBaseButton-primary"] p { color:#ffffff !important; }
            .stApp b[style*="color:#b7ef58"],
            .stApp span[style*="color:#b7ef58"] { color:#456b00 !important; }
            .stApp b[style*="color:#ff9b72"],
            .stApp span[style*="color:#ff9b72"] { color:#a6402b !important; }
            .stApp b[style*="color:#78aef8"],
            .stApp span[style*="color:#78aef8"] { color:#27649d !important; }
            </style>
            """,
            unsafe_allow_html=True,
        )
...
def sync_mode():
    ...
    with c1:
        ...
        lesson_button("정답 제출  →", type="primary", width="stretch")
    with c2:
        st.markdown('<div class="card"><div class="eyebrow">Instructor control</div><h3>현재 진행 상황</h3>', unsafe_allow_html=True)
        st.progress(0.68)
...
@st.fragment
def render_unit1_introduction():
    """Refresh the introduction independently and save each reading step."""
    try:
        render_unit1_introduction_content()
    finally:
        checkpoint_lesson_progress()


def render_unit1_introduction_content():
    ...
    with dialogue:
        with st.container(border=True):
            ...
            completed_reads = min(current_round, 3) * len(reading_lines) + (current_line if current_round < 3 else 0)
            if current_round < 3:
                ...
                st.progress(1.0, text=interface_text("읽기 완료 (3/3회)", "Reading complete (3/3)"))
            for index, line in enumerate(reading_lines):
                if current_round >= 3 or index < current_line:
                    ...
                    style = ""
                    marker = "○"
                st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{line}</span></div>", unsafe_allow_html=True)
            render_intro_reading_controls(1, current_round, len(reading_lines))
    with profile:
        st.markdown('<div id="unit1-intro-sequence"></div>', unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown(interface_text("**단원 도입 · 자기소개 순서**", "**Unit introduction · Self-introduction order**"))
            st.caption(interface_text(
                "아래 카드를 순서대로 눌러 자기소개가 어떻게 이어지는지 확인하세요.",
                "Select the cards in order to see how a self-introduction develops.",
            ))
            intro_sequence = [
                ("인사", "Greeting", "안녕하세요?"),
                ("이름", "Name", "저는 안나예요."),
                ("나라", "Country", "저는 한국 사람이에요."),
                ("직업", "Occupation", "저는 학생이에요."),
            ]
            intro_sequence_step = min(
                int(st.session_state.get("unit1_intro_sequence_step", 0)),
                len(intro_sequence),
            )
            for index, (korean_label, english_label, example) in enumerate(intro_sequence):
                if index < intro_sequence_step:
                    with st.container(border=True):
                        st.markdown(interface_text(
                            f"**✓ {index + 1}. {korean_label}** · {example}",
                            f"**✓ {index + 1}. {english_label}** · {example}",
                        ))
                elif index == intro_sequence_step:
                    lesson_button(
                        interface_text(
                            f"{index + 1}. {korean_label} 확인하기",
                            f"{index + 1}. Check {english_label.lower()}",
                        ),
                        key=f"unit1_intro_sequence_{index}",
                        type="secondary",
                        width="stretch",
                        disabled=not st.session_state.get("unit1_intro_sequence_requested", False),
                        show_disabled_help=False,
                        on_click=set_session_state_value,
                        args=("unit1_intro_sequence_step", index + 1),
                    )
                else:
                    lesson_button(
                        interface_text(
                        ...
                        key=f"unit1_intro_sequence_locked_{index}",
                        disabled=True,
                        show_disabled_help=False,
                        width="stretch",
                    )
            if intro_sequence_step == len(intro_sequence):
                render_intro_sequence_success(1)
            reset_column, next_column = st.columns([1, 1.35])
            with reset_column:
                lesson_button(
                    interface_text("순서 다시 보기", "Review the order again"),
                    key="unit1_intro_sequence_reset",
                    width="stretch",
                    on_click=set_session_state_value,
                    args=("unit1_intro_sequence_step", 0),
                )
            with next_column:
                next_label = interface_text(
                    "다음: 어휘와 표현 학습하기 →",
                ...
                if lesson_button(
                    ...
                    type="primary",
                    width="stretch",
                    disabled=intro_sequence_step < len(intro_sequence),
                ):
                    request_lesson_tab(1, 0)
                    st.session_state["_intro_vocabulary_heading_unit"] = 1
                    st.rerun()
            st.caption(interface_text(
                "여기에서는 자기소개 순서와 예문을 확인합니다. 직접 자기소개를 만드는 활동은 ‘활동 2’에서 진행합니다.",
                "Here you review the self-introduction order and examples. You will create your own self-introduction in Activity 2.",
            ))
    st.space("medium")


@st.fragment
def render_unit2_introduction():
    """Keep reading interactions from rebuilding the surrounding lesson page."""
    try:
        render_unit2_introduction_content()
    finally:
        checkpoint_lesson_progress()


def render_unit2_introduction_content():
    selected_tab = st.session_state.get("_unit2_lesson_tabs", "")
    vocabulary_visible = (
        selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
        if selected_tab else not get_unit_completion_steps(2)[0]
    )
    pending_tab = st.session_state.get("requested_lesson_tab")
    if pending_tab and pending_tab[0] == 2:
        vocabulary_visible = pending_tab[1] == 0
    if st.session_state.get("force_unit_home_step") == 2:
        vocabulary_visible = True
    st.session_state.setdefault("unit2_intro_collapsed", not vocabulary_visible)
    if vocabulary_visible:
        st.session_state.unit2_intro_collapsed = False
    round_key, line_key = "unit2_read_round", "unit2_read_line"
    current_round = st.session_state.get(round_key, 0)
    current_line = st.session_state.get(line_key, 0)
    sequence_key = "unit2_intro_sequence_step"
    sequence_step = st.session_state.get(sequence_key, 0)
    if not vocabulary_visible:
        with st.container(border=True):
            summary, replay, close = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary:
                st.markdown(interface_text("**단원 도입 완료 ✓**", "**Unit introduction complete ✓**")
                            if current_round >= 3 and sequence_step >= 4
                            else interface_text("**단원 도입**", "**Unit introduction**"))
                st.caption(interface_text("번호 묻기 → 번호 말하기 → 확인하기 → 바로잡기", "Ask → Answer → Confirm → Correct"))
            with replay:
                lesson_button(interface_text("단원 도입 다시보기", "Review unit introduction"),
                              key="unit2_intro_expand", on_click=set_session_state_value,
                              args=("unit2_intro_collapsed", False), width="stretch")
            with close:
                if not st.session_state.unit2_intro_collapsed:
                    lesson_button(interface_text("닫기", "Close"), key="unit2_intro_close",
                                  on_click=set_session_state_value, args=("unit2_intro_collapsed", True))
        st.space("medium")
    if st.session_state.unit2_intro_collapsed:
        return
    render_learning_info(interface_text("단원 도입: 먼저 대화 모델을 3회 읽어 보세요.", "Unit introduction: First, read the model dialogue three times."), icon=":material/play_circle:")
    st.caption(interface_text("친구와 전화번호를 주고받는 상황이에요. 대화와 핵심 문장을 살펴본 뒤, 1단계 어휘와 표현으로 이어집니다.", "You are exchanging phone numbers with a friend. Review the dialogue and key sentences, then continue to Step 1: Vocabulary and Expressions."))
    st.markdown(interface_text("### 단원 도입 · 대화 모델 3회 읽기", "### Unit introduction · Read the model dialogue three times"))
    dialogue, sequence = st.columns(2)
    reading_lines = [
        "A: 전화번호가 뭐예요?",
        "B: 010-2345-6789예요.",
        "A: 010-2345-6780이에요?",
        "B: 아니요, 010-2345-6780이 아니에요. 010-2345-6789예요.",
    ]
    with dialogue, st.container(border=True):
        st.markdown(interface_text("**대화 모델 · 3회 반복 읽기**", "**Model dialogue · Read three times**"))
        st.caption(interface_text("연습용 번호예요. 0은 ‘공’으로 읽어요. 010-2345-6789: 공일공 · 이삼사오 · 육칠팔구", "These are practice numbers. Read 0 as ‘공’: 공일공 · 이삼사오 · 육칠팔구."))
        completed_reads = min(current_round, 3) * len(reading_lines) + (current_line if current_round < 3 else 0)
        st.progress(completed_reads / 12, text=interface_text(f"읽기 진행 ({completed_reads}/12문장)", f"Reading progress ({completed_reads}/12 lines)"))
        for index, line in enumerate(reading_lines):
            done = current_round >= 3 or index < current_line
            style = "done" if done else "active" if index == current_line else ""
            marker = "✓" if done else "●" if index == current_line else "○"
            st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{line}</span></div>", unsafe_allow_html=True)
        render_intro_reading_controls(2, current_round, len(reading_lines))
    with sequence, st.container(border=True):
        st.markdown('<div id="unit2-intro-sequence"></div>', unsafe_allow_html=True)
        st.markdown(interface_text("**단원 도입 · 전화번호 대화 순서**", "**Unit introduction · Phone number dialogue order**"))
        st.caption(interface_text("대화를 3회 읽은 뒤 아래 카드를 순서대로 눌러 확인하세요.", "Read the dialogue three times, then select the cards in order."))
        steps = [("번호 묻기", "Ask for a number"), ("번호 말하기", "Say the number"), ("확인하기", "Confirm"), ("바로잡기", "Correct")]
        for index, (ko, en) in enumerate(steps):
            if index < sequence_step:
                with st.container(border=True):
                    st.markdown(f"**✓ {index + 1}. {interface_text(ko, en)}** · {reading_lines[index][3:]}")
            else:
                lesson_button(interface_text(f"{index + 1}. {ko} 확인하기", f"{index + 1}. {en}"), key=f"unit2_intro_sequence_{index}",
                              disabled=current_round < 3 or index != sequence_step or not st.session_state.get("unit2_intro_sequence_requested", False), width="stretch",
                              on_click=set_session_state_value, args=(sequence_key, index + 1))
        if sequence_step >= len(steps):
            render_intro_sequence_success(2)
        reset_column, next_column = st.columns([1, 1.35])
        with reset_column:
            lesson_button(interface_text("순서 다시 보기", "Review the order again"), key="unit2_intro_sequence_reset",
                          width="stretch", on_click=set_session_state_value, args=(sequence_key, 0))
        with next_column:
            if lesson_button(interface_text("다음: 어휘와 표현 학습하기 →", "Next: Learn vocabulary and expressions →"),
                             key="unit2_intro_continue_vocabulary", type="primary",
                             disabled=sequence_step < len(steps), width="stretch"):
                request_lesson_tab(2, 0)
                st.session_state["_intro_vocabulary_heading_unit"] = 2
                st.rerun()
        st.caption(interface_text("숫자와 ‘이/가’, ‘이/가 아니에요’를 배운 뒤 활동에서 직접 대화해 보세요.", "Learn numbers, ‘이/가’ and ‘이/가 아니에요’, then practice your own dialogue in the activities."))
    st.space("medium")


@st.fragment
def render_unit3_introduction():
    """Keep intro button interactions and saved progress aligned with Unit 2."""
    try:
        render_unit3_introduction_content()
    finally:
        checkpoint_lesson_progress()


def render_unit3_introduction_content():
    selected_tab = st.session_state.get("_unit3_lesson_tabs", "")
    vocabulary_visible = (
        selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
        if selected_tab else not get_unit_completion_steps(3)[0]
    )
    pending_tab = st.session_state.get("requested_lesson_tab")
    if pending_tab and pending_tab[0] == 3:
        vocabulary_visible = pending_tab[1] == 0
    if st.session_state.get("force_unit_home_step") == 3:
        vocabulary_visible = True
    st.session_state.setdefault("unit3_intro_collapsed", not vocabulary_visible)
    if vocabulary_visible:
        st.session_state.unit3_intro_collapsed = False
    round_key, line_key, sequence_key = "unit3_read_round", "unit3_read_line", "unit3_intro_sequence_step"
    current_round = st.session_state.get(round_key, 0)
    current_line = st.session_state.get(line_key, 0)
    sequence_step = min(int(st.session_state.get(sequence_key, 0)), 4)
    if not vocabulary_visible:
        with st.container(border=True):
            summary, replay, close = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary:
                st.markdown(interface_text("**단원 도입 완료 ✓**", "**Unit introduction complete ✓**") if current_round >= 3 and sequence_step >= 4 else interface_text("**단원 도입**", "**Unit introduction**"))
                st.caption(interface_text("가리키기 → 위치 묻기 → 위치 말하기 → 다른 물건 확인하기", "Point → Ask location → Say location → Check another object"))
            with replay:
                lesson_button(interface_text("단원 도입 다시보기", "Review unit introduction"), key="unit3_intro_expand", width="stretch",
                              on_click=set_session_state_value, args=("unit3_intro_collapsed", False))
            with close:
                if not st.session_state.unit3_intro_collapsed:
                    lesson_button(interface_text("닫기", "Close"), key="unit3_intro_close", on_click=set_session_state_value,
                                  args=("unit3_intro_collapsed", True))
        st.space("medium")
    if st.session_state.unit3_intro_collapsed:
        return
    render_learning_info(interface_text("단원 도입: 먼저 대화 모델을 3회 읽어 보세요.", "Unit introduction: First, read the model dialogue three times."), icon=":material/play_circle:")
    st.markdown(interface_text("### 단원 도입 · 대화 모델 3회 읽기", "### Unit introduction · Read the model dialogue three times"))
    st.caption(interface_text("물건을 가리키고 위치를 묻고 답하는 흐름을 먼저 익혀 보세요.", "First learn how to point out objects and ask and answer where they are."))
    dialogue, sequence = st.columns(2)
    lines = [
        "A: 이 책은 어디에 있어요?", "B: 책상 위에 있어요.",
        "A: 그 가방은 어디에 있어요?", "B: 의자 옆에 있어요.",
    ]
    with dialogue, st.container(border=True):
        st.markdown(interface_text("**대화 모델 · 3회 반복 읽기**", "**Model dialogue · Read three times**"))
        total = min(current_round, 3) * 4 + (current_line if current_round < 3 else 0)
        st.progress(total / 12, text=interface_text(f"읽기 진행 ({total}/12문장)", f"Reading progress ({total}/12 lines)"))
        for index, line in enumerate(lines):
            done = current_round >= 3 or index < current_line
            style = "done" if done else "active" if index == current_line else ""
            marker = "✓" if done else "●" if index == current_line else "○"
            st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{line}</span></div>", unsafe_allow_html=True)
        render_intro_reading_controls(3, current_round, 4)
    with sequence, st.container(border=True):
        st.markdown('<div id="unit3-intro-sequence"></div>', unsafe_allow_html=True)
        st.markdown(interface_text("**단원 도입 · 물건과 위치 순서**", "**Unit introduction · Object and location order**"))
        st.caption(interface_text("대화를 3회 읽은 뒤 아래 카드를 순서대로 눌러 확인하세요.", "Read the dialogue three times, then select the cards in order."))
        steps = [("가리키기", "Point it out", "이 책"), ("위치 묻기", "Ask where", "어디에 있어요?"), ("위치 말하기", "Say where", "책상 위에 있어요."), ("다른 물건 확인하기", "Check another object", "그 가방")]
        for index, (ko, en, example) in enumerate(steps):
            if index < sequence_step:
                with st.container(border=True):
                    st.markdown(f"**✓ {index + 1}. {interface_text(ko, en)}** · {example}")
            else:
                lesson_button(interface_text(f"{index + 1}. {ko} 확인하기", f"{index + 1}. {en}"), key=f"unit3_intro_sequence_{index}", width="stretch",
                              disabled=current_round < 3 or index != sequence_step or not st.session_state.get("unit3_intro_sequence_requested", False),
                              on_click=set_session_state_value, args=(sequence_key, index + 1))
        if sequence_step >= 4:
            render_intro_sequence_success(3)
        reset_column, next_column = st.columns([1, 1.35])
        with reset_column:
            lesson_button(interface_text("순서 다시 보기", "Review the order again"), key="unit3_intro_sequence_reset", width="stretch",
                          on_click=set_session_state_value, args=(sequence_key, 0))
        with next_column:
            if lesson_button(interface_text("다음: 어휘와 표현 학습하기 →", "Next: Learn vocabulary and expressions →"), key="unit3_intro_continue_vocabulary", type="primary", width="stretch", disabled=sequence_step < 4):
                continue_unit3_vocabulary()
        st.caption(interface_text("물건을 가리키고 위치를 묻고 답하는 표현을 배운 뒤 활동에서 직접 말해 보세요.", "Learn to point out objects and ask about locations, then practice speaking in the activities."))
    st.space("medium")


@st.fragment
def render_unit4_introduction():
    try:
        render_unit4_introduction_content()
    finally:
        checkpoint_lesson_progress()


def render_unit4_introduction_content():
    selected_tab = st.session_state.get("_unit4_lesson_tabs", "")
    vocabulary_visible = (
        selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
        if selected_tab else not get_unit_completion_steps(4)[0]
    )
    pending_tab = st.session_state.get("requested_lesson_tab")
    if pending_tab and pending_tab[0] == 4:
        vocabulary_visible = pending_tab[1] == 0
    if st.session_state.get("force_unit_home_step") == 4:
        vocabulary_visible = True
    st.session_state.setdefault("unit4_intro_collapsed", not vocabulary_visible)
    if vocabulary_visible:
        st.session_state.unit4_intro_collapsed = False
    current_round = st.session_state.get("unit4_read_round", 0)
    current_line = st.session_state.get("unit4_read_line", 0)
    sequence_step = min(int(st.session_state.get("unit4_intro_sequence_step", 0)), 4)
    if not vocabulary_visible:
        with st.container(border=True):
            summary, replay, close = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary:
                st.markdown(interface_text("**단원 도입 완료 ✓**", "**Unit introduction complete ✓")
                            if current_round >= 3 and sequence_step >= 4 else interface_text("**단원 도입**", "**Unit introduction**"))
                st.caption(interface_text("오늘 할 일 묻기 → 대답하기 → 되묻기 → 다른 할 일 말하기", "Ask → Answer → Ask back → Say another activity"))
            with replay:
                lesson_button(interface_text("단원 도입 다시보기", "Review unit introduction"),
                              key="unit4_intro_expand", width="stretch", on_click=set_session_state_value,
                              args=("unit4_intro_collapsed", False))
            with close:
                if not st.session_state.unit4_intro_collapsed:
                    lesson_button(interface_text("닫기", "Close"), key="unit4_intro_close",
                                  on_click=set_session_state_value, args=("unit4_intro_collapsed", True))
        st.space("medium")
    if st.session_state.unit4_intro_collapsed:
        return
    render_learning_info(interface_text("단원 도입: 먼저 대화 모델을 3회 읽어 보세요.", "Unit introduction: First, read the model dialogue three times."), icon=":material/play_circle:")
    st.markdown(interface_text("### 단원 도입 · 대화 모델 3회 읽기", "### Unit introduction · Read the model dialogue three times"))
    st.caption(interface_text("오늘 하는 일을 묻고 답하는 대화예요.", "This dialogue asks and answers what someone is doing today."))
    dialogue, sequence = st.columns(2)
    lines = ["가: 오늘 뭐 해요?", "나: 한국어를 공부해요.", "나: 오늘 뭐 해요?", "가: 저는 친구를 만나요."]
    with dialogue, st.container(border=True):
        st.markdown(interface_text("**대화 모델 · 3회 반복 읽기**", "**Model dialogue · Read three times**"))
        completed_reads = min(current_round, 3) * len(lines) + (current_line if current_round < 3 else 0)
        st.progress(completed_reads / 12, text=interface_text(f"읽기 진행 ({completed_reads}/12문장)", f"Reading progress ({completed_reads}/12 lines)"))
        for index, line in enumerate(lines):
            done = current_round >= 3 or index < current_line
            style = "done" if done else "active" if index == current_line else ""
            marker = "✓" if done else "●" if index == current_line else "○"
            st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{line}</span></div>", unsafe_allow_html=True)
        render_intro_reading_controls(4, current_round, len(lines))
    with sequence, st.container(border=True):
        st.markdown('<div id="unit4-intro-sequence"></div>', unsafe_allow_html=True)
        st.markdown(interface_text("**단원 도입 · 오늘 할 일 대화 순서**", "**Unit introduction · Activity dialogue order**"))
        st.caption(interface_text("대화를 3회 읽은 뒤 아래 카드를 순서대로 눌러 확인하세요.", "Read the dialogue three times, then select the cards in order."))
        steps = [("오늘 할 일 묻기", "Ask about today", "오늘 뭐 해요?"),
                 ("대답하기", "Answer", "한국어를 공부해요."),
                 ("되묻기", "Ask back", "오늘 뭐 해요?"),
                 ("다른 할 일 말하기", "Say another activity", "친구를 만나요.")]
        for index, (ko, en, example) in enumerate(steps):
            if index < sequence_step:
                with st.container(border=True):
                    st.markdown(f"**✓ {index + 1}. {interface_text(ko, en)}** · {example}")
            else:
                lesson_button(interface_text(f"{index + 1}. {ko} 확인하기", f"{index + 1}. {en}"),
                              key=f"unit4_intro_sequence_{index}", width="stretch",
                              disabled=current_round < 3 or index != sequence_step or not st.session_state.get("unit4_intro_sequence_requested", False),
                              on_click=set_session_state_value, args=("unit4_intro_sequence_step", index + 1))
        if sequence_step >= 4:
            render_intro_sequence_success(4)
        reset_column, next_column = st.columns([1, 1.35])
        with reset_column:
            lesson_button(interface_text("순서 다시 보기", "Review the order again"),
                          key="unit4_intro_sequence_reset", width="stretch",
                          on_click=set_session_state_value, args=("unit4_intro_sequence_step", 0))
        with next_column:
            if lesson_button(interface_text("다음: 어휘와 표현 학습하기 →", "Next: Learn vocabulary and expressions →"),
                             key="unit4_intro_continue_vocabulary", type="primary", width="stretch", disabled=sequence_step < 4):
                request_lesson_tab(4, 0)
                st.session_state["_intro_vocabulary_heading_unit"] = 4
                st.rerun()
    st.space("medium")


@st.fragment
def render_unit5_introduction():
    try:
        render_unit5_introduction_content()
    finally:
        checkpoint_lesson_progress()


def render_unit5_introduction_content():
    selected_tab = st.session_state.get("_unit5_lesson_tabs", "")
    vocabulary_visible = (
        selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
        if selected_tab else not get_unit_completion_steps(5)[0]
    )
    pending_tab = st.session_state.get("requested_lesson_tab")
    if pending_tab and pending_tab[0] == 5:
        vocabulary_visible = pending_tab[1] == 0
    if st.session_state.get("force_unit_home_step") == 5:
        vocabulary_visible = True
    st.session_state.setdefault("unit5_intro_collapsed", not vocabulary_visible)
    if vocabulary_visible:
        st.session_state.unit5_intro_collapsed = False
    current_round = st.session_state.get("unit5_read_round", 0)
    current_line = st.session_state.get("unit5_read_line", 0)
    sequence_step = min(int(st.session_state.get("unit5_intro_sequence_step", 0)), 4)
    if not vocabulary_visible:
        with st.container(border=True):
            summary, replay, close = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary:
                st.markdown(interface_text("**단원 도입 완료 ✓**", "**Unit introduction complete ✓")
                            if current_round >= 3 and sequence_step >= 4 else interface_text("**단원 도입**", "**Unit introduction**"))
                st.caption(interface_text("장소 묻기 → 장소 말하기 → 살 물건 묻기 → 물건 말하기", "Ask destination → Answer → Ask purchases → Answer"))
            with replay:
                lesson_button(interface_text("단원 도입 다시보기", "Review unit introduction"),
                              key="unit5_intro_expand", width="stretch", on_click=set_session_state_value,
                              args=("unit5_intro_collapsed", False))
            with close:
                if not st.session_state.unit5_intro_collapsed:
                    lesson_button(interface_text("닫기", "Close"), key="unit5_intro_close",
                                  on_click=set_session_state_value, args=("unit5_intro_collapsed", True))
        st.space("medium")
    if st.session_state.unit5_intro_collapsed:
        return
    render_learning_info(interface_text("단원 도입: 먼저 대화 모델을 3회 읽어 보세요.", "Unit introduction: First, read the model dialogue three times."), icon=":material/play_circle:")
    st.markdown(interface_text("### 단원 도입 · 대화 모델 3회 읽기", "### Unit introduction · Read the model dialogue three times"))
    st.caption(interface_text("어디에 가는지, 무엇을 사는지 묻고 답하는 대화예요.", "This dialogue asks where someone is going and what they will buy."))
    dialogue, sequence = st.columns(2)
    lines = ["안나: 주노 씨, 어디에 가요?", "주노: 마트에 가요.", "안나: 뭘 사요?", "주노: 빵하고 우유를 사요."]
    with dialogue, st.container(border=True):
        st.markdown(interface_text("**대화 모델 · 3회 반복 읽기**", "**Model dialogue · Read three times**"))
        completed_reads = min(current_round, 3) * len(lines) + (current_line if current_round < 3 else 0)
        st.progress(completed_reads / 12, text=interface_text(f"읽기 진행 ({completed_reads}/12문장)", f"Reading progress ({completed_reads}/12 lines)"))
        for index, line in enumerate(lines):
            ...
            marker = "✓" if done else "●" if index == current_line else "○"
            st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{line}</span></div>", unsafe_allow_html=True)
        render_intro_reading_controls(5, current_round, len(lines))
    with sequence, st.container(border=True):
        st.markdown('<div id="unit5-intro-sequence"></div>', unsafe_allow_html=True)
        st.markdown(interface_text("**단원 도입 · 장보기 대화 순서**", "**Unit introduction · Shopping dialogue order**"))
        st.caption(interface_text("대화를 3회 읽은 뒤 아래 카드를 순서대로 눌러 확인하세요.", "Read the dialogue three times, then select the cards in order."))
        steps = [("장소 묻기", "Ask destination", "어디에 가요?"),
                 ("장소 말하기", "Answer destination", "마트에 가요."),
                 ("살 물건 묻기", "Ask purchases", "뭘 사요?"),
                 ("물건 말하기", "Answer purchases", "빵하고 우유를 사요.")]
        for index, (ko, en, example) in enumerate(steps):
            if index < sequence_step:
                with st.container(border=True):
                    st.markdown(f"**✓ {index + 1}. {interface_text(ko, en)}** · {example}")
                ...
                lesson_button(interface_text(f"{index + 1}. {ko} 확인하기", f"{index + 1}. {en}"),
                              key=f"unit5_intro_sequence_{index}", width="stretch",
                              disabled=current_round < 3 or index != sequence_step or not st.session_state.get("unit5_intro_sequence_requested", False),
                              on_click=set_session_state_value, args=("unit5_intro_sequence_step", index + 1))
        if sequence_step >= 4:
            render_intro_sequence_success(5)
        reset_column, next_column = st.columns([1, 1.35])
        with reset_column:
            lesson_button(interface_text("순서 다시 보기", "Review the order again"),
                          key="unit5_intro_sequence_reset", width="stretch",
                          on_click=set_session_state_value, args=("unit5_intro_sequence_step", 0))
        with next_column:
            if lesson_button(interface_text("다음: 어휘와 표현 학습하기 →", "Next: Learn vocabulary and expressions →"),
                             key="unit5_intro_continue_vocabulary", type="primary", width="stretch", disabled=sequence_step < 4):
                request_lesson_tab(5, 0)
                st.session_state["_intro_vocabulary_heading_unit"] = 5
                st.rerun()
    st.space("medium")


LATER_UNIT_INTRODUCTIONS = {
    6: {
        "caption": ("과일 가게에서 물건과 수량을 묻고 답하는 대화예요.", "This dialogue asks for items and quantities at a fruit shop."),
        "sequence": ("주문 대화 순서", "Order dialogue sequence"),
        "lines": ["주인: 무엇을 드릴까요?", "손님: 사과 다섯 개 주세요.", "주인: 더 필요하세요?", "손님: 아니요. 얼마예요?"],
        "steps": [("필요한 물건 묻기", "Ask what is needed", "무엇을 드릴까요?"), ("물건과 수량 말하기", "Say item and quantity", "사과 다섯 개 주세요."), ("추가 주문 묻기", "Ask about another item", "더 필요하세요?"), ("가격 묻기", "Ask the price", "얼마예요?")],
    },
    7: {
        "caption": ("요일과 시간을 묻고 수업 일정을 말하는 대화예요.", "This dialogue asks about a weekday and class time."),
        "sequence": ("수업 일정 대화 순서", "Class schedule dialogue sequence"),
        "lines": ["가: 한국어 수업은 언제 해요?", "나: 목요일에 해요.", "가: 몇 시에 시작해요?", "나: 저녁 일곱 시에 시작해요."],
        "steps": [("요일 묻기", "Ask the weekday", "언제 해요?"), ("요일 말하기", "Say the weekday", "목요일에 해요."), ("시간 묻기", "Ask the time", "몇 시에 시작해요?"), ("시간 말하기", "Say the time", "저녁 일곱 시에 시작해요.")],
    },
    8: {
        "caption": ("날씨와 계절을 묻고 상태를 말하는 대화예요.", "This dialogue asks about weather and seasons."),
        "sequence": ("날씨 대화 순서", "Weather dialogue sequence"),
        "lines": ["가: 오늘 날씨가 어때요?", "나: 날씨가 더워요.", "가: 겨울에는 어때요?", "나: 겨울에는 추워요."],
        "steps": [("오늘 날씨 묻기", "Ask today's weather", "날씨가 어때요?"), ("오늘 날씨 말하기", "Describe today's weather", "더워요."), ("계절 날씨 묻기", "Ask about a season", "겨울에는 어때요?"), ("계절 날씨 말하기", "Describe a season", "겨울에는 추워요.")],
    },
    9: {
        "caption": ("어제 어디에서 무엇을 했는지 묻고 답하는 대화예요.", "This dialogue asks where someone went and what they did yesterday."),
        "sequence": ("지난 활동 대화 순서", "Past activity dialogue sequence"),
        "lines": ["가: 어제 어디에 갔어요?", "나: 공원에 갔어요.", "가: 공원에서 뭐 했어요?", "나: 산책했어요."],
        "steps": [("어제 간 곳 묻기", "Ask where someone went", "어디에 갔어요?"), ("장소 말하기", "Say the place", "공원에 갔어요."), ("한 일 묻기", "Ask what someone did", "뭐 했어요?"), ("한 일 말하기", "Say what happened", "산책했어요.")],
    },
    10: {
        "caption": ("함께 할 일을 제안하고 가는 목적을 말하는 대화예요.", "This dialogue proposes an activity and explains its purpose."),
        "sequence": ("주말 약속 대화 순서", "Weekend plan dialogue sequence"),
        "lines": ["가: 주말에 같이 영화관에 갈까요?", "나: 네, 좋아요. 왜 가요?", "가: 영화를 보러 가요.", "나: 좋아요. 토요일에 만나요."],
        "steps": [("함께 가자고 제안하기", "Suggest going together", "같이 영화관에 갈까요?"), ("가는 이유 묻기", "Ask why", "왜 가요?"), ("가는 목적 말하기", "Say the purpose", "영화를 보러 가요."), ("약속 정하기", "Make the plan", "토요일에 만나요.")],
    },
}


@st.fragment
def render_later_unit_introduction(unit_number):
    try:
        render_later_unit_introduction_content(unit_number)
    finally:
        checkpoint_lesson_progress()


def render_later_unit_introduction_content(unit_number):
    content = LATER_UNIT_INTRODUCTIONS[unit_number]
    selected_tab = st.session_state.get(f"_unit{unit_number}_lesson_tabs", "")
    vocabulary_visible = (
        selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
        if selected_tab else not get_unit_completion_steps(unit_number)[0]
    )
    pending_tab = st.session_state.get("requested_lesson_tab")
    if pending_tab and pending_tab[0] == unit_number:
        vocabulary_visible = pending_tab[1] == 0
    if st.session_state.get("force_unit_home_step") == unit_number:
        vocabulary_visible = True
    collapsed_key = f"unit{unit_number}_intro_collapsed"
    st.session_state.setdefault(collapsed_key, not vocabulary_visible)
    if vocabulary_visible:
        st.session_state[collapsed_key] = False
    current_round = st.session_state.get(f"unit{unit_number}_read_round", 0)
    current_line = st.session_state.get(f"unit{unit_number}_read_line", 0)
    sequence_step = min(int(st.session_state.get(f"unit{unit_number}_intro_sequence_step", 0)), 4)
    if not vocabulary_visible:
        with st.container(border=True):
            summary, replay, close = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary:
                st.markdown(interface_text("**단원 도입 완료 ✓**", "**Unit introduction complete ✓")
                            if current_round >= 3 and sequence_step >= 4 else interface_text("**단원 도입**", "**Unit introduction**"))
                st.caption(" → ".join(interface_text(ko, en) for ko, en, _ in content["steps"]))
            with replay:
                lesson_button(interface_text("단원 도입 다시보기", "Review unit introduction"),
                              key=f"unit{unit_number}_intro_expand", width="stretch",
                              on_click=set_session_state_value, args=(collapsed_key, False))
            with close:
                if not st.session_state[collapsed_key]:
                    lesson_button(interface_text("닫기", "Close"), key=f"unit{unit_number}_intro_close",
                                  on_click=set_session_state_value, args=(collapsed_key, True))
        st.space("medium")
    if st.session_state[collapsed_key]:
        return
    render_learning_info(interface_text("단원 도입: 먼저 대화 모델을 3회 읽어 보세요.", "Unit introduction: First, read the model dialogue three times."), icon=":material/play_circle:")
    st.markdown(interface_text("### 단원 도입 · 대화 모델 3회 읽기", "### Unit introduction · Read the model dialogue three times"))
    st.caption(interface_text(*content["caption"]))
    dialogue, sequence = st.columns(2)
    lines = content["lines"]
    with dialogue, st.container(border=True):
        st.markdown(interface_text("**대화 모델 · 3회 반복 읽기**", "**Model dialogue · Read three times**"))
        completed_reads = min(current_round, 3) * len(lines) + (current_line if current_round < 3 else 0)
        total_reads = 3 * len(lines)
        st.progress(completed_reads / total_reads, text=interface_text(f"읽기 진행 ({completed_reads}/{total_reads}문장)", f"Reading progress ({completed_reads}/{total_reads} lines)"))
        for index, line in enumerate(lines):
            done = current_round >= 3 or index < current_line
            style = "done" if done else "active" if index == current_line else ""
            marker = "✓" if done else "●" if index == current_line else "○"
            st.markdown(f"<div class='reading-line {style}'><span class='reading-line-marker'>{marker}</span><span class='reading-line-text'>{html.escape(line)}</span></div>", unsafe_allow_html=True)
        render_intro_reading_controls(unit_number, current_round, len(lines))
    with sequence, st.container(border=True):
        st.markdown(interface_text(f"**단원 도입 · {content['sequence'][0]}**", f"**Unit introduction · {content['sequence'][1]}**"))
        st.caption(interface_text("대화를 3회 읽은 뒤 아래 카드를 순서대로 눌러 확인하세요.", "Read the dialogue three times, then select the cards in order."))
        for index, (ko, en, example) in enumerate(content["steps"]):
            if index < sequence_step:
                with st.container(border=True):
                    st.markdown(f"**✓ {index + 1}. {interface_text(ko, en)}** · {example}")
            else:
                lesson_button(interface_text(f"{index + 1}. {ko} 확인하기", f"{index + 1}. {en}"),
                              key=f"unit{unit_number}_intro_sequence_{index}", width="stretch",
                              disabled=current_round < 3 or index != sequence_step or not st.session_state.get(f"unit{unit_number}_intro_sequence_requested", False),
                              on_click=set_session_state_value, args=(f"unit{unit_number}_intro_sequence_step", index + 1))
        if sequence_step >= 4:
            render_intro_sequence_success(unit_number)
        reset_column, next_column = st.columns([1, 1.35])
        with reset_column:
            lesson_button(interface_text("순서 다시 보기", "Review the order again"),
                          key=f"unit{unit_number}_intro_sequence_reset", width="stretch",
                          on_click=set_session_state_value, args=(f"unit{unit_number}_intro_sequence_step", 0))
        with next_column:
            if lesson_button(interface_text("다음: 어휘와 표현 학습하기 →", "Next: Learn vocabulary and expressions →"),
                             key=f"unit{unit_number}_intro_continue_vocabulary", type="primary", width="stretch", disabled=sequence_step < 4):
                request_lesson_tab(unit_number, 0)
                st.session_state["_intro_vocabulary_heading_unit"] = unit_number
                st.rerun()
    st.space("medium")
...
def dashboard():
    ...
    with left:
        if lesson_button(interface_text("다음 · 받침 발음 훈련  →", "NEXT · 받침 발음 훈련  →"), type="primary", key="next_lesson", width="stretch"):
        ...
        if lesson_button(interface_text("학습 시작하기  →", "Start learning  →"), type="primary", width="stretch"):
            st.session_state.go_practice = True
            st.rerun()
    with right:
...
def practice():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    with st.container(border=True):
        st.markdown('<div class="eyebrow">Question 03 / 06</div><h2>빈칸에 알맞은 조사를 골라 보세요.</h2><h2 style="margin-top:32px">오늘<span class="blank-placeholder" style="color:#888 !important;font-weight:400">▯</span> 날씨가 정말 좋아요.</h2>', unsafe_allow_html=True)
        choice = st.radio("조사 선택", ["은", "는", "이", "가"], horizontal=True, index=None, label_visibility="collapsed")
        if lesson_button("확인하기", type="primary"):
...
def assignments():
    ...
    if lesson_button("과제 제출하기  →", type="primary", width="stretch"): st.success("제출 준비가 완료되었습니다. 강사 피드백을 기다려 주세요.")
...
def practice():
    ...
    if st.session_state.practice_completed:
        ...
        with learning_button:
            lesson_button("선택 단원 학습으로", key="practice_go_learning", icon=":material/menu_book:", width="stretch", on_click=navigate_to_page, args=("내 학습",))
        with builder_button:
            lesson_button("문장 조합으로", key="practice_go_builder", icon=":material/extension:", width="stretch", on_click=navigate_to_page, args=("문장 조합",))
        with restart_button:
            if lesson_button("오늘 문제 다시 풀기", key="restart_practice", icon=":material/replay:", width="stretch"):
        ...
        with st.container(border=True):
            ...
            st.markdown(f'<h2 style="margin-top:32px">{question["sentence"].replace("__", "<span class=\"blank-placeholder\" style=\"color:#888 !important;font-weight:400\">▯</span>")}</h2>', unsafe_allow_html=True)
            choice = st.radio("답 선택", question["options"], horizontal=True, index=None, key=f"practice_choice_{index}", label_visibility="collapsed")
            result = st.session_state.practice_result
            completed_sentence = question["sentence"].replace("__", question["answer"])
            if result is None:
                if lesson_button("확인하기", type="primary", key=f"check_{index}"):
                ...
                if result["correct"]:
                    if lesson_button("다음 문제  →", type="primary", key=f"next_{index}"):
                    ...
                    if lesson_button("설명 확인 후 다시 풀기", type="primary", key=f"retry_{index}"):
                        st.session_state.practice_result = None
                        st.rerun()
...
def pronunciation_mission():
    ...
    with left:
        ...
        if audio:
            ...
            if lesson_button("OpenAI로 발음 분석하기", type="primary", width="stretch"):
        ...
        if result:
            ...
            if st.session_state.get("unlocked_stage", 1) < 2 and lesson_button("미션 완료하고 다음 단계 열기 →", type="primary", key="unlock_pronunciation_real"):
                st.session_state.unlocked_stage = 2
                st.session_state.total_xp = st.session_state.get("total_xp", 0) + 30
                save_progress()
                st.rerun()
...
def sync_mode():
    ...
    with c1:
        ...
        if lesson_button("정답 제출 →", type="primary", key="sync_submit_real", width="stretch"):
...
def teacher_dashboard():
    ...
    with left:
        ...
        if lesson_button("교실에 미션 보내기  →", type="primary", width="stretch"):
    ...
    for column, (key, label, description, reward) in zip(cols, tasks):
        with column:
            ...
            with st.container(border=True):
                ...
                st.caption(reward)
                if not done and lesson_button("시작하기", key=f"quest_{key}", type="primary" if key == "grammar" else "secondary"):
                    if key == "vocab":
...
def render_unit2_place_phone_quiz():
    ...
    with place_button_columns[0]:
        if lesson_button(interface_text("1번 답 확인", "Check Activity 1 answers"), key="unit2_activity2_place_check", type="primary", disabled=not place_answers_ready, width="stretch"):
    ...
    with place_button_columns[1]:
        if place_results is not None:
            lesson_button(interface_text("확인 닫기", "Close feedback"), key="unit2_activity2_place_result_close", type="secondary", width="stretch", on_click=clear_session_state_key, args=("unit2_activity2_place_results",))
    ...
    return bool(
        ...
        and (
            ...
            or st.session_state.get("unit2_activity2_place_passed", False)
        )
    )
...
def balanced_answer_options(options, answer, question_index):
    """Vary the correct answer's position without changing answer values."""
    choices = list(options)
    if answer not in choices or len(choices) < 2:
        return choices
    distractors = choices.copy()
    distractors.remove(answer)
    if len(choices) == 2:
        answer_position = question_index % 2
    else:
        answer_position = (0, 2, 1)[question_index % 3]
    distractors.insert(answer_position, answer)
    return distractors
...
def render_choice_set(
    ...
    selections = []
    reset_suffix = f"_r{st.session_state.get('unit3_reset_nonce', 0)}" if unit_number == 3 else ""
    question_area = (
            ...
            with question_content:
                ...
                selections.append(
                    st.selectbox(
                        ...
                        balanced_answer_options(options, answer, index),
                        index=None,
                        placeholder=interface_text("선택하세요", "Choose"),
                        key=f"unit{unit_number}_{stage}_choice_{index}{reset_suffix}",
    ...
    if lesson_button(
        ...
        key=f"unit{unit_number}_{stage}_check{reset_suffix}",
        type="primary",
        disabled=not ready,
        help=(interface_text("먼저 답을 모두 선택하세요.", "Choose all answers first.") if not ready and unit_number in (1, 2, 3, 4) else None),
    ):
        if feedback_visible:
            st.session_state[feedback_key] = False
        else:
...
def render_unit3_vocabulary_panel(vocabulary_words, active_index, example_sentence, selected_word, selected_meaning):
    ...
    with learning_column:
        ...
        with meaning_button_column:
            if lesson_button(interface_text("뜻 보기 / 뜻 가리기", "Show / hide meaning"), key="vocab_flip_3", width="stretch"):
...
def render_unit4_grammar2():
    ...
    preference_questions = [
        ("꽃 그림: 무엇을 좋아해요?", ["꽃를 좋아해요.", "꽃을 좋아해요.", "꽃이 좋아해요."], "꽃을 좋아해요.", "‘꽃’은 받침이 있으므로 ‘을’을 사용해요."),
        ("커피 그림: 무엇을 좋아해요?", ["커피을 좋아해요.", "커피가 좋아해요.", "커피를 좋아해요."], "커피를 좋아해요.", "‘커피’는 받침이 없으므로 ‘를’을 사용해요."),
        ("음악 그림: 무엇을 좋아해요?", ["음악을 좋아해요.", "음악를 좋아해요.", "음악에 좋아해요."], "음악을 좋아해요.", "‘음악’은 받침이 있으므로 ‘을’을 사용해요."),
        ("불고기 그림: 무엇을 좋아해요?", ["불고기을 좋아해요.", "불고기를 좋아해요.", "불고기가 좋아해요."], "불고기를 좋아해요.", "‘불고기’는 받침이 없으므로 ‘를’을 사용해요."),
    ]
...
def render_unit4_grammar1_intro():
    ...
    selected_action = st.selectbox(
        interface_text("오늘 할 일", "Activity to practice"), list(speaking_actions),
        index=None, placeholder=interface_text("선택하세요", "Choose"),
        key="unit4_grammar1_speaking_action",
    )
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit4_grammar1_speaking_check", type="primary", disabled=selected_action is None):
        st.session_state["unit4_grammar1_speaking_completed"] = True
        st.session_state["unit4_grammar1_checked_action"] = selected_action
        queue_grammar1_final_scroll(4)
    speaking_done = (
        selected_action is not None
        and st.session_state.get("unit4_grammar1_speaking_completed", False)
...
def render_unit3_activity1():
    ...
    selected_row = st.selectbox(interface_text("연습할 대화", "Dialogue to practice"), list(dialogue_rows), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit3_activity1_dialogue_row")
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit3_activity1_dialogue_done", type="primary", disabled=selected_row is None):
        st.session_state["unit3_activity1_dialogue_completed"] = True
        st.session_state["unit3_activity1_checked_row"] = selected_row
    dialogue_done = (
        selected_row is not None
        and st.session_state.get("unit3_activity1_dialogue_completed", False)
...
def render_unit4_activity1():
    ...
    selected_row = st.selectbox(
        interface_text("연습할 사람과 할 일", "Person and activity to practice"),
        list(activity_rows), index=None,
        placeholder=interface_text("선택하세요", "Choose"), key="unit4_activity1_row",
    )
    if selected_row is not None:
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit4_activity1_speaking_done", type="primary", disabled=selected_row is None):
        st.session_state["unit4_activity1_speaking_completed"] = True
        st.session_state["unit4_activity1_checked_row"] = selected_row
    speaking_done = (
        selected_row is not None
        and st.session_state.get("unit4_activity1_speaking_completed", False)
...
def render_unit3_activity2():
    ...
    st.divider()
    st.markdown(interface_text("### 2. 여러분의 방을 꾸미고 소개해 보세요.", "### 2. Design and describe your room"))
    st.caption(interface_text("세 개의 박스에서 서로 다른 물건과 알맞은 위치를 선택하세요.", "Choose three different objects and a suitable location for each one."))
    room_items = ["침대", "책상", "의자", "책", "가방", "컴퓨터", "시계", "옷장"]
    locations_by_item = {
        "침대": ["창문 옆", "문 옆", "왼쪽 벽 옆", "오른쪽 벽 옆"],
        "책상": ["창문 아래", "침대 옆", "왼쪽 벽 옆", "오른쪽 벽 옆"],
        "의자": ["책상 앞", "창문 옆", "침대 옆"],
        "책": ["책상 위", "침대 위", "가방 안", "옷장 안"],
        "가방": ["책상 위", "책상 아래", "의자 위", "침대 옆"],
        "컴퓨터": ["책상 위"],
        "시계": ["책상 위", "침대 옆", "벽"],
        "옷장": ["문 옆", "왼쪽 벽 옆", "오른쪽 벽 옆"],
    }
    slot_names = ["first", "second", "third"]
    slot_titles = [
        ("첫 번째 물건 놓기", "Place the first object"),
        ("두 번째 물건 놓기", "Place the second object"),
        ("세 번째 물건 놓기", "Place the third object"),
    ]
    room_selections = []
    room_columns = st.columns(3, gap="medium", vertical_alignment="top")
    for column, slot_name, (title_ko, title_en) in zip(room_columns, slot_names, slot_titles):
        with column:
            with st.container(border=True, height=300):
                ...
                location = st.selectbox(
                    interface_text("위치", "Location"), locations_by_item.get(item, []),
                    index=None, placeholder=interface_text("선택하세요", "Choose"),
                    key=f"unit3_activity2_builder_{slot_name}_location",
                    disabled=item is None,
                )
                room_selections.append((item, location))
    selections_ready = all(item and location for item, location in room_selections)
    selected_items = [item for item, _ in room_selections if item]
    different_items = bool(selections_ready and len(set(selected_items)) == 3)
    writing_ready = selections_ready and different_items
    room_description = ""
    if writing_ready:
        sentences = [f"{location}에 {item}{subject_particle(item)} 있어요." for item, location in room_selections]
        room_description = "제 방이에요. " + " ".join(sentences)
        render_learning_success(room_description, icon=":material/edit_note:")
    elif selections_ready:
        render_learning_warning(interface_text("서로 다른 물건 세 개를 선택해 주세요.", "Choose three different objects."))
...
def render_unit4_activity2():
    ...
    st.divider()
    st.markdown(interface_text("### 2. 여러분은 오늘 무엇을 해요? 골라서 문장을 완성해 보세요.", "### 2. What are you doing today? Choose words to complete three Korean sentences"))
    ...
    render_learning_info(
        interface_text("**선택 예문**", "**Example choices**") + "\n\n"
        "① 저는 지금 **집**에 있어요.\n\n"
        "② 오늘 **한국어를 공부해요**.\n\n"
        "③ 그리고 **음악을 들어요**.",
    ...
    with writing_columns[0]:
        place = st.selectbox(
            interface_text("① 지금 있는 곳", "① Current location"),
            ["집", "학교", "공원", "카페", "도서관", "회사", "식당", "교실"],
            index=None, placeholder=unit4_choose_label,
            key="unit4_activity2_place_choice",
        )
    ...
    writing_ready = place is not None and first_action != unit4_choose_label and second_action != unit4_choose_label
    response = ""
    if writing_ready:
        response = f"저는 지금 {place}에 있어요. 오늘 {first_action.rstrip('.')}. 그리고 {second_action.rstrip('.')}."
...
def render_unit5_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    for column, index in zip(place_columns, range(6)):
        with column:
            lesson_button(vocabulary_words[index], key=f"vocab_select_5_{index}",
                      type="primary" if index == active_index else "secondary",
    ...
    # The milk card is much narrower than the other crops. Give it the same
    # canvas width so Streamlit's fixed display width does not enlarge it.
    milk_crop = crop_image_region(unit5_vocab_image, (1080, 600, 1198, 875))
    milk_card = Image.new("RGBA", (180, 275), (255, 251, 243, 255))
    milk_card.alpha_composite(milk_crop, ((milk_card.width - milk_crop.width) // 2, 0))
    milk_mask = Image.new("L", milk_card.size, 0)
    ImageDraw.Draw(milk_mask).rounded_rectangle((0, 0, 179, 274), radius=15, fill=255)
    milk_card.putalpha(milk_mask)
    question_images = [
    ...
    def dialogue_image(box):
        # Match the restaurant crop's 223 × 375 frame for every picture.
        # Keep each illustration's original scale within that frame.
        cropped = crop_image_region(unit5_vocab_image, box)
        framed = Image.new("RGBA", (223, 375), (255, 251, 243, 255))
        framed.alpha_composite(cropped, ((223 - cropped.width) // 2, (375 - cropped.height) // 2))
        rounded = Image.new("L", framed.size, 0)
        ImageDraw.Draw(rounded).rounded_rectangle((0, 0, 222, 374), radius=15, fill=255)
        framed.putalpha(rounded)
        return framed

    dialogue_question_images = [
        (
            dialogue_image((542, 160, 765, 535)),
            dialogue_image((248, 600, 425, 875)),
        ),
        (
            dialogue_image((800, 160, 1018, 535)),
            dialogue_image((880, 600, 1048, 875)),
        ),
        (
            dialogue_image((1052, 160, 1260, 535)),
            dialogue_image((1222, 600, 1358, 875)),
        ),
        (
            dialogue_image((1290, 160, 1500, 535)),
            dialogue_image((1080, 600, 1198, 875)),
        ),
    ]
    dialogue_questions = [
        ("가: 여기는 어디예요?\n나: ______\n가: 무엇을 먹어요?\n나: ______", [
...
def render_unit5_grammar1_intro():
    ...
    questions = [
        ...
        ("재민 씨는 어디에 가요?", ["회사를 가요.", "회사에 가요.", "회사하고 가요."], "회사에 가요.", "‘회사에 가요’가 맞아요."),
        ("안나 씨는 학교에 가요?", ["네. 마트를 가요.", "아니요. 마트하고 가요.", "아니요. 마트에 가요."], "아니요. 마트에 가요.", "목적지 ‘마트’ 뒤에 ‘에’를 붙여요."),
        ("유진 씨는 회사에 가요?", ["네. 카페를 가요.", "아니요. 카페에 가요.", "아니요. 카페하고 가요."], "아니요. 카페에 가요.", "‘카페에 가요’라고 말해요."),
    ]
    ...
    destination = st.selectbox(
        interface_text("가는 장소", "Destination"),
        ["세종학당", "공원", "식당", "카페", "마트"],
        index=None, placeholder=interface_text("선택하세요", "Choose"),
        key="unit5_g1_destination",
    )
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit5_g1_speaking_done", type="primary", disabled=destination is None):
        st.session_state["unit5_grammar1_speaking_completed"] = True
        st.session_state["unit5_g1_checked_destination"] = destination
        queue_grammar1_final_scroll(5)
    speaking_done = destination is not None and st.session_state.get("unit5_grammar1_speaking_completed", False) and st.session_state.get("unit5_g1_checked_destination") == destination
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
...
def render_unit5_grammar2():
    ...
    item1 = st.selectbox(interface_text("교실에 있는 물건", "Object in the classroom"), ["시계", "칠판", "책상", "컴퓨터"], index=None, placeholder=interface_text("선택하세요", "Choose"), key="unit5_g2_item1_choice")
    person = st.selectbox(interface_text("교실에 있는 사람", "Person in the classroom"), ["안나 씨", "주노 씨", "마리 씨", "재민 씨", "유진 씨", "수지 씨"], index=None, placeholder=interface_text("선택하세요", "Choose"), key="unit5_g2_person_choice")
    sentence_ready = bool(item1 and person)
    if sentence_ready:
        person_particle = subject_particle(person)
        render_learning_info(f"교실에 {item1}하고 {person}{person_particle} 있어요.", icon=":material/forum:")
    if lesson_button(interface_text("2번 문장을 소리 내어 읽었어요", "I read the Activity 2 sentence aloud"), key="unit5_g2_speaking_done", type="primary", disabled=not sentence_ready):
        st.session_state["unit5_grammar2_speaking_completed"] = True
        st.session_state["unit5_g2_checked_items"] = (item1, person)
    speaking_done = sentence_ready and st.session_state.get("unit5_grammar2_speaking_completed", False) and st.session_state.get("unit5_g2_checked_items") == (item1, person)
    if speaking_done:
        render_learning_success(interface_text("2번 문장 읽기를 완료했어요.", "You completed the Activity 2 sentence reading."), icon=":material/check_circle:")
    else:
...
def render_unit5_activity1():
    ...
    place = st.selectbox(
        interface_text("장소", "Place"), ["식당", "백화점", "학교", "마트"],
        index=None, placeholder=interface_text("선택하세요", "Choose"),
        key="unit5_a1_place",
    )
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit5_a1_speaking_done", type="primary", disabled=place is None):
        st.session_state["unit5_activity1_speaking_completed"] = True
        st.session_state["unit5_a1_checked_place"] = place
    speaking_done = place is not None and st.session_state.get("unit5_activity1_speaking_completed", False) and st.session_state.get("unit5_a1_checked_place") == place
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
...
def render_unit5_activity2():
    ...
    first = st.selectbox(interface_text("첫 번째 물건", "First item"), ["옷", "신발", "가방", "화장품", "모자"], index=None, placeholder=interface_text("선택하세요", "Choose"), key="unit5_a2_first")
    second = st.selectbox(interface_text("두 번째 물건", "Second item"), ["신발", "옷", "가방", "화장품", "모자"], index=None, placeholder=interface_text("선택하세요", "Choose"), key="unit5_a2_second")
    response = ""
    sentence_ready = bool(first and second and first != second)
    if sentence_ready:
        ...
        render_learning_success(response, icon=":material/edit_note:")
        st.caption(interface_text("완성한 문장을 소리 내어 두 번 읽어 보세요.", "Read the completed sentence aloud twice."))
    if lesson_button(interface_text("2번 문장을 두 번 읽었어요", "I read the Activity 2 sentence twice"),
                     key="unit5_a2_read_aloud", type="primary", disabled=not sentence_ready):
        st.session_state["unit5_a2_read_completed"] = True
        st.session_state["unit5_a2_checked_items"] = (first, second)
    speaking_done = sentence_ready and st.session_state.get("unit5_a2_read_completed", False) and st.session_state.get("unit5_a2_checked_items") == (first, second)
    if speaking_done:
        render_learning_success(interface_text("2번 문장 읽기를 완료했어요.", "You completed reading the Activity 2 sentence."), icon=":material/check_circle:")
    else:
        st.session_state.pop("unit5_a2_read_completed", None)
    if not reading_done:
    ...
    return response if reading_done and speaking_done else ""


def render_unit6_reference(image_name, box_class, box_html, guide_width=500, crop_box=None):
    """Show a responsive reference pair and remember its image for nearby questions."""
    global ACTIVE_REFERENCE_IMAGE
    ...
    with st.container(key=key):
        ...
        with image_column:
            ...
            ACTIVE_REFERENCE_IMAGE = str(image_path)
            reference_image = (
                crop_reference_image_region(image_path, crop_box)
                if crop_box
                else fit_image_to_canvas(image_path, canvas_size=(500, 334), image_size=(500, 334))
            )
            st.image(reference_image, width="stretch")
        with guide_column:
...
def render_unit6_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    for start in range(0, len(vocabulary_words), 5):
        ...
        for column, index in zip(columns, range(start, min(start + 5, len(vocabulary_words)))):
            with column:
                lesson_button(vocabulary_words[index], key=f"vocab_select_6_{index}",
                          type="primary" if index == active_index else "secondary",
...
def render_unit6_grammar1_intro():
    ...
    item = st.selectbox(
        interface_text("교실 물건", "Classroom object"),
        ["책상", "의자", "컴퓨터", "시계"],
        index=None,
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit6_g1_speaking_done", type="primary", disabled=item is None or count is None):
        st.session_state["unit6_grammar1_speaking_completed"] = True
        st.session_state["unit6_g1_checked_choice"] = (item, count)
        queue_grammar1_final_scroll(6)
    speaking_done = item is not None and count is not None and st.session_state.get("unit6_grammar1_speaking_completed", False) and st.session_state.get("unit6_g1_checked_choice") == (item, count)
...
def render_unit6_activity1():
    ...
    fruit = st.selectbox(
        interface_text("과일", "Fruit"), ["사과", "배", "귤", "복숭아"],
        index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit6_a1_fruit",
    )
    quantity = st.selectbox(
        interface_text("수량", "Quantity"), ["한 개", "두 개", "세 개", "네 개", "다섯 개"],
        index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit6_a1_quantity",
    )
    if fruit is not None and quantity is not None:
        render_learning_info(f"주인: 무엇을 드릴까요?\n\n나: {fruit} {quantity} 주세요.", icon=":material/record_voice_over:")
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit6_a1_speaking_done", type="primary", disabled=fruit is None or quantity is None):
        st.session_state["unit6_activity1_speaking_completed"] = True
        st.session_state["unit6_a1_checked_choice"] = (fruit, quantity)
    speaking_done = fruit is not None and quantity is not None and st.session_state.get("unit6_activity1_speaking_completed", False) and st.session_state.get("unit6_a1_checked_choice") == (fruit, quantity)
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
...
def render_unit6_activity2():
    ...
    for index, column in enumerate(cols):
        with column:
            ...
            count = st.selectbox(
                interface_text(f"{index + 1}번째 수량", f"Quantity {index + 1}"),
                ["한 개", "두 개", "세 개", "네 개", "다섯 개"],
                index=None, placeholder=unit6_choose_label, key=f"unit6_a2_count_{index}",
            )
            selected.append((item, count))
    ready = all(item != unit6_choose_label and count is not None for item, count in selected) and len({item for item, _ in selected}) == 3
    response = ""
    if ready:
    ...
    if lesson_button(interface_text("2번 문장을 두 번 읽었어요", "I read the Activity 2 sentence twice"),
                     key="unit6_a2_read_aloud", type="primary", disabled=not ready):
        st.session_state["unit6_a2_read_completed"] = True
        st.session_state["unit6_a2_checked_response"] = response
    speaking_done = ready and st.session_state.get("unit6_a2_read_completed", False) and st.session_state.get("unit6_a2_checked_response") == response
    if speaking_done:
        render_learning_success(interface_text("2번 문장 읽기를 완료했어요.", "You completed reading the Activity 2 sentence."), icon=":material/check_circle:")
    else:
        st.session_state.pop("unit6_a2_read_completed", None)
    if not reading_done:
        st.caption(interface_text("먼저 1번의 두 문제를 모두 맞혀 주세요.", "First, answer both questions in Activity 1 correctly."))
    return response if reading_done and speaking_done else ""


def render_activity2_read_completion(unit_number, response, ready):
    """Use the same read-twice confirmation for later-unit Activity 2 sentences."""
    st.caption(interface_text("완성한 문장을 소리 내어 두 번 읽어 보세요.", "Read the completed sentence aloud twice."))
    if lesson_button(interface_text("2번 문장을 두 번 읽었어요", "I read the Activity 2 sentence twice"),
                     key=f"unit{unit_number}_a2_read_aloud", type="primary", disabled=not ready):
        st.session_state[f"unit{unit_number}_a2_read_completed"] = True
        st.session_state[f"unit{unit_number}_a2_checked_response"] = response
    speaking_done = ready and st.session_state.get(f"unit{unit_number}_a2_read_completed", False) and st.session_state.get(f"unit{unit_number}_a2_checked_response") == response
    if speaking_done:
        render_learning_success(interface_text("2번 문장 읽기를 완료했어요.", "You completed reading the Activity 2 sentence."), icon=":material/check_circle:")
    else:
        st.session_state.pop(f"unit{unit_number}_a2_read_completed", None)
    return speaking_done
...
def render_unit8_grammar1():
    ...
    st.caption(interface_text("질문이 동작을 하는지 또는 상태가 그러한지 묻는지 확인하고, 부정할 때 ‘안’을 어디에 놓는지 살펴보세요.", "Check whether each question asks about an action or condition, then place 안 immediately before the verb or adjective."))
    render_learning_info(
    ...
    first=render_choice_set(8,"grammar1_an",qs, hide_reference_button=True)
    st.divider()
    ...
    action = st.selectbox(
        ...
        index=None, placeholder=interface_text("선택하세요", "Select one"),
        key="unit8_g1_action",
    )
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"),key="unit8_g1_speaking",type="primary",disabled=action is None):
        st.session_state["unit8_g1_speaking_done"] = True
        st.session_state["unit8_g1_checked_action"] = action
        queue_grammar1_final_scroll(8)
    speaking_done=action is not None and st.session_state.get("unit8_g1_speaking_done",False) and st.session_state.get("unit8_g1_checked_action")==action
...
def render_unit8_grammar2():
    ...
    st.caption(interface_text("각 장면의 날씨와 상태를 확인한 뒤, ㅂ이 빠지고 ‘워요’로 바뀐 표현을 선택하세요.", "Identify the condition in each picture, then choose the form in which ㅂ changes to 워요."))
    ...
    irregular_image = Path(__file__).with_name("assets") / "units" / "unit8-b-irregular-exercises.png"
    irregular_images = [
        crop_image_region(irregular_image, (0, 0, 622, 622)),
        crop_image_region(irregular_image, (632, 0, 1254, 622)),
        crop_image_region(irregular_image, (0, 632, 622, 1254)),
        crop_image_region(irregular_image, (632, 632, 1254, 1254)),
    ]
    first=render_picture_word_cards(8, "grammar2_b", qs, irregular_images, single_image_width=180)
    st.divider()
    ...
    state = st.selectbox(
        ...
        state_options[subject] if subject else [],
        index=None, placeholder=interface_text("선택하세요", "Select one"),
        key="unit8_g2_state", disabled=subject is None,
    )
    ...
    if lesson_button(interface_text("2번 대화를 소리 내어 읽었어요", "I read the Activity 2 dialogue aloud"),key="unit8_g2_speaking",type="primary",disabled=subject is None or state is None): st.session_state["unit8_g2_speaking_done"]=True; st.session_state["unit8_g2_checked_choice"]=(subject,state)
    speaking_done=subject is not None and state is not None and st.session_state.get("unit8_g2_speaking_done",False) and st.session_state.get("unit8_g2_checked_choice")== (subject,state)
...
def render_unit8_activity1():
    ...
    st.caption(interface_text("서울과 부산의 그림을 비교하며 두 지역의 날씨가 어떻게 다른지 찾아보세요.", "Compare the Seoul and Busan pictures and identify how their weather differs."))
    ...
    first=render_choice_set(8,"activity1_weather",[("민호 씨는 지금 어디에 있어요?",["서울","부산","제주도"],"서울","그림의 왼쪽은 서울이에요."),("서울의 날씨는 어때요?",["비가 자주 와요.","정말 더워요.","눈이 와요."],"비가 자주 와요.","서울은 비가 오는 장면이에요.")], hide_reference_button=True)
    st.divider()
    ...
    region = st.selectbox(interface_text("지역", "Region"),["서울","부산","하와이","시드니"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit8_a1_city")
    weather = st.selectbox(interface_text("날씨", "Weather"),["비가 자주 와요","더워요","안 추워요","쌀쌀해요","추워요"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit8_a1_weather")
    if region is not None and weather is not None:
        region_topic = "은" if subject_particle(region) == "이" else "는"
        render_learning_info(
            f"가: {region}{region_topic} 날씨가 어때요?\n\n나: {region}{region_topic} :orange[**{weather}**].",
            icon=":material/forum:",
        )
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"),key="unit8_a1_speaking",type="primary",disabled=region is None or weather is None): st.session_state["unit8_a1_speaking_done"]=True; st.session_state["unit8_a1_checked_choice"]=(region,weather)
    speaking_done=region is not None and weather is not None and st.session_state.get("unit8_a1_speaking_done",False) and st.session_state.get("unit8_a1_checked_choice")== (region,weather)
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
        st.session_state.pop("unit8_a1_speaking_done", None)
...
def render_unit8_activity2():
    ...
    st.caption(interface_text("그림을 봄부터 겨울까지 차례로 보며 계절마다 달라지는 날씨 표현을 확인하세요.", "Read the pictures from spring through winter and notice how the weather expression changes by season."))
    render_unit6_reference(
    ...
    first=render_choice_set(8,"activity2_jeju",[("제주도의 봄은 어때요?",["따뜻해요","추워요","더워요"],"따뜻해요","봄은 따뜻해요."),("제주도의 여름은 어때요?",["더워요","쌀쌀해요","추워요"],"더워요","여름은 더워요."),("제주도의 가을은 어때요?",["시원해요","더워요","추워요"],"시원해요","가을은 시원해요."),("제주도의 겨울은 어때요?",["추워요","시원해요","따뜻해요"],"추워요","겨울은 추워요.")], hide_reference_button=True)
    st.divider()
    ...
    speaking_done = render_activity2_read_completion(8, response, bool(response))
    return response if first and speaking_done else ""
...
def render_unit8_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    st.caption(interface_text("그림에서 확인한 표현을 하나씩 눌러 예문과 함께 익혀 보세요.", "Select each expression from the picture and study it with its example sentence."))
    for start in range(0,len(vocabulary_words),5):
        cols=st.columns(min(5,len(vocabulary_words)-start))
        for col,index in zip(cols,range(start,min(start+5,len(vocabulary_words)))):
            with col: lesson_button(vocabulary_words[index],key=f"vocab_select_8_{index}",type="primary" if index==active_index else "secondary",on_click=select_vocabulary,args=(8,index),width="stretch")
    render_vocabulary_example(example_sentence,color="lime")
    ...
    st.caption(interface_text("봄·여름·가을·겨울 그림을 보고 각 계절에 가장 알맞은 날씨 표현을 선택하세요.", "Look at the spring, summer, autumn, and winter pictures and choose the best weather expression for each season."))
    ...
    season_image = Path(__file__).with_name("assets") / "units" / "unit8-weather-seasons.png"
    season_images = [
        crop_image_region(season_image, (15, 18, 588, 505)),
        crop_image_region(season_image, (603, 18, 1173, 505)),
        crop_image_region(season_image, (15, 521, 588, 1004)),
        crop_image_region(season_image, (603, 521, 1173, 1004)),
    ]
    second=render_picture_word_cards(8, "vocab_season", seasons, season_images, single_image_width=180)
    ...
    third=render_choice_set(8,"vocab_weather_dialogue",[("가: 일기예보에 이번 주말은 해가 나고 구름이 없대요. 날씨가 어때요?\n나: 날씨가 ___. 따뜻해요.",["맑아요","추워요","눈이 와요"],"맑아요","해가 나고 구름이 없으므로 맑아요."),("가: 겨울 날씨가 어때요?\n나: 아주 ___.",["추워요","더워요","시원해요"],"추워요","겨울은 추워요."),("가: 비가 와요?\n나: 아니요. 날씨가 ___.",["맑아요","비가 와요","눈이 와요"],"맑아요","비가 오지 않고 맑은 날씨예요.")],dialogue_layout=True, hide_reference_button=True)
...
def render_unit9_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    for start in range(0, len(vocabulary_words), 5):
        ...
        for column, index in zip(columns, range(start, min(start + 5, len(vocabulary_words)))):
            with column:
                lesson_button(vocabulary_words[index], key=f"vocab_select_9_{index}", type="primary" if index == active_index else "secondary", on_click=select_vocabulary, args=(9, index), width="stretch")
    ...
    st.caption(interface_text("각 문제의 실제 장면을 보고 장소와 그곳에서 한 활동을 연결하세요.", "Use each scene to connect the place with the activity completed there."))
    place_questions = [
    ...
    weekend_image = Path(__file__).with_name("assets") / "units" / "unit9-weekend-activities.png"
    place_images = [
        crop_image_region(weekend_image, (0, 0, 509, 508)),
        crop_image_region(weekend_image, (516, 0, 1023, 508)),
        crop_image_region(weekend_image, (1029, 0, 1536, 508)),
        crop_image_region(weekend_image, (1029, 515, 1536, 1024)),
    ]
    second = render_picture_word_cards(9, "vocab_place_action", place_questions, place_images, single_image_width=180)
    st.divider()
    ...
    ], dialogue_layout=True, hide_reference_button=True)
    return second and third
...
def render_unit9_grammar1():
    ...
    render_unit6_reference(
        "unit9-stage-scenes-v2.png", "unit9-grammar1-guide",
    ...
    with choice_column:
        place = st.selectbox("① 장소를 선택하세요", list(UNIT9_PLACE_ACTIONS), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_g1_place")
        action = st.selectbox("② 활동을 선택하세요", UNIT9_PLACE_ACTIONS[place] if place else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_g1_action", disabled=place is None)
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit9_g1_speaking", type="primary", disabled=place is None or action is None):
        st.session_state["unit9_g1_speaking_done"] = True
        st.session_state["unit9_g1_checked_choice"] = (place, action)
        queue_grammar1_final_scroll(9)
    speaking_done = place is not None and action is not None and st.session_state.get("unit9_g1_speaking_done", False) and st.session_state.get("unit9_g1_checked_choice") == (place, action)
    if not speaking_done:
...
def render_unit9_grammar2():
    ...
    render_unit6_reference(
        "unit9-stage-scenes-v2.png", "unit9-grammar2-guide",
    ...
    past_images = [
        ...
        crop_image_region(weekend_image, (516, 0, 1023, 508)),
        crop_image_region(weekend_image, (1029, 0, 1536, 508)),
        crop_image_region(weekend_image, (0, 515, 509, 1024)),
    ]
    first = render_picture_word_cards(9, "grammar2_past", past_questions, past_images, single_image_width=180)
    st.divider()
    ...
    with selector_column:
        present = st.selectbox(interface_text("바꿀 현재 표현", "Present expression"), list(past_forms), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_g2_present")
    ...
    if lesson_button(interface_text("2번 문장을 두 번 읽었어요", "I read the Activity 2 sentences aloud twice"), key="unit9_g2_speaking", type="primary", disabled=present is None):
        st.session_state["unit9_g2_speaking_done"] = True
        st.session_state["unit9_g2_checked_present"] = present
    speaking_done = present is not None and st.session_state.get("unit9_g2_speaking_done", False) and st.session_state.get("unit9_g2_checked_present") == present
    if not speaking_done:
...
def render_unit9_activity1():
    ...
    render_unit6_reference(
        "unit9-stage-scenes-v2.png", "unit9-activity1-guide",
    ...
    with choice_column:
        place = st.selectbox("① 내가 간 장소", list(UNIT9_PLACE_ACTIONS), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a1_place")
        action = st.selectbox("② 내가 한 활동", UNIT9_PLACE_ACTIONS[place] if place else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a1_action", disabled=place is None)
    if place is not None and action is not None:
      with memory_column:
        with st.container(border=True, height=184):
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"), key="unit9_a1_speaking", type="primary", disabled=place is None or action is None):
        st.session_state["unit9_a1_speaking_done"] = True
        st.session_state["unit9_a1_checked_choice"] = (place, action)
    speaking_done = place is not None and action is not None and st.session_state.get("unit9_a1_speaking_done", False) and st.session_state.get("unit9_a1_checked_choice") == (place, action)
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
...
def render_unit9_activity2():
    ...
    day = st.selectbox(interface_text("요일", "Weekday"), ["토요일", "일요일"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_day")
    row1, row2 = st.columns(2, gap="medium")
    with row1:
        time1 = st.selectbox(interface_text("첫 번째 시간", "First time"), ["오전", "오후", "저녁"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_time1")
        place1 = st.selectbox(interface_text("첫 번째 장소", "First place"), list(UNIT9_PLACE_ACTIONS), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_place1")
        action1 = st.selectbox(interface_text("첫 번째 활동", "First activity"), UNIT9_PLACE_ACTIONS[place1] if place1 else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_action1", disabled=place1 is None)
    with row2:
        time2 = st.selectbox(interface_text("두 번째 시간", "Second time"), ["오후", "저녁", "오전"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_time2")
        place2 = st.selectbox(interface_text("두 번째 장소", "Second place"), list(UNIT9_PLACE_ACTIONS), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_place2")
        action2 = st.selectbox(interface_text("두 번째 활동", "Second activity"), UNIT9_PLACE_ACTIONS[place2] if place2 else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_action2", disabled=place2 is None)
    feeling = st.selectbox(interface_text("마지막 느낌", "Final feeling"), ["정말 즐거웠어요", "아주 재미있었어요", "조금 피곤했어요", "기분이 좋았어요"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit9_a2_feeling")
    details_ready = all(value is not None for value in (day, time1, place1, action1, time2, place2, action2, feeling))
    place_symbols = {"공원": "🌳", "도서관": "📚", "식당": "🍚", "집": "🏠", "헬스장": "🏋️", "백화점": "🛍️"}
    action_symbols = {"산책했어요": "🚶", "운동했어요": "🏃", "공부했어요": "✏️", "책을 읽었어요": "📖", "밥을 먹었어요": "🥄", "영화를 봤어요": "🎬", "쉬었어요": "☕", "쇼핑했어요": "🛒"}
    st.markdown("#### 나의 주말 타임라인")
    timeline_columns = st.columns([1, 0.18, 1, 0.18, 1], gap="small", vertical_alignment="center")
    timeline_items = [
        (timeline_columns[0], "📅", day or interface_text("선택하세요", "Select one"), "어제"),
        (timeline_columns[2], place_symbols.get(place1, "📍"), f"{time1 or '…'} · {place1 or '…'}", action_symbols.get(action1, "") + " " + (action1 or "")),
        (timeline_columns[4], place_symbols.get(place2, "📍"), f"{time2 or '…'} · {place2 or '…'}", action_symbols.get(action2, "") + " " + (action2 or "")),
    ]
    ...
    st.markdown(
        ...
        f"<b style='color:#f2a65a'>{feeling or interface_text('선택하세요', 'Select one')}</b></div>",
        unsafe_allow_html=True,
    )
    response = f"어제는 {day}이었어요.\n{time1}에는 {place1}에서 {action1}.\n{time2}에는 {place2}에서 {action2}.\n{feeling}." if details_ready else ""
    if response:
        render_learning_success(response, icon=":material/edit_note:")
    if details_ready and (place1 == place2 or time1 == time2):
    ...
    speaking_done = render_activity2_read_completion(9, response, details_ready)
    return response if first and speaking_done else ""
...
def render_unit10_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    for start in range(0, len(vocabulary_words), 5):
        ...
        for column, index in zip(columns, range(start, min(start + 5, len(vocabulary_words)))):
            with column:
                lesson_button(vocabulary_words[index], key=f"vocab_select_10_{index}", type="primary" if index == active_index else "secondary", on_click=select_vocabulary, args=(10, index), width="stretch")
    render_learning_info(f"**예문:** :green[**{example_sentence}**]", icon=":material/menu_book:")
    st.divider()
    st.markdown(interface_text("### 2. 장소 그림에 알맞은 목적을 선택해 보세요.", "### 2. Look at the place pictures and choose the correct purpose."))
    st.caption(interface_text("각 장소 장면을 보고 그곳에 가는 목적을 선택하세요.", "Look at each place scene and choose the purpose for going there."))
    purpose_questions = [
    ...
    plan_image = Path(__file__).with_name("assets") / "units" / "unit10-weekend-plans.png"
    purpose_images = [
        crop_image_region(plan_image, (522, 10, 1015, 506)),
        crop_image_region(plan_image, (1025, 10, 1527, 506)),
        crop_image_region(plan_image, (9, 517, 513, 1014)),
        crop_image_region(plan_image, (1025, 517, 1527, 1014)),
    ]
    second = render_picture_word_cards(10, "vocab_purpose", purpose_questions, purpose_images, single_image_width=180)
    st.divider()
...
def render_unit10_grammar1():
    ...
    render_unit6_reference(
        "unit10-stage-scenes-v2.png", "unit10-grammar1-guide",
    ...
    first = render_picture_word_cards(10, "grammar1_suggestion", suggestion_questions, suggestion_images, single_image_width=180)
    st.divider()
    ...
    with left:
        day = st.selectbox("① 약속 요일", ["토요일", "일요일"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_g1_day")
        base = st.selectbox("② 함께할 일", list(suggestion_forms), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_g1_base")
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read dialogue 2 aloud twice"), key="unit10_g1_speaking", type="primary", disabled=day is None or base is None):
        st.session_state["unit10_g1_speaking_done"] = True
        st.session_state["unit10_g1_checked_choice"] = (day, base)
        queue_grammar1_final_scroll(10)
    speaking_done = day is not None and base is not None and st.session_state.get("unit10_g1_speaking_done", False) and st.session_state.get("unit10_g1_checked_choice") == (day, base)
    if not speaking_done:
        st.session_state.pop("unit10_g1_speaking_done", None)
    if first and speaking_done:
        st.session_state["unit10_grammar1_activities_completed"] = True
    else:
        st.session_state.pop("unit10_grammar1_activities_completed", None)
...
def render_unit10_grammar2():
    ...
    render_unit6_reference(
        "unit10-stage-scenes-v2.png", "unit10-grammar2-guide",
    ...
    first = render_picture_word_cards(10, "grammar2_purpose", purpose_form_questions, purpose_form_images, single_image_width=180)
    st.divider()
    ...
    with left:
        destination = st.selectbox("① 목적지", list(UNIT10_DESTINATION_PURPOSES), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_g2_destination")
        purpose = st.selectbox("② 목적", UNIT10_DESTINATION_PURPOSES[destination] if destination else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_g2_purpose", disabled=destination is None)
    if destination is not None and purpose is not None:
        with right:
            with st.container(border=True, height=174):
    ...
    if lesson_button(interface_text("2번 문장을 두 번 읽었어요", "I read sentence 2 aloud twice"), key="unit10_g2_speaking", type="primary", disabled=destination is None or purpose is None):
        st.session_state["unit10_g2_speaking_done"] = True
        st.session_state["unit10_g2_checked_choice"] = (destination, purpose)
    speaking_done = destination is not None and purpose is not None and st.session_state.get("unit10_g2_speaking_done", False) and st.session_state.get("unit10_g2_checked_choice") == (destination, purpose)
    if not speaking_done:
        st.session_state.pop("unit10_g2_speaking_done", None)
    if first and speaking_done:
        st.session_state["grammar2_done_10"] = True
    else:
        st.session_state.pop("grammar2_done_10", None)
...
def render_unit10_activity1():
    ...
    render_unit6_reference(
        "unit10-stage-scenes-v2.png",
        "unit10-activity1-guide",
        "<p><b>수진:</b> 민호 씨, <b style='color:#b7ef58'>토요일에</b> 같이 영화를 <b style='color:#ff9b72'>볼까요?</b></p>"
        "<p><b>민호:</b> 네, 좋아요. <b style='color:#78aef8'>몇 시에</b> <b style='color:#ff9b72'>만날까요?</b></p>"
        "<p><b>수진:</b> <b style='color:#78aef8'>두 시에</b> 만나요.</p>"
        "<p><b>민호:</b> <b style='color:#78aef8'>어디에서</b> <b style='color:#ff9b72'>만날까요?</b></p>"
        "<p><b>수진:</b> <b style='color:#b7ef58'>영화관 앞에서</b> 만나요.</p>",
        crop_box=(771, 0, 1536, 508),
    )
    ...
    with c1: day = st.selectbox("요일", ["토요일", "일요일"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a1_day")
    with c2: time = st.selectbox("시간", ["한 시", "두 시", "세 시", "네 시"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a1_time")
    with c3: plan = st.selectbox("함께할 일", ["영화를 볼까요?", "점심을 먹을까요?", "공원에서 산책할까요?", "놀이공원에 갈까요?"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a1_plan")
    ...
    if day is not None and time is not None and plan is not None:
        if reaction == "네, 좋아요!":
    ...
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read dialogue 2 aloud twice"), key="unit10_a1_speaking", type="primary", disabled=day is None or time is None or plan is None):
        st.session_state["unit10_a1_speaking_done"] = True
        st.session_state["unit10_a1_checked_choice"] = (day, time, plan, reaction)
    speaking_done = day is not None and time is not None and plan is not None and st.session_state.get("unit10_a1_speaking_done", False) and st.session_state.get("unit10_a1_checked_choice") == (day, time, plan, reaction)
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
        st.session_state.pop("unit10_a1_speaking_done", None)
    if first and speaking_done:
        st.session_state["activity1_completed_10"] = True
    else:
        st.session_state.pop("activity1_completed_10", None)
...
def render_unit10_activity2():
    ...
    render_unit6_reference(
        "unit10-stage-scenes-v2.png",
        "unit10-activity2-guide",
        "<p><b style='color:#b7ef58'>언제:</b> 토요일</p>"
        "<p><b style='color:#78aef8'>몇 시:</b> 두 시</p>"
        "<p><b style='color:#b7ef58'>어디:</b> 영화관 앞</p>"
        "<p><b style='color:#ff9b72'>왜:</b> 영화를 보러 가요.</p>"
        "<p style='margin-top:16px'>네 가지 정보를 확인하고 주말 약속 카드를 완성해요.</p>",
        crop_box=(771, 516, 1536, 1024),
    )
    ...
    first = render_choice_set(10, "activity2_structure", [("‘왜 가요?’에 알맞은 대답은 무엇이에요?", ["영화를 보러 가요", "토요일에 가요", "두 시에 가요"], "영화를 보러 가요", "‘왜’에는 목적을 대답해요."), ("약속을 제안하는 문장은 무엇이에요?", ["같이 갈까요?", "어제 갔어요.", "날씨가 좋아요."], "같이 갈까요?", "‘-(으)ㄹ까요?’로 함께할 일을 제안해요.")], hide_reference_button=True)
    st.markdown(interface_text("### 2. 정보를 선택해 나의 약속 카드를 완성해 보세요.", "### 2. Choose the details to complete your weekend-plan card."))
    left, right = st.columns(2, gap="medium")
    with left:
        day = st.selectbox("요일", ["토요일", "일요일"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a2_day")
        time = st.selectbox("시간", ["한 시", "두 시", "세 시", "네 시", "다섯 시"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a2_time")
    with right:
        place = st.selectbox("장소", list(UNIT10_DESTINATION_PURPOSES), index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a2_place")
        purpose = st.selectbox("목적", UNIT10_DESTINATION_PURPOSES[place] if place else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a2_purpose", disabled=place is None)
    meeting_point = st.selectbox("만나는 위치", [f"{place} 앞", f"{place} 입구"] if place else [], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit10_a2_meeting_point", disabled=place is None)
    symbols = {"놀이공원": "🎡", "영화관": "🎬", "식당": "🍚", "공원": "🌳", "카페": "☕", "도서관": "📚"}
    details_ready = all(value is not None for value in (day, time, place, purpose, meeting_point))
    response = ""
    if details_ready:
        with st.container(border=True):
    ...
    speaking_done = render_activity2_read_completion(10, response, details_ready)
    return response if first and speaking_done else ""
...
def render_unit7_vocabulary_panel(vocabulary_words, active_index, example_sentence):
    ...
    render_unit6_reference(
        "unit7-calendar.png",
        "unit7-vocab-guide",
        "<p><b>요일:</b> 월요일, 화요일, 수요일, 목요일, 금요일, 토요일, 일요일</p>"
        "<p><b>날짜:</b> 월·일은 한자어 수로 읽어요. 3월 5일 → 삼월 오일</p>"
        "<p><b>특별한 읽기:</b> 6월 → 유월, 10월 → 시월</p>"
        "<p><b>시간:</b> 시는 고유어 수, 분은 한자어 수로 읽어요.</p>"
        "<p><b>예:</b> 7:30 → 일곱 시 삼십 분 / 일곱 시 반</p>",
    )
    for start in range(0, len(vocabulary_words), 5):
        columns = st.columns(min(5, len(vocabulary_words)-start))
        for col, index in zip(columns, range(start, min(start+5, len(vocabulary_words)))):
            with col:
                lesson_button(vocabulary_words[index], key=f"vocab_select_7_{index}", type="primary" if index == active_index else "secondary", on_click=select_vocabulary, args=(7,index), width="stretch")
    render_vocabulary_example(example_sentence, color="lime")
    st.markdown(
        "<div class='grammar-feedback'><b>시간 읽기 한눈에 보기</b><br>"
        "1시 한 시 · 2시 두 시 · 3시 세 시 · 4시 네 시<br>"
        "10분 십 분 · 20분 이십 분 · 30분 삼십 분(반)</div>",
        unsafe_allow_html=True,
    )
    ...
    q3 = [
        ...
        ("가: 민호 씨, 생일이 언제예요?\n나: ___이에요.", ["8월 12일", "1월 16일", "6월 22일"], "8월 12일", "표에서 민호의 생일을 찾아요."),
    ]
    ...
    st.divider()
    st.markdown(interface_text("### 4. 디지털시계를 한국어로 읽어 보세요.", "### 4. Read each digital clock in Korean"))
    st.caption(interface_text("‘시’ 앞에는 고유어 수, ‘분’ 앞에는 한자어 수를 사용해요.", "Use native Korean numbers before 시 and Sino-Korean numbers before 분."))
    clock_cards = st.columns(3)
    for column, clock in zip(clock_cards, ("🕘 9:00", "🕝 2:30", "🕖 7:10")):
        with column:
            st.markdown(f"<div style='padding:18px;text-align:center;border:1px solid #334155;border-radius:12px;font-size:1.35rem;font-weight:850'>{clock}</div>", unsafe_allow_html=True)
    clock_done = render_choice_set(7, "vocab_clock", [
        ("9:00", ["아홉 시", "구 시", "아홉 분"], "아홉 시", "‘시’ 앞에는 고유어 수 ‘아홉’을 사용해요."),
        ("2:30", ["두 시 삼십 분", "이 시 서른 분", "둘 시 삼십 분"], "두 시 삼십 분", "‘둘’은 시 앞에서 ‘두’가 되고, 30분은 ‘삼십 분’이에요."),
        ("7:10", ["일곱 시 십 분", "칠 시 열 분", "일곱 분 십 시"], "일곱 시 십 분", "시는 고유어 수, 분은 한자어 수로 읽어요."),
    ], hide_reference_button=True)
    return done2 and dialogue_done and clock_done


def render_unit7_grammar1():
    render_learning_info(interface_text("시간·요일·날짜 뒤에 ‘에’를 붙여 언제 하는 일인지 말해요. 하지만 오늘·내일·어제 뒤에는 보통 ‘에’를 붙이지 않아요.", "Add 에 after a time, weekday, or date. Usually, do not add 에 after 오늘, 내일, or 어제."), icon=":material/school:")
    render_unit6_reference("unit7-calendar.png","unit7-grammar1-guide","<p><b>형태:</b> <b style='color:#b7ef58'>시간/요일/날짜 + 에</b></p><p>수업은 일곱 시<b>에</b> 시작해요.</p><p>수요일<b>에</b> 한국어 수업이 있어요.</p><p><b>주의:</b> 오늘·내일·어제에는 보통 ‘에’를 쓰지 않아요.<br>내일 친구를 만나요. (○)</p>")
    st.markdown(interface_text("### 1. 언제 ‘에’를 사용할까요?", "### 1. When should you use 에?"))
    st.caption(interface_text("모든 문장에 ‘에’를 넣지 말고 시간 표현을 잘 살펴보세요.", "Look carefully at the time expression; not every sentence needs 에."))
    qs=[("수업은 일곱 시__ 시작해요.",["에","에서","을"],"에","구체적인 시각 뒤에는 ‘에’를 사용해요."),("수요일__ 친구를 만나요.",["에","에서","를"],"에","요일 뒤에도 ‘에’를 사용해요."),("3월 5일__ 생일 파티를 해요.",["에","은","을"],"에","날짜 뒤에는 ‘에’를 사용해요."),("내일__ 영화를 봐요.",["(없음)","에","에서"],"(없음)","‘오늘·내일·어제’ 뒤에는 보통 ‘에’를 붙이지 않아요.")]
    ...
    day=st.selectbox(interface_text("요일", "Weekday"),["월요일","화요일","수요일","목요일","금요일"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_g1_day"); hour=st.selectbox(interface_text("시간", "Time"),["오전 아홉 시","오전 열 시","오후 세 시","저녁 일곱 시"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_g1_hour")
    if day is not None and hour is not None:
        render_learning_info(f"가: 언제 만나요?\n\n나: {day} {hour}에 만나요.\n\n가: 좋아요. 그때 만나요!",icon=":material/forum:")
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"),key="unit7_g1_speaking",type="primary",disabled=day is None or hour is None):
        st.session_state["unit7_g1_speaking_done"] = True
        st.session_state["unit7_g1_checked_choice"] = (day, hour)
        queue_grammar1_final_scroll(7)
    speaking_done = day is not None and hour is not None and st.session_state.get("unit7_g1_speaking_done",False) and st.session_state.get("unit7_g1_checked_choice")== (day,hour)
...
def render_unit7_grammar2():
    render_learning_info(interface_text("현재 시각은 ‘지금 몇 시예요?’, 일정 시각은 ‘몇 시에 시작해요?’라고 물어요.", "Ask 지금 몇 시예요? for the current time and 몇 시에 시작해요? for the time of an event."),icon=":material/school:")
    render_unit6_reference("unit7-time-lunch.png","unit7-grammar2-guide","<p><b>현재 시각:</b> 지금 몇 시예요?<br>일곱 시 삼십 분이에요. (= 일곱 시 반이에요.)</p><p><b>일정 시각:</b> 수업은 몇 시에 시작해요?<br>아홉 시에 시작해요.</p><p><b>기억:</b> 몇 시<b>예요</b>? / 몇 시<b>에</b> 시작해요?</p>")
    ...
    a=render_choice_set(7,"grammar2_clock",[("지금 몇 ___예요?",["시","명","월"],"시","현재 시각을 물을 때는 ‘몇 시예요?’라고 해요."),("일곱 시 삼십 ___이에요.",["분","일","월"],"분","시각의 분을 말할 때 ‘분’을 사용해요."),("2:30은 어떻게 읽어요?",["두 시 삼십 분","이 시 서른 분","둘 시 반 분"],"두 시 삼십 분","‘시’는 고유어 수, ‘분’은 한자어 수로 읽어요."),("수업 시작 시간을 물어보세요.",["몇 시에 시작해요?","몇 시예요?","몇 개 시작해요?"],"몇 시에 시작해요?","일정의 시각을 물을 때는 ‘몇 시에’를 사용해요.")], hide_reference_button=True)
    ...
    event_questions={"한국어 수업":"한국어 수업이 있어요","친구 만나기":"친구를 만나요","점심 먹기":"점심을 먹어요","운동하기":"운동해요"}
    event=st.selectbox(interface_text("활동", "Activity"),list(event_sentences),index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_g2_event"); t=st.selectbox(interface_text("시간", "Time"),["오전 아홉 시","오후 열두 시 반","오후 세 시","저녁 일곱 시 십 분"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_g2_time")
    if event is not None and t is not None:
        render_learning_info(f"가: 몇 시에 {event_questions[event]}?\n\n나: {t}에 {event_sentences[event]}",icon=":material/forum:")
    if lesson_button(interface_text("2번 문장을 소리 내어 읽었어요", "I read the Activity 2 sentence aloud"),key="unit7_g2_speaking",type="primary",disabled=event is None or t is None): st.session_state["unit7_g2_speaking_done"]=True; st.session_state["unit7_g2_checked_choice"]=(event,t)
    speaking_done = event is not None and t is not None and st.session_state.get("unit7_g2_speaking_done",False) and st.session_state.get("unit7_g2_checked_choice")== (event,t)
...
def render_unit7_activity1():
    ...
    st.caption(interface_text("그림과 대화를 함께 보고 일정 카드에서 요일과 시간을 정확히 확인하세요.", "Use the picture, dialogue, and schedule card to confirm the exact day and time."))
    st.markdown(
        """
        <style>
        div.st-key-unit7_activity1_reference_pair { width:1016px; max-width:100%; margin-top:-16px; }
        div.st-key-unit7_activity1_reference_pair [data-testid="stHorizontalBlock"] {
            display:grid; grid-template-columns:500px 500px;
            gap:16px !important; align-items:start; justify-content:start;
        }
        div.st-key-unit7_activity1_reference_pair [data-testid="stColumn"] {
            width:500px !important; min-width:500px !important;
        }
        div.st-key-unit7_activity1_reference_pair [data-testid="stImage"],
        div.st-key-unit7_activity1_reference_pair [data-testid="stImage"] img {
            width:500px !important; height:334px !important; object-fit:cover;
            display:block; border-radius:10px;
        }
        .unit7-activity1-dialogue-box {
            width:500px; height:334px; box-sizing:border-box; display:flex;
            flex-direction:column; justify-content:center; padding:26px;
            border:1px solid #334155; border-radius:10px; background:#172033;
            color:#dbe7f5; line-height:1.65;
        }
        .unit7-activity1-dialogue-box p { margin:6px 0; }
        @media (max-width:760px) {
            div.st-key-unit7_activity1_reference_pair { width:100%; }
            div.st-key-unit7_activity1_reference_pair [data-testid="stHorizontalBlock"] {
                display:grid; grid-template-columns:minmax(0, 1fr); gap:12px !important;
            }
            div.st-key-unit7_activity1_reference_pair [data-testid="stColumn"] {
            ...
            .unit7-activity1-dialogue-box {
                width:100%; height:auto; min-height:0; aspect-ratio:500/334; padding:18px;
            }
        }
        </style>
        """,
    ...
    with st.container(key="unit7_activity1_reference_pair"):
        activity_picture, activity_question = st.columns(2, gap=None, vertical_alignment="top")
        ...
        with activity_question:
            st.markdown(
                """
                <div class="unit7-activity1-dialogue-box">
                    <p><b>재민:</b> 안나 씨, 언제 세종학당에 가요?</p>
                    <p><b>안나:</b> 목요일에 가요.</p>
                    <p><b>재민:</b> 수업은 몇 시에 시작해요?</p>
                    <p><b>안나:</b> 저녁 일곱 시에 시작해요.</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
            st.space("small")
            done=render_choice_set(7,"activity1_schedule",[("안나 씨는 언제 세종학당에 가요?",["목요일","금요일","일요일"],"목요일","일정 카드에서 요일을 확인해요."),("수업은 몇 시에 시작해요?",["저녁 일곱 시","아침 아홉 시","열두 시"],"저녁 일곱 시","일정 카드와 대화에서 시간을 확인해요."),("맞는 문장을 고르세요.",["목요일 저녁 일곱 시에 수업이 있어요.","목요일에 일곱 분 수업이 있어요.","오늘에 수업이 있어요."],"목요일 저녁 일곱 시에 수업이 있어요.","요일과 시각 뒤에 ‘에’를 붙여 일정을 말해요.")], hide_reference_button=True)
    st.markdown(interface_text("### 2. 나의 수업 약속을 만들어 말해 보세요.", "### 2. Make and say your own class appointment"))
    day=st.selectbox(interface_text("요일", "Weekday"),["월요일","수요일","목요일","금요일"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_a1_day")
    tm=st.selectbox(interface_text("시간", "Time"),["오전 아홉 시","오후 세 시","저녁 일곱 시"],index=None,placeholder=interface_text("선택하세요", "Select one"),key="unit7_a1_time")
    if day is not None and tm is not None:
        render_learning_info(f"가: 한국어 수업은 언제 해요?\n\n나: {day} {tm}에 해요.\n\n가: 좋아요. 수업에서 만나요!",icon=":material/record_voice_over:")
    if lesson_button(interface_text("2번 대화를 두 번 읽었어요", "I read the Activity 2 dialogue aloud twice"),key="unit7_a1_speaking",type="primary",disabled=day is None or tm is None): st.session_state["unit7_a1_speaking_done"]=True; st.session_state["unit7_a1_checked_choice"]=(day,tm)
    speaking_done=day is not None and tm is not None and st.session_state.get("unit7_a1_speaking_done",False) and st.session_state.get("unit7_a1_checked_choice")== (day,tm)
    if speaking_done:
        render_learning_success(interface_text("2번 대화 읽기를 완료했어요.", "You completed the Activity 2 dialogue reading."), icon=":material/check_circle:")
    else:
        st.session_state.pop("unit7_a1_speaking_done", None)
...
def render_unit7_activity2():
    ...
    st.caption(interface_text("네 장면을 순서대로 보며 정확한 시간과 활동을 짝지으세요.", "Read the four scenes in order and match each activity with its exact time."))
    render_unit6_reference(
        "unit7-daily-scenes-v2.png",
        "unit7-activity2-guide",
        "<p><b style='color:#78aef8'>① 여섯 시</b>　일어나요.</p>"
        "<p><b style='color:#78aef8'>② 일곱 시</b>　아침을 먹어요.</p>"
        "<p><b style='color:#78aef8'>③ 열 시</b>　회의를 해요.</p>"
        "<p><b style='color:#78aef8'>④ 열두 시</b>　점심을 먹어요.</p>",
    )
    ...
    st.divider()
    st.markdown(interface_text("### 2. 시간 순서대로 나의 하루를 완성해 보세요.", "### 2. Complete your day in chronological order"))
    st.caption(interface_text("서로 다른 시간과 활동을 선택하세요. 아침부터 저녁까지 순서대로 선택하면 나의 하루 카드가 완성돼요.", "Choose different times and activities, in order from morning to evening.")); rows=[]
    time_options=[interface_text("선택하세요", "Choose"),"아침 일곱 시","오전 아홉 시","점심 열두 시","오후 세 시","저녁 일곱 시"]
    activity_options=[interface_text("선택하세요", "Choose"),"아침을 먹어요","한국어를 공부해요","친구를 만나요","운동해요","저녁을 먹어요"]
    for i in range(3):
        c=st.columns(2); 
        with c[0]: tm=st.selectbox(interface_text(f"{i+1}번째 시간", f"Time {i+1}"),time_options,key=f"unit7_a2_time_{i}")
        with c[1]: act=st.selectbox(interface_text(f"{i+1}번째 활동", f"Activity {i+1}"),activity_options,key=f"unit7_a2_act_{i}")
        rows.append((tm,act))
    choose_label=interface_text("선택하세요", "Choose")
    filled=all(t!=choose_label and a!=choose_label for t,a in rows)
    unique=filled and len({t for t,_ in rows})==3 and len({a for _,a in rows})==3
    time_order={value:index for index,value in enumerate(time_options)}
    ordered=unique and [time_order[t] for t,_ in rows]==sorted(time_order[t] for t,_ in rows)
    ready=filled and unique and ordered
    response="" if not ready else "\n\n".join(f"{index+1}. {t}에 {a}." for index,(t,a) in enumerate(rows))
    if not filled:
        st.caption(interface_text("세 시간과 활동을 모두 선택해 주세요.", "Choose all three times and activities."))
    elif not unique:
        render_learning_warning(interface_text("같은 시간이나 활동이 있어요. 서로 다르게 선택해 주세요.", "A time or activity is repeated. Choose different ones."))
    elif not ordered:
        render_learning_warning(interface_text("시간이 바뀌었어요. 이른 시간부터 차례대로 놓아 주세요.", "Put the times in order from earliest to latest."))
    if response:
        with st.container(border=True):
            st.markdown(interface_text("#### 🗓️ 나의 하루 시간표", "#### 🗓️ My Daily Schedule"))
            st.markdown(response)
    speaking_done = render_activity2_read_completion(7, response, ready)
    return response if reading and speaking_done else ""
...
def select_vocabulary(unit_number, index):
    ...
    # Vocabulary completion also requires the unit's exercises and reading.
    # The dashboard validates those requirements before recording completion.
...
def render_unit1_picture_dialogue(all_vocabulary_read=False):
    ...
    # Keep each card's answer state separate. Reusing one key caused a
    # previous card's feedback to block the next card after a reload.
    result_key = f"unit1_picture_card_correct_{card_index}"
    checked_choice_key = f"unit1_picture_card_checked_choice_{card_index}"
    feedback_key = f"unit1_picture_card_feedback_visible_{card_index}"
    current_choice_key = f"unit1_picture_choice_{card_index}"
    current_card_completed = (
        card_index < 3
        and st.session_state.get(result_key) is True
        and st.session_state.get(checked_choice_key) == st.session_state.get(current_choice_key)
    )
    completed_cards = (
        len(cards)
        if st.session_state.get("unit1_picture_dialogue_done", False)
        else card_index + int(current_card_completed)
    )
    ...
    st.progress(
        ...
        text=f"{interface_text('대화 완료', 'Dialogues complete')} ({completed_cards}/{len(cards)})",
    )
        ...
        if card_index < 3:
            ...
            with st.container(horizontal=True, gap="small"):
                ...
                if feedback_visible and st.session_state.get(result_key) is True:
                    if lesson_button(interface_text("다음 인물 →", "Next person →"), key=f"unit1_picture_next_{card_index}"):
            ...
            mystery_country = st.selectbox(interface_text("나라 선택", "Choose a country"), ["캐나다", "베트남", "미국", "프랑스", "태국", "인도네시아", "중국", "일본", "러시아", "케냐"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit1_mystery_country")
            mystery_job = st.selectbox(interface_text("직업 선택", "Choose an occupation"), ["회사원", "대학생", "의사", "경찰", "선생님", "가수", "요리사"], index=None, placeholder=interface_text("선택하세요", "Select one"), key="unit1_mystery_job")
            if mystery_country and mystery_job:
            ...
            if lesson_button(interface_text("대화 완성", "Complete dialogue"), key="unit1_picture_finish", type="primary", disabled=not (mystery_country and mystery_job)):
...
def dashboard():
    st.markdown('<div id="learning-home-top"></div>', unsafe_allow_html=True)
    if st.session_state.pop("scroll_learning_home", False):
        st.iframe(
            """
            <script>
    ...
    if current_unit["number"] == 2:
        render_unit2_introduction()
    elif current_unit["number"] == 3:
        render_unit3_introduction()
    elif current_unit["number"] == 4:
        render_unit4_introduction()
    elif current_unit["number"] == 5:
        render_unit5_introduction()
    elif 6 <= current_unit["number"] <= 10:
        render_later_unit_introduction(current_unit["number"])
    if (
        current_unit["number"] == 1
        and not st.session_state.get("_unit1_intro_initialized", False)
        and get_unit_completion_steps(1)[0]
        and st.session_state.get("force_unit_home_step") != 1
    ):
        st.session_state.unit1_intro_collapsed = True
    if current_unit["number"] == 1:
        st.session_state._unit1_intro_initialized = True
    unit1_vocabulary_visible = False
    if current_unit["number"] == 1:
        selected_tab = st.session_state.get("_unit1_lesson_tabs")
        unit1_vocabulary_visible = (
            selected_tab.startswith(("● 어휘와 표현", "● Vocabulary & Expressions"))
            if selected_tab else not get_unit_completion_steps(1)[0]
        )
        pending_tab = st.session_state.get("requested_lesson_tab")
        if pending_tab and pending_tab[0] == 1:
            unit1_vocabulary_visible = pending_tab[1] == 0
        if st.session_state.get("force_unit_home_step") == 1:
            unit1_vocabulary_visible = True
        if unit1_vocabulary_visible:
            st.session_state.unit1_intro_collapsed = False
    unit1_intro_collapsed = bool(st.session_state.get("unit1_intro_collapsed", False))
    if current_unit["number"] == 1 and not unit1_vocabulary_visible:
        with st.container(border=True):
            summary_column, replay_column, close_column = st.columns([2.3, 1, .55], vertical_alignment="center")
            with summary_column:
                intro_summary = interface_text(
                    "**단원 도입 완료 ✓** · 대화 모델 3회 읽기 · 인사 → 이름 → 나라 → 직업",
                    "**Unit introduction complete ✓** · Read the model dialogue three times · Greeting → Name → Country → Occupation",
                ) if (st.session_state.get("unit1_read_round", 0) >= 3
                      and st.session_state.get("unit1_intro_sequence_step", 0) >= 4) else interface_text(
                    "**단원 도입** · 대화 모델 3회 읽기 · 인사 → 이름 → 나라 → 직업",
                    "**Unit introduction** · Read the model dialogue three times · Greeting → Name → Country → Occupation",
                )
                st.markdown(intro_summary)
            with replay_column:
                lesson_button(
                    interface_text("단원 도입 다시보기", "Review unit introduction"),
                    key="unit1_intro_expand",
                    width="stretch",
                    on_click=set_session_state_value,
                    args=("unit1_intro_collapsed", False),
                )
            with close_column:
                if not unit1_intro_collapsed:
                    lesson_button(
                        interface_text("닫기", "Close"),
                        key="unit1_intro_close",
                        width="stretch",
                        on_click=set_session_state_value,
                        args=("unit1_intro_collapsed", True),
                    )
        st.space("medium")
    if current_unit["number"] == 1 and not unit1_intro_collapsed:
        render_unit1_introduction()
    if current_unit["number"] in (2, 3, 4):
        st.space("medium")
    if current_unit["number"] == 1:
        st.markdown('<div id="unit1-vocabulary-step"></div>', unsafe_allow_html=True)
    path_support = (
        "Vocabulary → Grammar 1 → Grammar 2 → Activity 1 → Activity 2"
        if english_support_enabled()
        else "어휘와 표현 → 문법 1 → 문법 2 → 활동 1 → 활동 2 순서로 학습합니다."
    )
    path_heading = current_unit["title"]
    path_label = interface_text("단계별 학습", "Step-by-step learning")
    st.markdown(f'<div class="eyebrow">{current_unit_label} · {path_label}</div><div id="unit-learning-heading"><h2>{path_heading}</h2></div><p class="sub lesson-path-copy">{path_support}</p>', unsafe_allow_html=True)
    vocab_done, grammar1_done, grammar2_done, activity1_done, unit_completed = get_unit_completion_steps(current_unit["number"])
    ...
    unlocked_steps = [True, grammar1_unlocked, grammar2_unlocked, activity1_unlocked, activity2_unlocked]
    tab_labels = [
        f"{name}{'  ✓' if completed else ('  🔒' if not unlocked else '')}"
        for name, completed, unlocked in zip(tab_names, completed_steps, unlocked_steps)
    ]
    if not step1_done:
        active_step = 0
        ...
        active_step = None
    force_home_tab = st.session_state.pop("force_unit_home_step", None) == current_unit["number"]
    if force_home_tab:
        active_step = 0
    requested_lesson_tab = st.session_state.pop("requested_lesson_tab", None)
    automatic_stage_completion = st.session_state.pop("_automatic_stage_completion", False)
    requested_tab_index = None
    if requested_lesson_tab and requested_lesson_tab[0] == current_unit["number"]:
        candidate_index = requested_lesson_tab[1]
        if 0 <= candidate_index < len(unlocked_steps) and unlocked_steps[candidate_index]:
            active_step = candidate_index
            requested_tab_index = candidate_index
    completed_css = ",\n".join(
    ...
    completed_tab_color = "#365900" if st.session_state.get("app_theme") == "밝은색" else "#ffffff"
    completed_rule = f"{completed_css} {{ color:{completed_tab_color} !important; font-weight:700 !important; }}" if completed_css else ""
    active_number = active_step + 1 if active_step is not None else None
    active_tab_color = "#a6402b" if st.session_state.get("app_theme") == "밝은색" else "#ff5f52"
    active_rule = (
    ...
    if current_unit["number"] in (1, 2, 4, 5):
        active_rule = (
    ...
    locked_rule = (
        f"{locked_css} {{ opacity:.5 !important; pointer-events:none !important; cursor:not-allowed !important; }}"
        if locked_css else ""
    )
    st.markdown(
        f"""
    ...
    st.markdown('<div id="lesson-five-stage-tabs"></div>', unsafe_allow_html=True)
    unit1_tab_options = {}
    if current_unit["number"] == 1:
        if requested_tab_index is not None:
            st.session_state["_unit1_lesson_tabs"] = tab_labels[requested_tab_index]
        elif force_home_tab:
            st.session_state["_unit1_lesson_tabs"] = tab_labels[0]
        elif st.session_state.get("_unit1_lesson_tabs") not in tab_labels:
            previous_tab = st.session_state.get("_unit1_lesson_tabs", "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get("_unit1_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state["_unit1_lesson_tabs"] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": "_unit1_lesson_tabs",
        ...
        if requested_tab_index is not None:
            ...
            st.session_state["_unit2_lesson_tabs"] = tab_labels[0]
        elif st.session_state.get("_unit2_lesson_tabs") not in tab_labels:
            previous_tab = st.session_state.get("_unit2_lesson_tabs", "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get("_unit2_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state["_unit2_lesson_tabs"] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": "_unit2_lesson_tabs",
            "on_change": collapse_unit2_intro_on_tab_change,
            "args": (tab_labels[0],),
        }
    elif current_unit["number"] == 3:
        if requested_tab_index is not None:
            st.session_state["_unit3_lesson_tabs"] = tab_labels[requested_tab_index]
        elif force_home_tab:
            st.session_state["_unit3_lesson_tabs"] = tab_labels[0]
        elif st.session_state.get("_unit3_lesson_tabs") not in tab_labels:
            previous_tab = st.session_state.get("_unit3_lesson_tabs", "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get("_unit3_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state["_unit3_lesson_tabs"] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": "_unit3_lesson_tabs",
            "on_change": collapse_unit3_intro_on_tab_change,
            "args": (tab_labels[0],),
        }
    elif current_unit["number"] == 4:
        if requested_tab_index is not None:
            st.session_state["_unit4_lesson_tabs"] = tab_labels[requested_tab_index]
        elif force_home_tab:
            st.session_state["_unit4_lesson_tabs"] = tab_labels[0]
        elif st.session_state.get("_unit4_lesson_tabs") not in tab_labels:
            previous_tab = st.session_state.get("_unit4_lesson_tabs", "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get("_unit4_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state["_unit4_lesson_tabs"] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": "_unit4_lesson_tabs",
            "on_change": collapse_unit4_intro_on_tab_change,
            "args": (tab_labels[0],),
        }
    elif current_unit["number"] == 5:
        if requested_tab_index is not None:
            st.session_state["_unit5_lesson_tabs"] = tab_labels[requested_tab_index]
        elif force_home_tab:
            st.session_state["_unit5_lesson_tabs"] = tab_labels[0]
        elif st.session_state.get("_unit5_lesson_tabs") not in tab_labels:
            previous_tab = st.session_state.get("_unit5_lesson_tabs", "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get("_unit5_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state["_unit5_lesson_tabs"] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": "_unit5_lesson_tabs",
            "on_change": collapse_unit5_intro_on_tab_change,
            "args": (tab_labels[0],),
        }
    elif 6 <= current_unit["number"] <= 10:
        unit_number = current_unit["number"]
        tab_key = f"_unit{unit_number}_lesson_tabs"
        if requested_tab_index is not None:
            st.session_state[tab_key] = tab_labels[requested_tab_index]
        elif force_home_tab:
            st.session_state[tab_key] = tab_labels[0]
        elif st.session_state.get(tab_key) not in tab_labels:
            previous_tab = st.session_state.get(tab_key, "")
            previous_index = next((i for i, name in enumerate(tab_names) if previous_tab.startswith(name)), st.session_state.get(f"_unit{unit_number}_selected_lesson_step"))
            selected_index = previous_index if previous_index is not None else (active_step if active_step is not None else 4)
            st.session_state[tab_key] = tab_labels[selected_index]
        unit1_tab_options = {
            "key": tab_key,
            "on_change": collapse_later_unit_intro_on_tab_change,
            "args": (unit_number, tab_labels[0]),
        }
    # Streamlit includes default in the tab widget's identity. Keep it stable
    # for Units 1 and 2; session state handles selection and explicit navigation.
    vocab_tab, grammar1_tab, grammar2_tab, activity1_tab, activity2_tab = st.tabs(
        tab_labels,
        default=tab_labels[0 if current_unit["number"] in (1, 2, 4, 5, 6, 7, 8, 9, 10) else (active_step if active_step is not None else 4)],
        **unit1_tab_options,
    )
    if current_unit["number"] in range(1, 11):
        # An explicit rerun inside a number button can clean up widget state.
        # Retain the selected step independently of the tabs widget's lifetime.
        st.session_state[f"_unit{current_unit['number']}_selected_lesson_step"] = tab_labels.index(st.session_state[f"_unit{current_unit['number']}_lesson_tabs"])
    ...
    with vocab_tab:
        with st.container(border=True):
            ...
            if current_unit["number"] == 4:
                ...
                st.caption(interface_text("장소와 식품 단어를 하나씩 누르고 예문을 소리 내어 읽어 보세요.", "Select each place and food word, then read its example sentence aloud."))
            else:
            ...
            if current_unit["number"] == 2:
                ...
                unit2_visual_questions = [
                    ("버스", "몇 번이에요?", ["140번", "204번", "678번"], "140번", "백사십번", "bus"),
                    ("달력", "몇 월이에요?", ["3월", "5월", "8월"], "5월", "오월", "calendar"),
                    ("가격표", "얼마예요?", ["500원", "800원", "1,000원"], "800원", "팔백원", "price"),
                    ("방", "몇 호예요?", ["320호", "405호", "508호"], "405호", "사백오호", "room"),
                ]
                ...
                for index, (label, question, options, correct, correct_reading, visual_type) in enumerate(unit2_visual_questions):
                    with visual_question_columns[index % 2]:
                        with st.container(border=True, height=390):
                            ...
                            answer_key = f"unit2_visual_number_{index}"
                            if st.session_state.get(answer_key) in {"선택하세요", "Choose"}:
                                st.session_state[answer_key] = None
                            selected_answer = st.selectbox(question, balanced_answer_options(options, correct, index), key=answer_key, index=None, placeholder=unit2_choose_label)
            ...
            with st.container(key=f"unit{current_unit['number']}_vocabulary_progress_header"):
                st.progress(
                    ...
                    f"{interface_text('학습 완료', 'Completed')} ({vocabulary_read_count}/{len(vocabulary_words)})",
                )
            if current_unit["number"] == 1:
                country_count = 11
            ...
            if current_unit["number"] not in (3, 5, 6):
                meaning_button = interface_text("뜻 보기 / 뜻 가리기", "Show / Hide meaning") if current_unit["number"] in (1, 2) else "뜻 보기 / 뜻 가리기"
                if lesson_button(meaning_button, key=f"vocab_flip_{current_unit['number']}"):
            ...
            if current_unit["number"] == 1:
                st.divider()
                render_unit1_picture_dialogue(all_vocabulary_read=all_vocabulary_read)
            ...
            if current_unit["number"] == 1:
                activity2_reset_nonce = st.session_state.get("unit1_activity2_reset_nonce", 0)
                vocab_done = get_unit_completion_steps(1)[0] and not st.session_state.get("unit1_vocab_reviewing", False)
                missing_vocab_tasks = []
                if not all_vocabulary_read:
                    missing_vocab_tasks.append(interface_text("나라·직업 어휘 18개 확인", "Review all 18 country and occupation words"))
                if not unit1_tasks_done:
                    missing_vocab_tasks.append(interface_text("인물 카드 대화 4개", "Four person-card dialogues"))
                if st.session_state.get("unit1_read_round", 0) < 3:
                ...
                vocab_done = get_unit_completion_steps(3)[0]
                render_unit3_stage_actions(
                    "vocab", vocabulary_stage_ready,
                    interface_text("어휘 12개와 그림 대화 3문제를 모두 완료해 주세요.", "Complete all 12 vocabulary items and the three picture dialogues."),
                )
            elif current_unit["number"] == 4:
                vocab_done = get_unit_completion_steps(4)[0]
                render_unit4_stage_actions("vocab", vocabulary_stage_ready,
                                           interface_text("어휘와 두 그림 활동을 모두 완료해 주세요.", "Complete the vocabulary and both picture activities."))
            elif current_unit["number"] == 5:
                vocab_done = get_unit_completion_steps(5)[0]
                render_unit5_stage_actions("vocab", vocabulary_stage_ready,
                                           interface_text("어휘와 그림 활동을 모두 완료해 주세요.", "Complete the vocabulary and picture activities."))
            ...
            if 6 <= current_unit["number"] <= 10:
                render_later_unit_stage_actions(
                    current_unit["number"], "vocab",
                    interface_text("어휘를 모두 읽고 이 단원의 선택 활동을 완료해 주세요.", "Read all vocabulary and complete this unit's selection activities."),
                )
    with grammar1_tab:
        with st.container(border=True):
            if current_unit["number"] == 1:
            ...
            if current_unit["number"] == 2:
                ...
                unit2_g1_picture_exercises = [
                    ("마리", "마리 씨가 누구예요?", "제가 마리예요.", "younger-sister.png", ["제가 유진이에요.", "제가 마리예요.", "제가 주노예요."]),
                    ("주노 씨 동생", "주노 씨 동생이 누구예요?", "제가 주노 씨 동생이에요.", "student.png", ["제가 주노 씨 동생이에요.", "제가 마리 씨 친구예요.", "제가 요리사예요."]),
                    ("마리 씨 친구", "마리 씨 친구가 누구예요?", "제가 마리 씨 친구예요.", "vietnam.png", ["제가 주노 씨 동생이에요.", "제가 요리사예요.", "제가 마리 씨 친구예요."]),
                    ("요리사", "요리사가 누구예요?", "제가 요리사예요.", "cook.png", ["제가 마리예요.", "제가 요리사예요.", "제가 주노 씨 동생이에요."]),
                ]
                ...
                if unit2_g1_feedback_visible:
                    if all(unit2_g1_picture_answers):
                        render_learning_success(
                            interface_text(
                                "네 대화를 모두 정확하게 완성했어요.",
                                "You completed all four dialogues correctly.",
                            ),
            ...
            if 4 <= current_unit["number"] <= 10 and st.session_state.pop(f"_scroll_grammar1_final_{current_unit['number']}", False):
                st.html(
                    """
                    <script>
                    (() => {
            ...
            if current_unit["number"] != 1:
                ...
                # practiced; the unit's headline example may use a different
                # person or grammar point.
                current_completed_sentence = sentence.replace("__", answer)
                st.caption(f"{interface_text('완성 문장 예시', 'Completed sentence example')}: {current_completed_sentence}")
                grammar_choice = st.radio(
                    interface_text("빈칸에 알맞은 말을 선택하세요", "Choose the correct Korean expression for the blank"),
                    balanced_answer_options(options, answer, grammar_index),
                    key=f"grammar_quiz_choice_{current_unit['number']}_{grammar_index}"
                    + (f"_r{st.session_state.get('unit3_reset_nonce', 0)}" if current_unit["number"] == 3 else ""),
                    horizontal=True,
                ...
                grammar_passed_key = f"grammar1_passed_{current_unit['number']}_{grammar_index}"
                ...
                if not grammar_feedback_current:
                    ...
                    st.session_state[grammar_passed_key] = False
                ...
                grammar_check_column, grammar_next_column = st.columns(2, gap="small", width=360)
                with grammar_check_column:
                    if lesson_button(
                        interface_text("설명 닫기", "Hide explanation")
                        ...
                        help=interface_text(
                            "먼저 어휘와 표현을 완료하세요." if not grammar1_unlocked else "먼저 답을 선택하세요.",
                            "Complete Vocabulary and Expressions first." if not grammar1_unlocked else "Choose an answer first.",
                        ) if not grammar1_unlocked or grammar_choice is None else None,
                        width="stretch",
                    ):
                        if grammar_feedback_visible:
                            st.session_state[grammar_feedback_key] = False
                            ...
                            st.session_state[grammar_passed_key] = grammar_choice == answer
                            st.session_state[grammar_feedback_key] = True
                        st.rerun()
                if grammar_index < len(grammar_questions) - 1:
                    with grammar_next_column:
                        can_advance = (
                            grammar1_unlocked
                            and st.session_state.get(grammar_passed_key, False)
                            and grammar_choice == answer
                        )
                        if lesson_button(interface_text("다음 문장 →", "Next sentence →"), key=f"grammar_next_{current_unit['number']}_{grammar_index}", type="primary", disabled=not can_advance, help=interface_text("정답을 맞히면 이동할 수 있어요.", "Answer correctly to continue.") if not can_advance else None, width="stretch"):
            ...
            grammar_result_is_current = (
                st.session_state.get(f"grammar1_passed_{current_unit['number']}_{grammar_index}", False)
                and grammar_choice == answer
            ...
            if grammar_result_is_current:
                if grammar_index == len(grammar_questions) - 1:
                    if current_unit["number"] == 1:
                        ...
                        completed_picture_count = sum(
                            selected_answer == exercise[4]
                            for exercise, selected_answer in zip(picture_exercises, saved_picture_feedback or [])
                        )
                        st.progress(
                            completed_picture_count / len(picture_exercises),
                            text=f"{interface_text('그림 대화 완료', 'Picture dialogues complete')} ({completed_picture_count}/{len(picture_exercises)})",
                        )
                        picture_answers = []
                            ...
                            with st.container(border=True):
                                ...
                                with dialogue_column:
                                    ...
                                    selected_answer = st.radio(
                                        f"{index + 1}번 대답",
                                        balanced_answer_options(choices, correct_answer, index),
                        ...
                        if lesson_button(picture_feedback_button, key="unit1_grammar1_picture_check", type="primary", disabled=not picture_answers_ready and not picture_feedback_visible, help=interface_text("먼저 답을 모두 선택하세요.", "Choose all answers first.") if not picture_answers_ready and not picture_feedback_visible else None):
                        ...
                        st.divider()
                        st.markdown(interface_text("### 2. 사진을 보고 이름과 나라를 선택해 대화를 완성해 보세요.", "### 2. Look at the photo and complete the dialogue with a name and country"))
                        st.caption(interface_text("이름과 나라를 목록에서 선택하면 알맞은 소개 대화가 완성됩니다. 완성된 대화를 소리 내어 읽어 보세요.", "Choose a name and country from the lists, then read the completed Korean dialogue aloud."))
                        friend_photo, friend_form = st.columns([1, 2], vertical_alignment="center")
                        with friend_photo:
                            render_unit1_study_image("male-friend.png", canvas_size=(220, 220), image_size=(205, 210))
                        with friend_form:
                            exercise_friend_name = st.selectbox(interface_text("친구 이름", "Friend’s name"), ["민", "민수", "지훈", "준호"], key="unit1_grammar1_friend_name_choice", index=None, placeholder=interface_text("선택하세요", "Choose"))
                        ...
                        if exercise_friend_name is not None and exercise_friend_country is not None:
                            name_ending = "이에요" if subject_particle(exercise_friend_name) == "이" else "예요"
                            # Ask an open country question so every selected
                            # country produces a grammatically consistent answer.
                            friend_country_answer = f"{exercise_friend_country} 사람이에요."
                            render_learning_success(
                                f"가: 누구예요?\n\n나: {exercise_friend_name}{name_ending}. 제 친구예요.\n\n가: 어느 나라 사람이에요?\n\n나: {friend_country_answer}",
                                icon=":material/forum:",
                            )
                        st.divider()
                        final_questions = [
                            ("제 이름은 지우__.", "예요", "‘지우’는 받침이 없으므로 ‘예요’를 사용해요."),
                            ("저는 회사원__.", "이에요", "‘회사원’은 받침이 있으므로 ‘이에요’를 사용해요."),
                            ("제 친구는 요리사__.", "예요", "‘요리사’는 받침이 없으므로 ‘예요’를 사용해요."),
                            ("우리 형은 경찰__.", "이에요", "‘경찰’은 받침이 있으므로 ‘이에요’를 사용해요."),
                        ]
                        final_index_key = "unit1_grammar1_final_index"
                        final_index = min(st.session_state.get(final_index_key, 0), len(final_questions) - 1)
                        final_sentence, final_answer, final_explanation = final_questions[final_index]
                        final_choice_key = f"unit1_grammar1_final_choice_{final_index}"
                        final_checked_key = f"unit1_grammar1_final_checked_choice_{final_index}"
                        saved_final_completed_count = sum(
                            st.session_state.get(f"unit1_grammar1_final_checked_choice_{index}") == question[1]
                            and st.session_state.get(f"unit1_grammar1_final_choice_{index}") == question[1]
                            for index, question in enumerate(final_questions)
                        )
                        st.progress(
                            saved_final_completed_count / len(final_questions),
                            text=f"{interface_text('마지막 문법 확인', 'Final grammar check')} ({saved_final_completed_count}/{len(final_questions)})",
                        )
                        st.caption(interface_text("한 문장씩 정답을 확인하고 다음 문장으로 넘어가세요.", "Check each answer before moving to the next Korean sentence."))
                        st.markdown(f"### {final_sentence.replace('__', '＿＿＿＿')}")
                        completed_final_sentence = final_sentence.replace("__", final_answer)
                        unit1_final_choice = st.radio(
                            "빈칸에 알맞은 말을 선택하세요",
                            balanced_answer_options(["이에요", "예요"], final_answer, final_index),
                            key=final_choice_key,
                            horizontal=True,
                            index=None,
                            label_visibility="collapsed",
                        )
                        final_check_column, final_next_column = st.columns(2, gap="small", width=360)
                        with final_check_column:
                            if lesson_button(interface_text("정답 확인", "Check answer"), key=f"unit1_grammar1_final_check_{final_index}", disabled=unit1_final_choice is None, help=interface_text("먼저 답을 선택하세요.", "Choose an answer first.") if unit1_final_choice is None else None, width="stretch"):
                                st.session_state[final_checked_key] = unit1_final_choice
                                st.rerun()
                        final_answer_current = st.session_state.get(final_checked_key) == unit1_final_choice
                        with final_next_column:
                            if final_index < len(final_questions) - 1 and lesson_button(
                                interface_text("다음 문장 →", "Next sentence →"),
                                key=f"unit1_grammar1_final_next_{final_index}",
                                disabled=not (final_answer_current and unit1_final_choice == final_answer),
                                width="stretch",
                            ):
                                st.session_state[final_index_key] = final_index + 1
                                st.rerun()
                        if final_answer_current and unit1_final_choice is not None:
                            st.caption(f"{interface_text('완성 문장 예시', 'Completed sentence example')}: {completed_final_sentence}")
                            ...
                            render_learning_info(interface_text(f"왜 정답일까요? {final_explanation}", f"Why? {final_explanation}"), icon=":material/lightbulb:")
                        ...
                        unit1_grammar1_ready = (
                            ...
                            and exercise_friend_name is not None
                            and exercise_friend_country is not None
                            and saved_final_completed_count == len(final_questions)
                        )
                        render_unit1_stage_actions(
                            "grammar1", unit1_grammar1_ready and grammar1_unlocked,
                            interface_text(
                                "그림 대화 네 문제, 친구 소개, 마지막 문법 확인을 모두 마쳐 주세요.",
                                "Complete the four picture dialogues, friend introduction, and final grammar check.",
                            ),
                        )
                    elif current_unit["number"] != 2:
                        if current_unit["number"] == 4:
            ...
            if current_unit["number"] == 3:
                unit3_grammar1_ready = (
                    st.session_state.get("unit3_grammar1_activities_completed", False)
                    and grammar_index == len(grammar_questions) - 1
                    and grammar_result_is_current
                )
                render_unit3_stage_actions(
                    "grammar1",
                    unit3_grammar1_ready and grammar1_unlocked,
                    interface_text(
                        "문법 1의 두 활동과 네 문장 연습을 모두 완료해 주세요.",
                        "Complete both Grammar 1 activities and all four practice sentences.",
                    ),
                )
            if current_unit["number"] == 4:
                unit4_grammar1_ready = (
                    st.session_state.get("unit4_grammar1_picture_completed", False)
                    and st.session_state.get("unit4_grammar1_speaking_completed", False)
                    and grammar_index == len(grammar_questions) - 1
                    and grammar_result_is_current
                )
                render_unit4_stage_actions("grammar1", unit4_grammar1_ready and grammar1_unlocked,
                                           interface_text("그림 대화, 말하기와 네 문장 연습을 모두 완료해 주세요.", "Complete the picture dialogue, speaking, and four sentence exercises."))
            if current_unit["number"] == 5:
                unit5_grammar1_ready = (
                    st.session_state.get("unit5_grammar1_activities_completed", False)
                    and grammar_index == len(grammar_questions) - 1
                    and grammar_result_is_current
                )
                render_unit5_stage_actions("grammar1", unit5_grammar1_ready and grammar1_unlocked,
                                           interface_text("그림 문제, 말하기와 네 문장을 모두 완료해 주세요.", "Complete the picture, speaking, and four sentence exercises."))
            if 6 <= current_unit["number"] <= 10:
                unit_number = current_unit["number"]
                grammar1_ready = st.session_state.get(f"unit{unit_number}_grammar1_activities_completed", False)
                if unit_number in (6, 7, 8, 9):
                    grammar1_ready = grammar1_ready and grammar_index == len(grammar_questions) - 1 and grammar_result_is_current
                auto_complete_stage(unit_number, "grammar1", grammar1_ready and grammar1_unlocked)
                render_later_unit_stage_actions(
                    unit_number, "grammar1",
                    interface_text("문법 1의 선택·말하기 활동과 마무리 확인을 완료해 주세요.", "Complete the Grammar 1 choices, speaking practice, and final check."),
                )
    with grammar2_tab:
        with st.container(border=True):
            if current_unit["number"] == 1:
                grammar2_reset_nonce = st.session_state.get("unit1_grammar2_reset_nonce", 0)
            ...
            if current_unit["number"] == 1:
                ...
                render_learning_info(interface_text(
                    "나라를 묻는 대화는 ‘어느 나라 사람이에요?’라고 물어요. 선택한 나라에 맞춰 대답해요.",
                    "Ask ‘어느 나라 사람이에요?’ and answer with the country you selected.",
                    ...
                    with st.container(border=True):
                        ...
                        with answer_column:
                            ...
                            choice = st.radio(
                                ...
                                key=f"unit1_grammar2_choice_{index}_{grammar2_reset_nonce}",
                                horizontal=True,
                ...
                with grammar2_button_columns[0]:
                    ...
                    if lesson_button(interface_text("네 문장 확인", "Check four sentences"), key="unit1_grammar2_check", type="primary", disabled=not grammar2_unlocked or not unit1_g2_answers_ready, width="stretch"):
                ...
                with grammar2_button_columns[1]:
                    if saved_g2_feedback:
                        lesson_button(interface_text("확인 닫기", "Close feedback"), key="unit1_grammar2_feedback_close", type="secondary", width="stretch", on_click=clear_session_state_key, args=("unit1_grammar2_feedback",))
                if saved_g2_feedback:
                    ...
                    for index, (question, selected_answer) in enumerate(zip(people_questions, saved_g2_feedback)):
                        ...
                        if selected_answer == answer_sentence:
                            ...
                            render_learning_error(feedback)
                st.divider()
                st.markdown(interface_text("### 2. 그림 속 친구의 이름과 직업을 정해 소개해 보세요.", "### 2. Choose a name and occupation for the friend in the picture"))
                st.caption(interface_text("그림 속 인물을 내 친구라고 생각해 보세요. 이름과 직업을 목록에서 선택하세요. 그림에서 이름이나 직업을 맞히는 문제가 아니에요.", "Imagine the person in the picture is your friend. Choose a name and occupation from the lists. You do not need to guess a name or occupation from the picture."))
                friend_photo, friend_form = st.columns([1, 2], vertical_alignment="center")
                with friend_photo:
                    render_unit1_study_image("male-friend.png", canvas_size=(220, 220), image_size=(205, 210))
                    st.caption(interface_text("내가 소개할 친구", "The friend I will introduce"))
                with friend_form:
                    friend_name = st.selectbox(interface_text("친구 이름", "Friend’s name"), ["민", "민수", "지훈", "준호"], key="unit1_grammar2_friend_name_choice", index=None, placeholder=interface_text("선택하세요", "Choose"))
                    friend_job = st.selectbox(interface_text("친구 직업", "Friend’s occupation"), ["회사원", "대학생", "선생님", "의사", "요리사", "가수"], key="unit1_grammar2_friend_job", index=None, placeholder="선택하세요")
                friend_particle = "은" if friend_name and subject_particle(friend_name) == "이" else "는"
                job_ending = "이에요" if friend_job and subject_particle(friend_job) == "이" else "예요"
                friend_sentence = f"{friend_name}{friend_particle} 제 친구예요.\n\n{friend_name}{friend_particle} {friend_job}{job_ending}." if friend_name is not None and friend_job is not None else ""
                if friend_sentence:
                    st.markdown(interface_text("**내가 완성한 친구 소개**", "**My completed introduction**"))
                    render_learning_success(friend_sentence, icon=":material/groups:")
                    st.caption(interface_text("내가 정한 이름과 직업이 들어간 두 문장을 소리 내어 읽어 보세요.", "Read both sentences aloud using the name and occupation you chose."))
                else:
                    st.caption(interface_text("이름과 직업을 선택하면 소개 문장이 나타납니다.", "Choose a name and occupation to see your introduction."))
                unit1_g2_answers_current = saved_g2_feedback == selected_g2_answers
                if saved_g2_feedback is None and st.session_state.get("unit1_grammar2_quiz_passed", False):
                    unit1_g2_answers_current = all(
                        selected == question[3]
                        for question, selected in zip(people_questions, selected_g2_answers)
                    )
                unit1_g2_ready = (
                ...
                friend_completion_help = None
                if not st.session_state.get("unit1_grammar2_quiz_passed", False) or not unit1_g2_answers_current:
                    friend_completion_help = interface_text("먼저 1번의 네 문장을 모두 맞혀 주세요.", "First, answer all four sentences in Activity 1 correctly.")
                elif not friend_sentence:
                    friend_completion_help = interface_text("친구 이름과 직업을 선택해 주세요.", "Choose your friend’s name and occupation.")
                render_unit1_stage_actions("grammar2", unit1_g2_ready and grammar2_unlocked, friend_completion_help)
            elif current_unit["number"] == 2:
                st.space("small")
                ...
                unit2_negative_questions = [
                    ("선생님이에요?", ["아니요. 선생님가 아니에요.", "아니요. 선생님이 아니에요. 학생이에요."], "아니요. 선생님이 아니에요. 학생이에요."),
                    ("컴퓨터예요?", ["아니요. 컴퓨터가 아니에요. 텔레비전이에요.", "아니요. 컴퓨터이 아니에요."], "아니요. 컴퓨터가 아니에요. 텔레비전이에요."),
                    ("영화관이 7층이에요?", ["네. 7층이에요.", "아니요. 7층이 아니에요. 8층이에요."], "아니요. 7층이 아니에요. 8층이에요."),
                    ("여자예요?", ["아니요. 여자가 아니에요. 남자예요.", "네. 여자예요."], "아니요. 여자가 아니에요. 남자예요."),
                ]
                unit2_negative_results = []
                unit2_negative_answers = []
                ...
                for index, (question, choices, correct) in enumerate(unit2_negative_questions):
                    with st.container(border=True):
                        ...
                        with question_col:
                            st.markdown(f"**가:** {question}")
                            selected = st.radio("나의 대답", balanced_answer_options(choices, correct, index), key=f"unit2_negative_{index}", horizontal=True, index=None, label_visibility="collapsed")
                        unit2_negative_results.append(selected == correct)
                        unit2_negative_answers.append(selected)
                unit2_negative_ready = all(answer is not None for answer in unit2_negative_answers)
                if lesson_button(interface_text("네 문장 확인", "Check four sentences"), key="unit2_negative_check", type="primary", disabled=not unit2_negative_ready):
                    st.session_state["unit2_negative_checked_answers"] = list(unit2_negative_answers)
                    st.session_state["unit2_negative_quiz_passed"] = all(unit2_negative_results)
                if st.session_state.get("unit2_negative_checked_answers") == unit2_negative_answers and unit2_negative_ready:
                    if all(unit2_negative_results):
                        render_learning_success(interface_text("네 문장을 모두 맞혔어요!", "All four answers are correct!"), icon=":material/check_circle:")
                    else:
                        incorrect_indices = [index for index, is_correct in enumerate(unit2_negative_results) if not is_correct]
                        incorrect_labels = ", ".join(interface_text(f"{index + 1}번", f"Question {index + 1}") for index in incorrect_indices)
                        render_learning_warning(interface_text(f"{sum(unit2_negative_results)}/4개가 맞아요. 틀린 문항은 {incorrect_labels}이에요.", f"{sum(unit2_negative_results)}/4 are correct. Review: {incorrect_labels}."))
                ...
                for index, (label, image_name, question, correct_answer) in enumerate(negative_dialogue_cards):
                    with negative_dialogue_columns[index]:
                        with st.container(border=True):
                            ...
                            st.caption(label)
                            st.markdown(f"**가:** {question}")
                            selected_answer = st.radio("대답", balanced_answer_options([correct_answer, "네, 맞아요."], correct_answer, index), key=f"unit2_negative_dialogue_{index}", index=None, label_visibility="collapsed")
                ...
                unit2_grammar2_ready = (
                    st.session_state.get("unit2_negative_quiz_passed", False)
                    and st.session_state.get("unit2_negative_checked_answers") == unit2_negative_answers
                    and all(unit2_dialogue_results)
                )
                ...
                render_unit3_stage_actions("grammar2",
                                           st.session_state.get("grammar2_done_3", False) and grammar2_unlocked,
                                           interface_text("문법 2의 두 활동을 모두 정확하게 완료해 주세요.", "Complete both Grammar 2 activities correctly."))
            elif current_unit["number"] == 4:
                render_unit4_grammar2()
                render_unit4_stage_actions("grammar2", st.session_state.get("grammar2_done_4", False) and grammar2_unlocked,
                                           interface_text("문법 2의 두 활동을 모두 맞혀 주세요.", "Answer both Grammar 2 activities correctly."))
            elif current_unit["number"] == 5:
                render_unit5_grammar2()
                render_unit5_stage_actions("grammar2", st.session_state.get("grammar2_done_5", False) and grammar2_unlocked,
                                           interface_text("문법 2의 두 활동을 모두 완료해 주세요.", "Complete both Grammar 2 activities."))
            elif current_unit["number"] == 6:
                ...
                with grammar_save_columns[0]:
                    if lesson_button("정답 및 문법 확인", key=f"grammar_save_{current_unit['number']}", type="primary", disabled=not grammar2_unlocked or not grammar_sentence.strip(), width="stretch"):
                ...
                with grammar_save_columns[1]:
                    if feedback_state and feedback_state[2] == grammar_sentence.strip():
                        lesson_button("확인 닫기", key=f"grammar2_feedback_close_{current_unit['number']}", type="secondary", width="stretch", on_click=clear_session_state_key, args=(feedback_key,))
                if feedback_state and feedback_state[2] == grammar_sentence.strip():
                    if feedback_state[0]:
    ...
    with activity1_tab:
        with st.container(border=True):
            ...
            if current_unit["number"] == 1:
                ...
                with question_columns[0]:
                    anna_job = st.selectbox(interface_text("안나 씨의 직업은 무엇이에요?", "What is Anna’s occupation?"), ["학생", "회사원", "선생님"], key="unit1_activity1_anna_job", index=None, placeholder="선택하세요")
                with question_columns[1]:
                    juno_job = st.selectbox(interface_text("주노 씨의 직업은 무엇이에요?", "What is Juno’s occupation?"), ["학생", "회사원", "요리사"], key="unit1_activity1_juno_job", index=None, placeholder="선택하세요")
                greeting_answered = anna_job is not None and juno_job is not None
                ...
                with greeting_button_columns[0]:
                    if lesson_button(interface_text("1번 답 확인", "Check Activity 1 answers"), key="unit1_activity1_greeting_check", type="primary", disabled=not greeting_answered, width="stretch"):
                        st.session_state["unit1_activity1_greeting_checked"] = (anna_job, juno_job)
                        st.session_state["unit1_activity1_greeting_passed"] = (anna_job, juno_job) == ("회사원", "학생")
                        checked_greeting = (anna_job, juno_job)
                with greeting_button_columns[1]:
                    if checked_greeting:
                        lesson_button(interface_text("확인 닫기", "Close feedback"), key="unit1_activity1_greeting_result_close", type="secondary", width="stretch", on_click=clear_session_state_key, args=("unit1_activity1_greeting_checked",))
                if checked_greeting:
                ...
                with role_columns[0]:
                    selected_friend_name = st.selectbox(
                        interface_text("이름", "Name"),
                        list(friend_roster),
                        key="unit1_activity1_friend_name",
                        index=None,
                        placeholder="선택하세요",
                    )
                with role_columns[1]:
                ...
                if selected_friend_name is not None and selected_friend_job is not None:
                ...
                unit1_activity1_ready = (
                    activity1_unlocked
                    and (
                        checked_greeting == (anna_job, juno_job) == ("회사원", "학생")
                        or (
                            checked_greeting is None
                            and st.session_state.get("unit1_activity1_greeting_passed", False)
                            and (anna_job, juno_job) == ("회사원", "학생")
                        )
                    )
                    and bool(selected_friend_name is not None and selected_friend_job is not None)
                )
                render_unit1_stage_actions(
                ...
                with activity1_answer_columns[1]:
                    phone_answer = st.selectbox(
                        ...
                        ["010-1359-6783", "010-1359-6784"],
                        key="unit2_activity1_phone",
                        index=None,
                        placeholder="선택하세요",
                    )
                activity1_answers_ready = conversation_topic is not None and phone_answer is not None
                ...
                with activity1_check_columns[0]:
                    if lesson_button(interface_text("1번 답 확인", "Check Activity 1 answers"), key="unit2_activity1_check", type="primary", disabled=not activity1_answers_ready, width="stretch"):
                ...
                with activity1_check_columns[1]:
                    if activity1_check_result:
                        lesson_button(interface_text("확인 닫기", "Close feedback"), key="unit2_activity1_result_close", type="secondary", width="stretch", on_click=clear_session_state_key, args=("unit2_activity1_check_result",))
                if (
                ...
                st.space("small")
                phone_friend_names = list(friend_phone_roster)
                phone_friend = st.selectbox(
                    interface_text("번호를 확인할 사람", "Person whose number you will check"),
                    phone_friend_names,
                    key="unit2_activity1_friend",
                    on_change=reset_unit2_activity1_phone_choices,
                )
                phone_friend_index = phone_friend_names.index(phone_friend)
                ...
                phone_reset_nonce = st.session_state.get("unit2_activity1_phone_reset_nonce", 0)
                heard_phone_key = "unit2_activity1_heard_phone" if phone_reset_nonce == 0 else f"unit2_activity1_heard_phone_{phone_reset_nonce}"
                confirmation_key = "unit2_activity1_confirmation" if phone_reset_nonce == 0 else f"unit2_activity1_confirmation_{phone_reset_nonce}"
                heard_phone_options = (
                    [incorrect_friend_phone, correct_friend_phone]
                    if phone_friend_index % 2 == 0
                    else [correct_friend_phone, incorrect_friend_phone]
                )
                ...
                if heard_phone is None:
                    correct_confirmation = "네, 맞아요."
                    confirmation_choices = ["네, 맞아요.", "아니요."]
                elif heard_phone == correct_friend_phone:
                    correct_confirmation = "네, 맞아요."
                ...
                render_learning_info(
                    ...
                    f":orange[재민: {heard_phone or '＿＿＿＿＿＿＿＿＿＿'}, 맞아요?]\n\n"
                ...
                with confirmation_button_columns[0]:
                    if lesson_button(interface_text("2번 답 확인", "Check Activity 2 answer"), key="unit2_activity1_confirmation_check", type="primary", width="stretch", disabled=confirmation_answer is None):
                ...
                with confirmation_button_columns[1]:
                    if confirmation_result:
                        lesson_button(
                            interface_text("확인 닫기", "Close feedback"),
                            key="unit2_activity1_confirmation_result_close",
                            type="secondary",
                            width="stretch",
                            on_click=close_unit2_activity1_confirmation_feedback,
                        )
                if (
                    confirmation_result
                ...
                if activity1_check_result is None and st.session_state.get("unit2_activity1_part1_passed", False):
                    unit2_activity1_part1_current = (
                        conversation_topic == "전화번호"
                        and phone_answer == "010-1359-6783"
                    )
                ...
                if (
                    not confirmation_result
                    and heard_phone is None
                    and confirmation_answer is None
                    and st.session_state.get("unit2_activity1_part2_passed", False)
                ):
                    unit2_activity1_part2_current = True
                ...
                render_unit3_stage_actions("activity1",
                                           st.session_state.get("activity1_completed_3", False) and activity1_unlocked,
                                           interface_text("활동 1의 읽기와 말하기를 모두 완료해 주세요.", "Complete both Activity 1 reading and speaking tasks."))
            elif current_unit["number"] == 4:
                render_unit4_activity1()
                render_unit4_stage_actions("activity1", st.session_state.get("activity1_completed_4", False) and activity1_unlocked,
                                           interface_text("읽기와 말하기 활동을 모두 완료해 주세요.", "Complete both the reading and speaking activities."))
            elif current_unit["number"] == 5:
                render_unit5_activity1()
                render_unit5_stage_actions("activity1", st.session_state.get("activity1_completed_5", False) and activity1_unlocked,
                                           interface_text("읽기와 말하기 활동을 모두 완료해 주세요.", "Complete the reading and speaking activities."))
            elif current_unit["number"] == 6:
                ...
                if lesson_button("활동 1 완료", key=f"activity1_done_{current_unit['number']}", disabled=not activity1_unlocked or not response.strip()):
                    st.session_state[f"activity1_completed_{current_unit['number']}"] = True
                    st.rerun()
            if 6 <= current_unit["number"] <= 10:
                render_later_unit_stage_actions(
                    current_unit["number"], "activity1",
                    interface_text("활동 1의 읽기와 말하기를 모두 완료해 주세요.", "Complete both Activity 1 reading and speaking."),
                )
    with activity2_tab:
        with st.container(border=True):
            if current_unit["number"] not in (3, 4, 5, 6, 7, 8, 9, 10):
            ...
            if current_unit["number"] == 1:
                ...
                st.markdown(
                    """
                    <style>
                    div[class*="st-key-unit1_activity2_profile_"] { min-height:0 !important; }
                    div[class*="st-key-unit1_activity2_profile_check"] {
                        margin-bottom:0 !important;
                    }
                    .unit1-activity2-section2-heading {
            ...
            div[class*="st-key-unit1_grammar_dialogue_"] [data-testid="stColumn"] {
                ...
                for index, (column, (image_name, name, introduction)) in enumerate(zip(introduction_columns, introductions)):
                    with column:
                        with st.container(border=True, key=f"unit1_activity2_profile_{index}"):
                ...
                with answer_columns[0]:
                    wei_name = st.selectbox(interface_text("첫 번째 사람의 이름", "First person’s name"), ["웨이", "유나", "민수"], key=f"unit1_activity2_wei_name_choice_{activity2_reset_nonce}", index=None, placeholder=unit1_profile_choose_label)
                    wei_job = st.selectbox(interface_text("첫 번째 사람의 직업", "First person’s occupation"), ["회사원", "가수", "요리사"], key=f"unit1_activity2_wei_job_{activity2_reset_nonce}", index=None, placeholder="선택하세요")
                with answer_columns[1]:
                    yuna_name = st.selectbox(interface_text("두 번째 사람의 이름", "Second person’s name"), ["유나", "웨이", "마리아"], key=f"unit1_activity2_yuna_name_choice_{activity2_reset_nonce}", index=None, placeholder=unit1_profile_choose_label)
                    yuna_job = st.selectbox(interface_text("두 번째 사람의 직업", "Second person’s occupation"), ["의사", "가수", "요리사"], key=f"unit1_activity2_yuna_job_{activity2_reset_nonce}", index=None, placeholder="선택하세요")
                profile_answers_ready = all([wei_name is not None, yuna_name is not None, wei_job is not None, yuna_job is not None])
                ...
                with profile_button_columns[1]:
                    if checked_profile_answers:
                        lesson_button(interface_text("확인 닫기", "Close feedback"), key="unit1_activity2_profile_result_close", type="secondary", width="stretch", on_click=clear_session_state_key, args=("unit1_activity2_profile_answers",))
                if checked_profile_answers:
                    profile_feedback = [
                    ...
                    for label, selected, correct, explanation in profile_feedback:
                        if selected == correct:
                            ...
                            feedback = interface_text(f"{label}: 선택한 답은 ‘{selected}’예요. 정답은 ‘{correct}’입니다. {explanation}", f"{label}: You chose ‘{selected}’. The correct answer is ‘{correct}’. {explanation}")
                            render_learning_error(feedback)

                section2_heading = interface_text("2. 여러분을 소개하는 글을 써 보세요.", "2. Write your own self-introduction")
                st.markdown(
                    f"<h3 class='unit1-activity2-section2-heading'>{html.escape(section2_heading)}</h3>",
                    unsafe_allow_html=True,
                )
                st.caption(interface_text(
                    "이름·나라·직업을 선택하고 ‘이에요/예요’와 ‘은/는’을 사용한 자기소개를 확인하세요.\n\n예) 저는 민수예요. 저는 한국 사람이에요. 저는 회사원이에요.",
                    "Choose a name, country, and occupation to see your Korean introduction using 이에요/예요 and 은/는.\n\nExample: 저는 민수예요. 저는 한국 사람이에요. 저는 회사원이에요.",
                ))
                intro_columns = st.columns(3)
                with intro_columns[0]:
                    intro_name = st.selectbox(interface_text("① 이름", "① Name"), ["마리아", "민수", "지우", "안나", "웨이", "유나"], key=f"unit1_final_name_choice_{activity2_reset_nonce}", index=None, placeholder=interface_text("선택하세요", "Choose"), disabled=not activity2_unlocked)
                with intro_columns[1]:
                    intro_country = st.selectbox(interface_text("② 나라", "② Country"), ["한국", "캐나다", "베트남", "미국", "프랑스", "태국", "인도네시아", "중국", "일본", "러시아", "케냐"], key=f"unit1_final_country_{activity2_reset_nonce}", index=None, placeholder="선택하세요", disabled=not activity2_unlocked)
                with intro_columns[2]:
                    intro_job = st.selectbox(interface_text("③ 직업", "③ Occupation"), ["회사원", "대학생", "의사", "경찰", "선생님", "가수", "요리사"], key=f"unit1_final_job_{activity2_reset_nonce}", index=None, placeholder="선택하세요", disabled=not activity2_unlocked)
                name_ending = "이에요" if intro_name and subject_particle(intro_name) == "이" else "예요"
                introduction_response = f"안녕하세요? 저는 {intro_name}{name_ending}. 저는 {intro_country} 사람이에요. {intro_job}{'이에요' if subject_particle(intro_job) == '이' else '예요'}." if intro_name is not None and intro_country is not None and intro_job is not None else ""
                ...
                if checked_profile_answers is None and st.session_state.get("unit1_activity2_profile_passed", False):
                    unit1_profile_check_current = (
                        current_profile_answers == {
                            "웨이 이름": "웨이",
                            "웨이 직업": "요리사",
                            "유나 이름": "유나",
                ...
                st.divider()
                st.space("small")
                st.markdown(interface_text("**2. 연습용 이름·전화번호·이메일 주소를 골라 보세요.**", "**2. Choose a practice name, phone number, and email address**"))
                st.caption(interface_text("가상 연락처 세 줄의 이름·전화번호·이메일 주소를 모두 선택해 보세요.", "Choose the name, phone number, and email address for all three fictional contacts."))
                render_learning_info(
                    interface_text(
                        "실제 개인정보 대신 준비된 가상 연락처를 선택하세요. 이메일은 @example.com 앞부분을 고르면 완성됩니다.",
                        "Choose from the fictional contact details provided. Select the part before @example.com to complete the email address.",
                    ),
                ...
                for index in range(3):
                    ...
                    with contact_columns[0]:
                        contact_name = st.selectbox(
                            ...
                            placeholder=interface_text("선택하세요", "Choose"),
                            index=None,
                            key=f"unit2_contact_name_choice_{index}",
                            label_visibility="collapsed",
                        )
                    with contact_columns[1]:
                        contact_phone = st.selectbox(
                            f"{index + 1}번 전화번호",
                            ["010-1234-5678", "010-2345-6789", "010-3456-7890", "010-4567-8901"],
                            placeholder=interface_text("선택하세요", "Choose"),
                            index=None,
                            key=f"unit2_contact_phone_choice_{index}",
                            label_visibility="collapsed",
                        )
                    with contact_columns[2]:
                        ...
                        with email_name_column:
                            contact_email_name = st.selectbox(
                                f"{index + 1}번 이메일 아이디",
                                ["mari", "minsu", "anna", "yuna", "wei"],
                                placeholder=interface_text("선택하세요", "Choose"),
                                index=None,
                                key=f"unit2_contact_email_name_choice_{index}",
                                label_visibility="collapsed",
                            )
                        with email_domain_column:
                            st.markdown("**@example.com**")
                        contact_email = f"{contact_email_name}@example.com" if contact_email_name else ""
                    contact_rows.append((contact_name or "", contact_phone or "", contact_email))
                ...
                st.caption(interface_text(f"연락처 선택 ({len(completed_contacts)}/3)", f"Contacts selected ({len(completed_contacts)}/3)"))
                if incomplete_contacts:
                    st.caption(interface_text("선택을 시작한 줄의 이름·전화번호·이메일 주소를 모두 골라 주세요.", "Choose the name, phone number, and email address in every row you started."))
                contact_response = "\n".join(" · ".join(row) for row in completed_contacts) if len(completed_contacts) == len(contact_rows) else ""
                if not contact_response and get_unit_completion_steps(2)[4]:
                    # Earlier versions completed this stage after one contact.
                    st.session_state["unit2_activity2_reviewing"] = True
                if contact_response:
            ...
            if activity2_unlocked and not response.strip():
                st.caption(interface_text("필수 학습을 모두 마치면 활동 2가 자동으로 완료됩니다.", "Activity 2 completes automatically when you finish all required activities."))
            activity2_completed = get_unit_completion_steps(current_unit["number"])[4] and not st.session_state.get(f"unit{current_unit['number']}_activity2_reviewing", False)
            if current_unit["number"] == 1:
                render_unit1_stage_actions(
                ...
                render_unit4_stage_actions("activity2", activity2_unlocked and bool(response.strip()))
            elif current_unit["number"] == 5:
                render_unit5_stage_actions("activity2", activity2_unlocked and bool(response.strip()))
            elif 6 <= current_unit["number"] <= 10:
                unit_number = current_unit["number"]
                auto_complete_stage(unit_number, "activity2", activity2_unlocked and bool(response.strip()))
                render_later_unit_stage_actions(
                    unit_number, "activity2",
                    interface_text("활동 2의 문장과 읽기를 완료해 주세요.", "Complete the Activity 2 sentence and reading."),
                )
    st.space("medium")
    if requested_tab_index is not None and not automatic_stage_completion:
        align_heading = st.session_state.pop("_intro_vocabulary_heading_unit", None) == current_unit["number"]
        render_lesson_tab_navigation(requested_tab_index, align_heading=align_heading)
    # 단원 핵심 정리와 3단계 복습은 활동 2를 완료한 뒤에만 공개합니다.
    # 검수 모드에서도 학습자 화면의 후속 내용은 잠금 규칙을 따릅니다.
    post_unit_unlocked = (
        unit_completed
        and selected_lesson_step >= 4
        and st.session_state.get(f"_unit_summary_open_{current_unit['number']}", False)
    )
    if not post_unit_unlocked:
        if unit_completed and selected_lesson_step >= 4:
            unlock_guide = interface_text("활동 2를 마쳤어요. ‘다음: 단원 정리·복습’ 버튼을 누르면 아래 내용이 열립니다.", "Activity 2 is complete. Select ‘Next: Unit summary and review’ to open the sections below.")
        elif unit_completed:
    ...
    review_description = interface_text(
        ...
        if current_unit["number"] in (1, 2, 3, 4, 5) else
    ...
    if review_vocab_done and review_grammar_done and review_sentence_done:
        ...
        lesson_button(interface_text("복습 다시 시작", "Restart review"), type="secondary", on_click=restart_review_routine, args=(current_unit["number"],))
    else:
    ...
    with r2:
        ...
        st.markdown(f'<div class="card" style="margin-bottom:16px"><div class="eyebrow">02 · Practice</div><h3>{grammar_review_title}</h3><p class="tiny">{grammar_review_copy}</p></div>', unsafe_allow_html=True)
        lesson_button(
    ...
    with r3:
        ...
        st.markdown(f'<div class="card" style="margin-bottom:16px"><div class="eyebrow">03 · Build</div><h3>{sentence_review_title}</h3><p class="tiny">{sentence_review_copy}</p></div>', unsafe_allow_html=True)
        lesson_button(
    ...
    if current_unit["number"] == 1:
        ...
        st.divider()
        if st.session_state.get("unit3_wrap_up_completed", False):
            render_learning_success(
                interface_text("수고했어요! 단원 핵심 정리와 3단계 복습까지 모두 마쳤어요. 3단원 학습을 성공적으로 마무리했어요 ✓",
                               "Well done! You completed the unit summary and all three review steps. Unit 3 is successfully complete ✓"),
                icon=":material/check_circle:",
            )
        elif unit3_wrap_up_ready():
            lesson_button(interface_text("3단원 학습 마무리하기", "Finish Unit 3"),
                          key="unit3_wrap_up_finish", type="primary", width="stretch",
                          on_click=complete_unit3_wrap_up)
        else:
            with st.container(border=True, key="unit3_wrap_up_guidance"):
                st.markdown(interface_text(
                    "✨ 마지막 정리 단계예요. 단원 핵심 정리와 3단계 복습을 완료하면 3단원 학습을 마무리할 수 있어요.",
                    "✨ This is the final wrap-up. Complete the unit summary and three-step review to finish Unit 3.",
                ))
    elif current_unit["number"] == 4:
        st.divider()
        if st.session_state.get("unit4_wrap_up_completed", False):
            render_learning_success(
                interface_text("수고했어요! 단원 핵심 정리와 3단계 복습까지 모두 마쳤어요. 4단원 학습을 성공적으로 마무리했어요 ✓",
                               "Well done! You completed the unit summary and all three review steps. Unit 4 is successfully complete ✓"),
                icon=":material/check_circle:",
            )
        elif unit4_wrap_up_ready():
            lesson_button(interface_text("4단원 학습 마무리하기", "Finish Unit 4"),
                          key="unit4_wrap_up_finish", type="primary", width="stretch",
                          on_click=complete_unit4_wrap_up)
        else:
            with st.container(border=True, key="unit4_wrap_up_guidance"):
                st.markdown(interface_text(
                    "✨ 마지막 정리 단계예요. 단원 핵심 정리와 3단계 복습을 완료하면 4단원 학습을 마무리할 수 있어요.",
                    "✨ This is the final wrap-up. Complete the unit summary and three-step review to finish Unit 4.",
                ))
    elif current_unit["number"] == 5:
        st.divider()
        if st.session_state.get("unit5_wrap_up_completed", False):
            render_learning_success(interface_text(
                "수고했어요! 3단계 복습을 모두 마쳤어요. 5단원 학습을 성공적으로 마무리했어요 ✓",
                "Well done! You completed all three review steps. Unit 5 is successfully complete ✓",
            ), icon=":material/check_circle:")
        elif unit5_wrap_up_ready():
            lesson_button(interface_text("5단원 학습 마무리하기", "Finish Unit 5"),
                          key="unit5_wrap_up_finish", type="primary", width="stretch",
                          on_click=complete_unit5_wrap_up)
        else:
            with st.container(border=True, key="unit5_wrap_up_guidance"):
                st.markdown(interface_text(
                    "✨ 마지막 정리 단계예요. 3단계 복습을 완료하면 5단원 학습을 마무리할 수 있어요.",
                    "✨ This is the final wrap-up. Complete the three-step review to finish Unit 5.",
                ))
...
with st.sidebar:
    st.markdown('''<div class="brand"><span class="brand-mark">✦</span><span class="instructor-name" aria-label="모모"><svg class="momo-logo" viewBox="0 0 72 32" role="img" aria-hidden="true"><path d="M3 4H27V17H3Z M15 17V28 M3 28H29"/><path class="momo-accent" d="M3 5L7 2 M27 5L23 2"/><rect x="39" y="3" width="27" height="14"/><path d="M52.5 17V28 M38 28H68"/><path d="M34 8V19Q34 23 39 23H59"/></svg><span class="momo-signature">MOMO</span></span><span class="course-name">의 한국어 강좌</span></div>''', unsafe_allow_html=True)
    ...
    theme_labels = {
        "한국어": {"title": "화면 색상", "black": "검은색", "nav": "메뉴", "unit": "학습 단원", "home": "학습 홈으로"},
    ...
    lesson_button(
        ...
        icon=":material/home:",
        key="go_home",
        on_click=restart_current_unit_from_home,
        args=(st.session_state.get("selected_unit_number", 1),),
    )
    home_help = interface_text(
        "현재 단원을 처음부터 다시 학습합니다. 완료 기록과 XP는 유지됩니다.",
        "Restart the current unit from the beginning. Completion history and XP are preserved.",
    )
    st.markdown(
        """
        <style>
        .home-button-guide { display:none; pointer-events:none; }
        [data-testid="stElementContainer"]:has(.home-button-guide) { margin:0; }
        .st-key-go_home:has(button:hover:not(:focus))
        + [data-testid="stElementContainer"] .home-button-guide,
        .st-key-go_home:has(button:focus-visible)
        + [data-testid="stElementContainer"] .home-button-guide {
            display:block; padding:8px 10px; margin:0 0 8px;
            border:1px solid #888; border-radius:8px;
            font-size:.82rem; line-height:1.45;
        }
        </style>
        """
        f'<div class="home-button-guide" role="note">{html.escape(home_help)}</div>',
        unsafe_allow_html=True,
    )
    available_units = unlocked_units(
        st.session_state,
        st.session_state.get("_lesson_history", {}),
        review_mode=REVIEW_MODE,
    )
    selected_unit_number = st.session_state.get("selected_unit_number", 1)
    if selected_unit_number not in available_units:
        selected_unit_number = available_units[-1]
        st.session_state.selected_unit_number = selected_unit_number
        st.session_state.pop("selected_unit_label", None)
    completed_units = {
        number for number in range(1, 11)
        if get_unit_completion_steps(number)[-1]
    }
    unit_options = []
    for number in range(1, 11):
        status = " ✓" if number in completed_units else (" 🔒" if number not in available_units else "")
        unit_options.append(
            f"{number}단원 · {TEXTBOOK_UNITS[number]['title'].split(' · ', 1)[-1]}{status}"
        )
    selected_option = unit_options[selected_unit_number - 1]
    if st.session_state.get("selected_unit_label") not in unit_options:
        st.session_state.pop("selected_unit_label", None)
    selected_unit_label = st.selectbox(
        ui["unit"], unit_options,
        index=selected_unit_number - 1,
        key="selected_unit_label",
        help=interface_text(
            "🔒 잠긴 단원은 이전 단원의 활동 2를 완료하면 열립니다.",
            "🔒 A locked unit opens after you complete Activity 2 in the previous unit.",
        ),
        on_change=enforce_unit_selection,
        args=(available_units, selected_option),
    )
    if "unit_access_notice" in st.session_state:
        locked_unit = st.session_state.pop("unit_access_notice")
        st.caption(interface_text(
            f"🔒 {locked_unit}단원은 이전 단원의 활동 2를 완료하면 열립니다.",
            f"🔒 Unit {locked_unit} opens after you complete Activity 2 in the previous unit.",
        ))
    ...
    completed_course_steps, total_course_steps, course_ratio = course_progress(
        st.session_state, st.session_state.get("_lesson_history", {})
    )
    course_percent = int(course_ratio * 100)
    st.progress(course_ratio)
    progress_copy = interface_text(
        f"세종한국어 1A · {course_percent}% 진행 · {completed_course_steps}/{total_course_steps}단계",
        f"Sejong Korean 1A · {course_percent}% · {completed_course_steps}/{total_course_steps} steps",
    )
    st.markdown(f"<span class='tiny'>{progress_copy}</span>", unsafe_allow_html=True)
